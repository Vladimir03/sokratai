...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (req.method === "GET" && segments.length === 3 && segments[2] === "signals") {
      ...
      const { data: revs, error: revsError } = await db
        .from("board_page_revs")
        .select("page_id, rev, updated_by, updated_at")
        .eq("board_id", guest.board_id);
      // Fail-closed (ревью синк-пасса, P1): сбой чтения НЕ маскируется пустым
      // «здоровым» снапшотом — клиент принял бы его за «изменений нет» (а
      // детектор удалённых листов счёл бы ВСЕ листы удалёнными). 503 → клиент
      // посчитает сбой и повторит следующим тиком.
      if (revsError || !revs) {
        return jsonError(503, "SIGNALS_READ_FAILED", "Не удалось прочитать обновления доски. Повторяем.");
      }
