...
async function handleRefine(
  ...
  const system = [
    systemPrompt,
    schemaBlock,
    'РЕЖИМ ПРАВКИ: тебе дают УЖЕ РАСПОЗНАННЫЙ черновик ОДНОЙ задачи и комментарий репетитора. Исправь черновик по комментарию (не меняя того, что репетитор не просил) и верни РОВНО ОДНУ задачу в массиве tasks по той же схеме.',
  ].join("\n\n");
