...
6. Классификация: определи exam ("ege"/"oge"/null), kim_number (ЕГЭ 1..25, ОГЭ
   ...
   «Политика», «Право». Это подсказки — репетитор подтвердит. Если рядом с
   номером задания указано «Тип N» (формат сборников РЕШУ ЕГЭ: «3. Тип 5
   № 8085») — это номер задания КИМ: kim_number = N. Порядковый номер задания
   в подборке номером КИМ НЕ является.
...
ТАБЛИЦА ОТВЕТОВ: если на странице или изображении есть СПИСОК/ТАБЛИЦА ответов
(пары «номер задания → ответ» БЕЗ условий) — верни её содержимое в поле
"answers_table" (массив объектов {"num": номер задания, "answer": "ответ"}).
НИКОГДА не создавай задания из строк таблицы ответов. Если условие задания и
его ответ из таблицы видны в ОДНОМ материале — сразу впиши ответ в поле answer
этого задания. Нет таблицы ответов → answers_table = [].
...
6. Классификация: exam ("ege"/"oge"/null — только если формат экзамена явно ясен
   ...
   по содержанию. Это подсказки — репетитор подтвердит. Пометка «Тип N» рядом с
   номером задания (формат сборников РЕШУ ЕГЭ: «3. Тип 5 № 8085») — это явно
   указанный номер задания КИМ: kim_number = N. Порядковый номер задания в
   подборке номером КИМ НЕ является.
...
/**
 * Кросс-чанковая таблица ответов (2026-07-17): нормализовать answers_table из
 * ответа модели. Сборники держат ответы отдельной таблицей — при чанках по 2
 * страницы модель чанка с таблицей не видит условий из других чанков, поэтому
 * таблица возвращается структурно, а КЛИЕНТ мержит по номерам задач.
 */
function normalizeAnswersTable(raw: unknown): Array<{ num: number; answer: string }> {
  if (!Array.isArray(raw)) return [];
  const out: Array<{ num: number; answer: string }> = [];
  for (const item of raw.slice(0, 200)) {
    if (typeof item !== "object" || item === null) continue;
    const rec = item as Record<string, unknown>;
    const num = typeof rec.num === "number" ? Math.trunc(rec.num) : NaN;
    const answer = typeof rec.answer === "string" ? rec.answer.trim().slice(0, 200) : "";
    if (Number.isFinite(num) && num >= 1 && num <= 999 && answer.length > 0) {
      out.push({ num, answer });
    }
  }
  return out;
...
): Promise<{ tasks: unknown[]; answersTable: Array<{ num: number; answer: string }> }> {
...
async function handleExtract(
  ...
  let answersTable: Array<{ num: number; answer: string }> = [];
  try {
    ...
    rawTasks = extraction.tasks;
    answersTable = extraction.answersTable;
  } catch {
  ...
  console.log("kb_ai_extract_done", {
    ...
    answers_table: answersTable.length,
  });

  // answers_table — кросс-чанковый мердж делает КЛИЕНТ (по номерам маркеров
  // текстового слоя); старый клиент поле игнорирует (deploy-skew-safe).
  return jsonOk(cors, { drafts, stats, answers_table: answersTable });
}
...
async function handleRefine(
  ...
  let rawTasks: unknown[];
  try {
    // Таблица ответов в refine не используется (правится одна задача).
    ({ tasks: rawTasks } = await runExtraction(messages, refineUsageLogger));
  } catch {
