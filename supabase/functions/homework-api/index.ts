...
// VALID_CEFR_LEVELS / VALID_WORK_MODES / нормализаторы / deriveTaskKind /
// isPositiveHalfStepNumber вынесены в `./validators.ts` (2026-08-05) —
// index.ts не импортируется в vitest (top-level Deno.serve), а инварианты
// rule 40 обязаны быть под тестами. См. импорт в шапке файла.
...
// isPositiveHalfStepNumber (шаг 0.5 у max_score, rule 40) — в ./validators.ts.
...
async function handleSetTutorScoreOverride(
  ...
  const { data: existing, error: tsErr } = await db
    .from("homework_tutor_task_states")
    .select("id, ai_score, tutor_score_override, earned_score, best_earned_score, ai_help_events, status, hint_count, wrong_answer_count, attempts, thread_id, task_id, tutor_force_completed_at")
  ...
  const { data: updated, error: updErr } = await db
    ...
    .eq("id", existing.id)
    .select("id, ai_score, tutor_score_override, tutor_score_override_comment, tutor_score_override_at, earned_score, best_earned_score, ai_help_events, status, hint_count, wrong_answer_count, attempts, thread_id, task_id, tutor_force_completed_at")
    .maybeSingle();
  if (updErr || !updated) {
...
// Anti-leak стрипы вынесены в `./student_strip.ts` (2026-08-05) — `index.ts`
// не импортируется в vitest (top-level `Deno.serve`), а стрипы обязаны быть
// под тестами: это второй слой анти-утечки (rule 40).
// stripHiddenMessages / stripStudentSensitiveTaskStateFields /
// stripIndependentPreRevealFields — см. импорт в шапке файла.
...
> {
  const { data, error } = await db
    .from("homework_tutor_task_states")
    // ai_help_events здесь НЕ селектится намеренно: снапшот отдаёт только балл,
    // метрика самостоятельности из него не считается (§22 требует полный набор
    // счётчиков у любого списка с ai_help_events).
    .select("status, earned_score, best_earned_score, ai_score, available_score")
    .eq("id", taskStateId)
    .maybeSingle();
  if (error || !data) {
...
async function handleStudentSubmission(
  ...
  const { data: task, error: taskError } = await db
    .from("homework_tutor_tasks")
    // smoke-allow: tutor-only-select серверный грейдинг читает эталон, в ответ ученику эти поля не уходят (§29)
