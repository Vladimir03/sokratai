...
// Браузерный канон: src/lib/taskOptions.ts (конвенция «mirror
// locally», как answer-alternatives/attachment-refs). Правишь логику — правь
 ...
 *
 * Границы слова — через группы `(^|[^\p{L}\p{N}])`, НЕ lookbehind: модуль
 * зеркалится в браузерный src/lib/taskOptions.ts, где lookbehind запрещён
 * (Safari < 16.4, rule 80). Цифра внутри числа («1861») не считается.
 */
