...
-- Plan: ~/.claude/plans/1-glowing-spindle.md. Depends on M1 (20260615150000) helpers.
--
-- ⚠️ rule-10 FROZEN money RPCs — тела скопированы VERBATIM из последних активных определений; ЕДИНСТВЕННЫЕ
-- добавления помечены `-- CREDIT`. Payment/group/return-поведение НЕ меняется. ОБЯЗАТЕЛЬНОЕ независимое
-- ревью этого диффа перед деплоем.
--
-- VERBATIM sources:
--   complete_lesson_and_create_payment, update_group_participant_payment_status,
--   tutor_delete_lessons, tutor_revert_lesson  ← 20260610155050_606b568b-...sql
--   mark_payment_as_paid_by_telegram (сигнатура (_payment_id UUID, _telegram_id TEXT)) ← 20260226130718_ea74a69b-...sql
--
-- Контракт CREDIT: оплачено → _sync_lesson_credit (net 0 с debit); НЕ оплачено / снятие → _reverse_lesson_credit.
-- Покрытые payment-write-сайты (rule-40 dual-write — grep INSERT/UPDATE tutor_payments status='paid'):
--   complete (group+individual), group-toggle, Telegram /pay. tutor_confirm_lessons (bulk) — транзитивно
--   через complete. revert/delete — сторнируют И debit, И credit (фикс латентного бага: revert оплаченного
--   занятия сейчас сторнит только debit → баланс прыгает +amount).

-- ════════════════════════════════════════════════════════════════════════════
-- 1. complete_lesson_and_create_payment — verbatim + _sync_lesson_credit / _reverse_lesson_credit
-- ════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  _is_paid := _payment_status IN ('paid', 'paid_earlier');  -- CREDIT
  ...
  IF _is_group THEN
    ...
    LOOP
      IF _participant.payment_amount IS NOT NULL AND _participant.payment_amount > 0 THEN
        ...
        IF _is_paid THEN                                                                   -- CREDIT
          PERFORM public._sync_lesson_credit(                                              -- CREDIT
            _lesson_id, _participant.tutor_student_id, _tutor_id, _participant.payment_amount, _actor);
        ELSE                                                                               -- CREDIT
          PERFORM public._reverse_lesson_credit(_lesson_id, _participant.tutor_student_id);-- CREDIT
        END IF;                                                                            -- CREDIT
      END IF;
    ...
    IF _resolved_amount IS NOT NULL AND _tutor_student_id IS NOT NULL THEN
      ...
      IF _is_paid THEN                                                          -- CREDIT
        PERFORM public._sync_lesson_credit(                                     -- CREDIT
          _lesson_id, _tutor_student_id, _tutor_id, _resolved_amount, _actor);
      ELSE                                                                      -- CREDIT
        PERFORM public._reverse_lesson_credit(_lesson_id, _tutor_student_id);   -- CREDIT
      END IF;                                                                   -- CREDIT
    END IF;
  END IF;
...
-- ════════════════════════════════════════════════════════════════════════════
-- 2. update_group_participant_payment_status — verbatim + credit toggle
-- ════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  IF _participant_amount IS NOT NULL AND _participant_amount > 0 THEN
    ...
    IF _payment_status = 'paid' THEN                                         -- CREDIT
      PERFORM public._sync_lesson_credit(                                    -- CREDIT
        _lesson_id, _tutor_student_id, _tutor_id, _participant_amount, _actor);
    ELSE                                                                     -- CREDIT
      PERFORM public._reverse_lesson_credit(_lesson_id, _tutor_student_id);  -- CREDIT
...
-- ════════════════════════════════════════════════════════════════════════════
-- 3. mark_payment_as_paid_by_telegram — verbatim + credit on lesson-linked paid
-- ════════════════════════════════════════════════════════════════════════════
...
DECLARE
  _tutor_id     UUID;
  _rows_updated INT;
  _led_lesson   UUID;                        -- CREDIT
  _led_student  UUID;                        -- CREDIT
  _led_amount   INTEGER;                      -- CREDIT
  _actor        UUID;                         -- CREDIT
BEGIN
  ...
  RETURNING tp.lesson_id, tp.tutor_student_id, ROUND(tp.amount)::int  -- CREDIT
    INTO _led_lesson, _led_student, _led_amount;                       -- CREDIT

  GET DIAGNOSTICS _rows_updated = ROW_COUNT;

  -- CREDIT (review round-2 #1): любая «деньги получены» через /pay → зачёт на баланс, идемпотентно.
  IF _rows_updated > 0 AND _led_amount IS NOT NULL AND _led_amount > 0 THEN
    ...
    ELSE
      -- legacy ручная строка (`lesson_id IS NULL`) — идемпотентный credit по source_payment_id.
...
-- ════════════════════════════════════════════════════════════════════════════
-- 4. tutor_delete_lessons — verbatim + reverse active lesson-CREDITS too
-- ════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- Сторно active lesson-debit И lesson-credit ДО удаления строк занятий (source_lesson_id → SET NULL
  -- иначе потеряет связь). Delete блокируется на paid выше → credit-цикл обычно no-op (defensive).
  FOR _led IN
    SELECT id FROM public.tutor_ledger_entries
    WHERE source_lesson_id = ANY (_delete_ids)
      AND source_kind = 'lesson' AND kind IN ('debit', 'credit') AND reversed_by_entry_id IS NULL  -- CREDIT
  LOOP
...
-- ════════════════════════════════════════════════════════════════════════════
-- 5. tutor_revert_lesson — verbatim + reverse active lesson-CREDITS (фикс фантома +amount)
-- ════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- Сторно И debit, И credit (отмена занятия = и начисление, и оплата откатываются → net 0).
  -- Без credit-цикла revert оплаченного занятия сторнил бы только debit → баланс прыгал +amount (фантом).
  FOR _led IN
    SELECT id FROM public.tutor_ledger_entries
...
-- ════════════════════════════════════════════════════════════════════════════
-- 6. Defense-in-depth (review round-2 #4): закрыть прямой клиентский DML на tutor_payments
-- ════════════════════════════════════════════════════════════════════════════
-- Legacy `createTutorPayment`/`markPaymentAsPaid`/`deleteTutorPayment` (прямой PostgREST на tutor_payments)
-- минуют ledger → bypass зачёта. После reframe «Оплаты» они МЁРТВЫЕ (0 UI-вызовов), но stale-вкладка
-- старого бандла в окне deploy-skew могла бы их вызвать. REVOKE write от authenticated → такой вызов
-- падает (fail-loud), а не тихо минует баланс. SECURITY DEFINER money-RPC (complete/group-toggle/`/pay`)
-- работают как owner → НЕ затронуты. SELECT оставлен (нужен `getTutorPayments`/`useTutorHomeData`).
REVOKE INSERT, UPDATE, DELETE ON public.tutor_payments FROM authenticated;

-- NOTE (review round-2 #5, осознанно не чиним здесь): reverse-циклы в tutor_revert_lesson/tutor_delete_lessons
-- берут ledger-row-локи ДО lesson-row-UPDATE, а complete-RPC — наоборот (lesson-row → ledger-row).
-- Обратный порядок → теоретический 40P01 при конкурентных revert+re-complete ОДНОГО занятия. Это
-- ПРЕД-СУЩЕСТВУЕТ (debit-loop в 2a имел тот же паттерн; credit добавлен в тот же loop, порядок не изменён)
-- и НЕ коррумпирует деньги (deadlock → чистый атомарный rollback, retryable). Полный фикс требует брать
-- advisory-lock до `UPDATE tutor_lessons` в frozen complete-RPC — отложено как отдельный осознанный раунд.
