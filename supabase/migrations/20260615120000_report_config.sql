-- «Отчёт родителю» v2 (ОС Елены, 2026-06-15) — конфиг отчёта на ссылке + память выбора оплаты.
-- Аддитивно (rule 95): новые nullable/defaulted колонки на student_report_links + tutors.
-- RLS student_report_links («Tutors manage own report links», owns_tutor_student) уже
-- покрывает новые колонки; table-level GRANT тоже. tutors.report_show_debt_default —
-- память последнего выбора «показывать оплату» (mirror клиентского update mini_groups_enabled).
...
-- CHECK-констрейнты идемпотентно (повторный прогон миграции не падает).
DO $$ BEGIN
  ALTER TABLE public.student_report_links
...
-- Память последнего выбора «показывать оплату» (tutor-level) — prefill нового отчёта.
...
-- «Одна активная ссылка на ученика» — закрепляем в БД (гонка двух вкладок → две активные
-- ссылки; revoke гасит показанную, вторая остаётся рабочей у родителя). Сначала дедуп:
-- гасим все активные, кроме самой свежей по каждому ученику (tie-break по slug — детерминизм).
...
-- Энтропия slug: отчёт несёт имя+баллы+баланс (чувствительнее homework-preview) → для НОВЫХ
-- ссылок 24 hex (96 бит) вместо 8 (32 бита). Legacy 8-символьные slug продолжают работать
-- (regex на клиенте/edge ослаблены до {8,64}).
ALTER TABLE public.student_report_links
