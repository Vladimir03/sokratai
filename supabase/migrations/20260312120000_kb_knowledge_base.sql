...
-- Additive migration only — does NOT alter any existing homework_tutor_* tables.

-- ══════════════════════════════════════════════════════════════
-- 0) Enum: exam_type (safe idempotent creation)
-- ══════════════════════════════════════════════════════════════
...
-- ══════════════════════════════════════════════════════════════
-- 1) КАТАЛОГ СОКРАТА (read-only для репетиторов)
-- ══════════════════════════════════════════════════════════════
...
-- ══════════════════════════════════════════════════════════════
-- 2) ЛИЧНАЯ БАЗА РЕПЕТИТОРА (папки, рекурсивная структура)
-- ══════════════════════════════════════════════════════════════
...
CREATE TABLE IF NOT EXISTS public.kb_tasks (
  ...
  -- Two-space model: every task must belong to catalog (topic) or personal base (folder)
  CONSTRAINT kb_tasks_space_check CHECK (topic_id IS NOT NULL OR folder_id IS NOT NULL)
);

-- ══════════════════════════════════════════════════════════════
-- 4) МАТЕРИАЛЫ (каталог ИЛИ личная папка)
-- ══════════════════════════════════════════════════════════════
...
-- ══════════════════════════════════════════════════════════════
-- 5) HOMEWORK ↔ KB LINK (snapshot при добавлении в ДЗ)
--    FK → homework_tutor_assignments (existing table, NOT altered)
-- ══════════════════════════════════════════════════════════════

CREATE TABLE IF NOT EXISTS public.homework_kb_tasks (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  -- Links to existing homework system (NOT altered)
  homework_id UUID NOT NULL REFERENCES public.homework_tutor_assignments(id) ON DELETE CASCADE,
  -- SET NULL: if task deleted from KB, snapshot survives
  task_id UUID REFERENCES public.kb_tasks(id) ON DELETE SET NULL,
  sort_order INTEGER DEFAULT 0,

  -- Snapshot: frozen at "В ДЗ" click time
...
-- ══════════════════════════════════════════════════════════════
-- 7b) SECURITY DEFINER helper: cross-owner folder validation
--     Prevents referencing another user's folder in parent_id / folder_id
-- ══════════════════════════════════════════════════════════════
...
AS $$
  SELECT EXISTS (
    SELECT 1 FROM kb_folders
    WHERE id = _folder_id AND owner_id = _owner_id
  )
$$;

-- ══════════════════════════════════════════════════════════════
-- 8) RLS POLICIES — kb_topics & kb_subtopics (read-only for all authenticated)
-- ══════════════════════════════════════════════════════════════

CREATE POLICY "KB topics select all authenticated"
  ON public.kb_topics
  FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "KB subtopics select all authenticated"
  ON public.kb_subtopics
  FOR SELECT
  TO authenticated
  USING (true);

-- ══════════════════════════════════════════════════════════════
-- 9) RLS POLICIES — kb_folders (owner-only CRUD)
-- ══════════════════════════════════════════════════════════════

CREATE POLICY "KB folders select own"
  ON public.kb_folders
  FOR SELECT
  TO authenticated
  USING (owner_id = auth.uid());

CREATE POLICY "KB folders insert own"
  ON public.kb_folders
  FOR INSERT
  TO authenticated
  WITH CHECK (
...
CREATE POLICY "KB folders update own"
  ON public.kb_folders
  FOR UPDATE
  TO authenticated
  USING (owner_id = auth.uid())
...
CREATE POLICY "KB folders delete own"
  ON public.kb_folders
  FOR DELETE
  TO authenticated
  USING (owner_id = auth.uid());
...
-- ══════════════════════════════════════════════════════════════
-- 10) RLS POLICIES — kb_tasks
--     Catalog (owner_id IS NULL): readable by all authenticated
--     Personal (owner_id = user): full CRUD for owner only
-- ══════════════════════════════════════════════════════════════
...
CREATE POLICY "KB tasks insert own"
  ON public.kb_tasks
  FOR INSERT
  TO authenticated
  WITH CHECK (
...
CREATE POLICY "KB tasks update own"
  ON public.kb_tasks
  FOR UPDATE
  TO authenticated
  USING (owner_id = auth.uid())
...
CREATE POLICY "KB tasks delete own"
  ON public.kb_tasks
  FOR DELETE
  TO authenticated
  USING (owner_id = auth.uid());
...
-- ══════════════════════════════════════════════════════════════
-- 11) RLS POLICIES — kb_materials (same pattern as kb_tasks)
-- ══════════════════════════════════════════════════════════════

CREATE POLICY "KB materials select public or own"
  ON public.kb_materials
  FOR SELECT
  TO authenticated
  USING (
    owner_id IS NULL
...
CREATE POLICY "KB materials insert own"
  ON public.kb_materials
  FOR INSERT
  TO authenticated
  WITH CHECK (
...
CREATE POLICY "KB materials update own"
  ON public.kb_materials
  FOR UPDATE
  TO authenticated
  USING (owner_id = auth.uid())
...
CREATE POLICY "KB materials delete own"
  ON public.kb_materials
  FOR DELETE
  TO authenticated
  USING (owner_id = auth.uid());
...
-- ══════════════════════════════════════════════════════════════
-- 12) RLS POLICIES — homework_kb_tasks
--     Tutor: full CRUD via assignment ownership
--     Student: read-only for assigned homework
-- ══════════════════════════════════════════════════════════════

-- Helper: avoids RLS recursion when checking assignment ownership
...
AS $$
  SELECT EXISTS (
    ...
    WHERE id = _homework_id AND tutor_id = auth.uid()
  )
$$;

-- Tutor policies
CREATE POLICY "KB hw tasks select by tutor"
  ON public.homework_kb_tasks
  FOR SELECT
  TO authenticated
  USING (public.is_kb_homework_tutor(homework_id));

CREATE POLICY "KB hw tasks insert by tutor"
  ON public.homework_kb_tasks
  FOR INSERT
  TO authenticated
  WITH CHECK (public.is_kb_homework_tutor(homework_id));

CREATE POLICY "KB hw tasks update by tutor"
  ON public.homework_kb_tasks
  FOR UPDATE
  TO authenticated
  USING (public.is_kb_homework_tutor(homework_id))
...
CREATE POLICY "KB hw tasks delete by tutor"
  ON public.homework_kb_tasks
  FOR DELETE
  TO authenticated
  USING (public.is_kb_homework_tutor(homework_id));

-- Student: read snapshots only for active/closed homework (not draft)
CREATE POLICY "KB hw tasks select by student"
  ON public.homework_kb_tasks
  FOR SELECT
  TO authenticated
  USING (
