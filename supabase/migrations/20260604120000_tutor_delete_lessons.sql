-- =============================================================================
-- Schedule Google-Calendar parity — Round A: safe hard-delete of lessons.
--
-- Bug: tutors could only CANCEL (soft, status='cancelled', leaves a ghost). The
-- raw client deleteLesson() is unsafe: tutor_payments.lesson_id → SET NULL
-- (orphans paid money), parent_lesson_id → SET NULL (breaks the series),
-- participants/materials → CASCADE.
--
-- This RPC is the ONLY safe delete path. It:
--   • resolves the affected set by scope ('this' | 'this_and_following' | 'all')
--   • MONEY GUARD: removes pending/overdue payments for the set, but RAISEs if any
--     affected lesson has a PAID payment (never silently orphan paid money)
--   • re-parents the series if the root is deleted (avoids orphaned children)
--   • then DELETEs the rows (participants/materials cascade — desired for a true delete)
--
-- Ownership: tutor_lessons.tutor_id → tutors.id, gated on tutors.user_id = auth.uid()
-- (mirror update_lesson_series). Forward note: under the v2 balance model this
-- swaps pending-payment-removal for ledger-debit reversal.
-- =============================================================================
