-- Student balance ledger — Phase 2a, TASK-6: правка пополнения (атомарный reverse+new).
-- SPEC v7 (TASK-6). Append-only сохраняется: «правка» = сторно старой записи + новая запись
-- в ОДНОЙ транзакции; новая запись ссылается на старую через replaces_entry_id (для свёрнутого
-- отображения «исправлено: было 4000 → стало 400» в ленте операций).
-- Units: РУБЛИ integer. Lesson-debit'ы здесь НЕ правятся — их канонический путь = re-complete
-- занятия (complete_lesson_and_create_payment, amount-aware) / tutor_revert_lesson (rule 60).

-- ─── Связь «новая запись заменяет старую» ─────────────────────────────────────
...
BEGIN
  ...
  -- Правится только пополнение (topup credit). Lesson-debit — через занятие; offsetting-строки — никогда.
  ...
  IF _rev IS NULL THEN RAISE EXCEPTION 'ALREADY_REVERSED'; END IF;  -- concurrent edit/cancel lost the race
