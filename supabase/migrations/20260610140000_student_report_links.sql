-- «Отчёт родителю» (Phase 2c, scheduling-payments-balance) — share-ссылки на публичный
-- read-only отчёт ученика (/p/report/:slug). Mirror homework_share_links (rule 40):
-- slug = bearer-token (знание ссылки = доступ); tutor CRUD через PostgREST + RLS;
-- публичное чтение — ТОЛЬКО service_role edge (public-student-report).
-- Отзыв ссылки = revoked_at (страница говорит «ссылка больше не действует»).
