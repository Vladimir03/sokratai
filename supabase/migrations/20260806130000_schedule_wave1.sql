...
-- Аддитивно; существующие строки валидны (в проде color нигде не заполнен, recurrence_rule только weekly).

-- 1) Цвет индивидуального ученика (NULL = без цвета; значения — из пресет-палитры на фронте)
ALTER TABLE public.tutor_students ADD COLUMN IF NOT EXISTS color TEXT;

-- Формат #RRGGBB. Защитный UPDATE перед CHECK — на случай мусора в non-prod окружениях.
...
-- 2) Тот же формат-CHECK на существующий tutor_groups.color
...
-- 3) Тема урока. Ученику отдаётся ТОЛЬКО через edge-whitelist student-lessons-api (rule 98),
--    клиентского select для ученика на tutor_lessons нет.
ALTER TABLE public.tutor_lessons ADD COLUMN IF NOT EXISTS topic TEXT;

-- 4) Периоды повтора серий: daily / weekly / biweekly / monthly.
--    Существующие строки — только 'weekly' (biweekly разрешён старым CHECK, но не писался) → superset валиден.
