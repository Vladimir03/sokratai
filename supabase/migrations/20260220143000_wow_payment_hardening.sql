...
BEGIN
  -- Ownership check: by telegram_id for bot calls, by auth.uid() for web calls.
  ...
  -- Idempotent upsert by lesson_id (manual payments stay with lesson_id = NULL).
...
-- 3) Debt aggregation RPC (pending + overdue)
