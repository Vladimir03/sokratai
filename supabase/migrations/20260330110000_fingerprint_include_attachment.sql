...
-- for image-based tasks with empty text fields.
--
-- Problem: tasks with conditions on images (empty text) and same numeric
-- answer (e.g. "50") got identical fingerprints → hidden_duplicate.
--
-- Solution: add attachment_url to fingerprint hash.
-- ════════════════════════════════════════════
...
BEGIN
  ...
  -- Compute fingerprint (now includes attachment_url)
...
BEGIN
  ...
  -- Compute new fingerprint (now includes attachment_url)
...
BEGIN
  ...
  -- Check if text, answer, or attachment changed
...
-- 5) Recompute fingerprints on ALL catalog tasks (owner_id IS NULL)
-- ════════════════════════════════════════════
...
--    After recomputing fingerprints, tasks that were hidden_duplicate
--    may now have unique fingerprints → promote to active.
-- ════════════════════════════════════════════
...
WHERE owner_id IS NULL
  ...
  AND NOT EXISTS (
    -- Only unhide if no OTHER active task shares the same (new) fingerprint
