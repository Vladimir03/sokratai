...
-- Прежний check-then-write в edge (SELECT rev → сравнение → UPDATE elements →
-- SELECT/UPDATE rev) не атомарен: два устройства с одинаковым base_rev проходили
-- проверку оба, второе молча затирало первое, а бамп терял инкременты.
-- Теперь CAS и бамп — одна транзакция под FOR UPDATE. Идемпотентна.

-- ─── 1. Сохранение elements с CAS по rev ──────────────────────────────────────

CREATE OR REPLACE FUNCTION public.wb_save_page_elements(
  p_page_id uuid,
  p_elements jsonb,
  -- NULL = без CAS-проверки (переходный клиент без base_rev); бамп всё равно атомарный.
  p_base_rev bigint,
  p_updated_by text
) RETURNS jsonb
LANGUAGE plpgsql
...
-- ─── 2. Атомарный бамп сигнала (создание листа, смена зоны/разлиновки) ────────
-- Прежний SELECT → UPDATE в двух edge терял инкременты при конкуренции.
...
-- ─── Гранты: только service_role (rule 99 — тройной REVOKE, порядок важен) ────
