...
CREATE POLICY "HW tutor tasks select by assignment owner"
  ...
  FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM public.homework_tutor_assignments a WHERE a.id = homework_tutor_tasks.assignment_id AND a.tutor_id = auth.uid()));

CREATE POLICY "HW tutor tasks insert by assignment owner"
  ON public.homework_tutor_tasks
  FOR INSERT TO authenticated
  WITH CHECK (EXISTS (SELECT 1 FROM public.homework_tutor_assignments a WHERE a.id = homework_tutor_tasks.assignment_id AND a.tutor_id = auth.uid()));
...
CREATE POLICY "HW tutor student assignments select by owner"
  ...
  FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM public.homework_tutor_assignments a WHERE a.id = homework_tutor_student_assignments.assignment_id AND a.tutor_id = auth.uid()) AND public.is_tutor_of_student(homework_tutor_student_assignments.student_id));

CREATE POLICY "HW tutor student assignments insert by owner"
  ON public.homework_tutor_student_assignments
  FOR INSERT TO authenticated
  WITH CHECK (EXISTS (SELECT 1 FROM public.homework_tutor_assignments a WHERE a.id = homework_tutor_student_assignments.assignment_id AND a.tutor_id = auth.uid()) AND public.is_tutor_of_student(homework_tutor_student_assignments.student_id));
...
CREATE POLICY "HW tutor submissions select by assignment owner"
  ...
  FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM public.homework_tutor_assignments a WHERE a.id = homework_tutor_submissions.assignment_id AND a.tutor_id = auth.uid()));

CREATE POLICY "HW tutor submissions update by assignment owner"
  ON public.homework_tutor_submissions
  FOR UPDATE TO authenticated
  USING (EXISTS (SELECT 1 FROM public.homework_tutor_assignments a WHERE a.id = homework_tutor_submissions.assignment_id AND a.tutor_id = auth.uid()))
  WITH CHECK (EXISTS (SELECT 1 FROM public.homework_tutor_assignments a WHERE a.id = homework_tutor_submissions.assignment_id AND a.tutor_id = auth.uid()));

CREATE POLICY "HW tutor submission items select by assignment owner"
  ON public.homework_tutor_submission_items
  FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM public.homework_tutor_submissions s JOIN public.homework_tutor_tasks t ON t.id = homework_tutor_submission_items.task_id JOIN public.homework_tutor_assignments a ON a.id = s.assignment_id WHERE s.id = homework_tutor_submission_items.submission_id AND t.assignment_id = s.assignment_id AND a.tutor_id = auth.uid()));
...
CREATE POLICY "HW students select assigned assignments"
  ON public.homework_tutor_assignments
  FOR SELECT TO authenticated
  USING (status IN ('active', 'closed') AND EXISTS (SELECT 1 FROM public.homework_tutor_student_assignments hsa WHERE hsa.assignment_id = homework_tutor_assignments.id AND hsa.student_id = auth.uid()));

CREATE POLICY "HW students select tasks of assigned assignments"
  ON public.homework_tutor_tasks
  FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM public.homework_tutor_assignments a JOIN public.homework_tutor_student_assignments hsa ON hsa.assignment_id = a.id WHERE a.id = homework_tutor_tasks.assignment_id AND a.status IN ('active', 'closed') AND hsa.student_id = auth.uid()));
...
CREATE POLICY "HW students insert own submissions"
  ON public.homework_tutor_submissions
  FOR INSERT TO authenticated
  WITH CHECK (student_id = auth.uid() AND EXISTS (SELECT 1 FROM public.homework_tutor_assignments a JOIN public.homework_tutor_student_assignments hsa ON hsa.assignment_id = a.id WHERE a.id = homework_tutor_submissions.assignment_id AND hsa.student_id = auth.uid() AND a.status IN ('active', 'closed')));
...
CREATE POLICY "HW students update own in progress submissions"
  ...
  WITH CHECK (student_id = auth.uid() AND status IN ('in_progress', 'submitted') AND EXISTS (SELECT 1 FROM public.homework_tutor_assignments a JOIN public.homework_tutor_student_assignments hsa ON hsa.assignment_id = a.id WHERE a.id = homework_tutor_submissions.assignment_id AND hsa.student_id = auth.uid() AND a.status IN ('active', 'closed')));

CREATE POLICY "HW students insert own submission items"
  ON public.homework_tutor_submission_items
  FOR INSERT TO authenticated
  WITH CHECK (EXISTS (SELECT 1 FROM public.homework_tutor_submissions s JOIN public.homework_tutor_tasks t ON t.id = homework_tutor_submission_items.task_id WHERE s.id = homework_tutor_submission_items.submission_id AND t.assignment_id = s.assignment_id AND s.student_id = auth.uid() AND s.status = 'in_progress'));

CREATE POLICY "HW students select own submission items"
  ON public.homework_tutor_submission_items
  FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM public.homework_tutor_submissions s JOIN public.homework_tutor_tasks t ON t.id = homework_tutor_submission_items.task_id WHERE s.id = homework_tutor_submission_items.submission_id AND t.assignment_id = s.assignment_id AND s.student_id = auth.uid()));

CREATE POLICY "HW students update own in progress submission items"
  ON public.homework_tutor_submission_items
  FOR UPDATE TO authenticated
  USING (EXISTS (SELECT 1 FROM public.homework_tutor_submissions s JOIN public.homework_tutor_tasks t ON t.id = homework_tutor_submission_items.task_id WHERE s.id = homework_tutor_submission_items.submission_id AND t.assignment_id = s.assignment_id AND s.student_id = auth.uid() AND s.status = 'in_progress'))
  WITH CHECK (EXISTS (SELECT 1 FROM public.homework_tutor_submissions s JOIN public.homework_tutor_tasks t ON t.id = homework_tutor_submission_items.task_id WHERE s.id = homework_tutor_submission_items.submission_id AND t.assignment_id = s.assignment_id AND s.student_id = auth.uid() AND s.status = 'in_progress'));
...
WITH CHECK (
  ...
  AND EXISTS (
    SELECT 1 FROM public.homework_tutor_submissions s
