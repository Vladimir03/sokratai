...
--
-- WHY (incident 2026-06-15): there is NO automated tutor-tariff («AI-старт») payment
-- path — yookassa-create-payment is hardcoded to the student Premium plan (699₽/30d).
-- A paying tutor's plan therefore lived only as a manual edit of
-- profiles.subscription_tier='premium' (+ subscription_expires_at). A missed / expired /
-- mistyped manual edit silently capped that tutor's students at the FREE 10/day homework
-- limit instead of 50 — because get_subscription_status (20260512134831_*) decides the
-- 50/day boost by reading the *tutor's* profiles.subscription_tier. Elena's pilot students
-- hit exactly this. These RPCs replace error-prone raw SQL with an admin-gated, audited op.
--
-- SINGLE SOURCE OF TRUTH for «is this tutor paid»: profiles.subscription_tier='premium'
-- (and subscription_expires_at NULL or > now()) OR profiles.trial_ends_at > now().
-- These RPCs only ever touch the premium fields (tier + expires_at); trial is left alone.
--
-- Admin gate reuses the canonical public.is_admin(uid) (= has_role(uid,'admin')
-- OR is_admin_email(uid), SECURITY DEFINER, 20251218205417_*).

-- ── Audit log (append-only; никогда не редактируем строки — только новые) ──────────────
...
-- Только админы читают историю; запись — ИСКЛЮЧИТЕЛЬНО через SECURITY DEFINER RPC ниже.
...
begin
  ...
  order by 7 asc, 2 asc; -- is_paid (false=НЕ оплачен first), then email
end;
$$;

-- ── Write: grant premium until a date (admin only, audited) ───────────────────────────
...
-- ── Write: revoke premium (set back to free, audited) ─────────────────────────────────
...
comment on function public.admin_grant_tutor_plan(text, timestamptz, text) is
  'Admin-only: set profiles.subscription_tier=premium + subscription_expires_at for a tutor (by email). '
  'Makes their students get the 50/day homework AI limit. Audited in admin_tutor_plan_grants.';
