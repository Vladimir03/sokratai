...
### 30. Mock-exams OCR & AI Grading Recovery (2026-05-21, TASK-OCR-1)

**Trigger:** Egor's pilot 2026-05-21 — submitted attempts с фото загружены, но AI-черновик не пришёл. Tutor видит пустую `TutorMockExamReview` («AI не успел»). Все 6 Часть 2 карточек fallback'ы; Часть 1 OCR не сработал. Vladimir: «AI должен помочь, а не помогает».

**Root cause (P0 race):** `mock-exam-student-api::handleSubmitAttempt` ставил `status='ai_checking'` напрямую (BEFORE UPDATE trigger освежал `updated_at`); fire-and-forget вызывал `mock-exam-grade::handleGrade`, который видел свежий `ai_checking` (< 120s) и возвращал 202 ALREADY_GRADING. **Grader никогда не запускался.**

**Fix (additive, no migrations):**

1. **`mock-exam-student-api::handleSubmitAttempt`** — submit оставляет `status='submitted'` (queued for AI), submitted_at=now. Frontend response теперь `status: "submitted"`. Канонический state-machine: `in_progress → submitted → ai_checking → awaiting_review → approved`. Grader сам делает CAS-claim submitted → ai_checking.

2. **`_shared/mock-exam-part1-ocr.ts::buildPart1FreeformOCRPrompt`** (NEW) — prompt для произвольного фото Часть 1 (тетрадный лист / черновик / скан, не ФИПИ-бланк). Same JSON output shape как `buildPart1BlankOCRPrompt` (sanitizer reused). `buildPart1BlankOCRPrompt` (FIPI grid) сохраняется.

3. **`mock-exam-grade::runPart1OCR`** — добавлен parameter `promptMode: 'blank' | 'freeform'`. По нему выбирается prompt builder. `handleGrade::shouldRunPart1OCR` derives `ocrPath` из `attempt.blank_photo_url` (canonical ФИПИ → mode='blank') ИЛИ `attempt.part1_blank_photo_url` (fallback → mode='freeform'). OCR теперь работает на любом фото Часть 1.

4. **`mock-exam-tutor-api::handleRetryPart1OCR`** — принимает оба пути (`blank_photo_url` OR `part1_blank_photo_url`). Также accepts `ai_checking` IF stale (`updated_at` > 120s) — для recovery stuck attempts (Egor's case). Grader сам делает stale-lock-claim.

5. **`MAX_PROMPT_IMAGE_BYTES`** в mock-exam-grade: 5MB → 8MB. Upload cap = 10MB, Gemini Vision API tail limit = 20MB. Меньше silent `image_inline_failed` rejection для HEIC / legacy фото.

6. **`__meta.prompt_mode`** добавлен в `ai_part1_ocr_json` (success И failure snapshots). `MockExamPart1OCRMetaSuccess.prompt_mode?: 'blank' | 'freeform'` optional для backward compat с Phase 6 pilot attempts.

**Hard invariants:**

- **State machine canonical**: `handleSubmitAttempt` НИКОГДА не ставит `ai_checking` напрямую. Только `submitted`. Grader делает CAS. При расширении flow — не нарушать.
- **OCR scope теперь broader**: OCR triggers на `(blank_photo_url || part1_blank_photo_url) && answer_method='blank'`. Не на per-task Часть 1 photo upload (этого нет в схеме) и не на bulk Часть 2 photos (там Pass 1 bulk assignment + Pass 2 grading).
- **`score_source` provenance preserved** (CLAUDE.md §25): OCR upserts `score_source='ocr'`; tutor manual `'tutor'` не overwritten при retry. Никаких изменений в RLS.
- **Anti-leak preserved**: `solution_text` / `correct_answer` (Часть 2) / `rubric_*` / `ai_draft_json` / `ai_part1_ocr_json` НЕ leak до approval. Frontend types extension `prompt_mode?` — additive, не student-facing reveal.
- **Backward compat для pilot attempts**: existing `ai_part1_ocr_json` rows без `prompt_mode` остаются валидны (optional type field). Phase 6 attempts продолжают работать.
- **Stale lock recovery 120s**: единый порог во всех 3 callsite (grader CAS, grader's else-branch, retry-part1-ocr). При изменении — обновить все 3.

**Out of scope (deferred):**

- `mock_exam_grading_jobs` table + queue: пилот scale (5-6 фото / неделя) не требует. Очередь — когда blowouts.
- `OPENROUTER_API_KEY` integration: Lovable Gateway уже даёт `google/gemini-2.5-pro` (использован для OCR). Env var может появиться как opt-in для будущей model ladder (`anthropic/claude-sonnet-4.5` fallback).
- `EdgeRuntime.waitUntil`: Supabase Edge Functions API не публичное. Текущий fire-and-forget с `.catch()` adequate для < 1s enqueue.

**При расширении OCR pipeline:**

1. Новый prompt mode (e.g. `'bulk_freeform'`, `'ocr_table'`) → расширяй `MockExamPart1OCRPromptMode` union в `src/types/mockExam.ts` И Deno-mirror в `_shared/mock-exam-part1-ocr.ts`. Sanitizer перепиши только если меняется output shape.
2. Новый OCR engine (не Gemini) → extend `LOVABLE_MODEL_OCR` через env var + сохраняй `__meta.provider` помимо `gemini_model`.
3. Stuck attempts recovery: всегда добавляй stale-lock path (mirror retry-part1-ocr 120s logic), не блокируй tutor вечно по `ai_checking`. Применяется к **обоим** manual triggers — `retry-part1-ocr` И `regrade-part2` (Round 2 fix mirrors them).
4. Photo upload cap bump: `MAX_PROMPT_IMAGE_BYTES` ≤ `MAX_PHOTO_BYTES` - 2MB margin (sufficient для base64 overhead).

**Round 2 hard invariants (sticky banner + RU errors + regrade stale-lock, 2026-05-21):**

- **Stale-lock unification (120s threshold)** — единый порог во всех 3 callsite: `mock-exam-grade::handleGrade` CAS, `handleRetryPart1OCR`, `handleRegradePart2`. При изменении одного — обновить ВСЕ ТРИ. Иначе появится pattern asymmetry («retry работает, regrade — нет»).
- **`MockExamGradingProgressBanner` derive-only** — компонент НЕ требует backend `grading_progress` колонки. Все signals derive'ятся из existing `attempt.{submitted_at, ai_part1_ocr_json, part2_solutions[].ai_draft}`. При расширении step-level visibility — добавлять новые derived states, не новые DB поля.
- **`useMockExamAttempt::refetchInterval=5000`** активен ТОЛЬКО когда status ∈ {submitted, ai_checking}. Не bleed'ит на approved/manually_entered — было бы runaway polling cost.
- **Localized error messages** — все user-facing toast errors в retry-part1-ocr / regrade-part2 / `ai_checking` paths на русском с actionable advice. Internal `console.warn/error` JSON остаются английскими (логи для разработчика).

**Round 3 hard invariants (auto-zero approve + correct_answer + false-banner fix, 2026-05-21):**

- **Approve never blocks** — `handleApproveAll` БОЛЬШЕ НЕ возвращает 400 INCOMPLETE_PART1/INCOMPLETE_PART2. Для kim где (tutor_score=null AND ai_suggested_score=null) — backend auto-fills `tutor_score=0` + `status='tutor_modified'` + transparent `tutor_comment` чтобы student видел причину. При расширении: новые approval gates НЕ добавлять без отдельного UX decision (force review = anti-pattern, замедляет tutor).
- **`auto_zeroed_*_kims` response arrays** — `handleApproveAll` возвращает `auto_zeroed_part1_kims` + `auto_zeroed_part2_kims` для transparency UI toast. При расширении approve flow — сохранять этот контракт response.
- **Part 2 photo-loaded detection** (Round 3 false-banner fix) — banner «Фото не загружено» показывается ТОЛЬКО когда `!hasIndividualPhoto && !hasBulkAssignment && !aiActuallyGraded`. Для bulk attempts с `ai_draft.assigned_photo_indices.length > 0` — info-chip «AI взял фото №X из пакета», БЕЗ alarm.
- **Reference solution collapsible** — `Part2TaskCard` рендерит `<details>` с `solution.solution_text` через MathText. Default свёрнуто (anti-spoiler от случайного просмотра + меньше viewport).
- **Round 3.1 — нет дублирующих low-conf badges** — состояние «AI не уверен в распознавании» выражается ТОЛЬКО через amber-рамку клетки + tooltip. Никаких inline «⚠ AI?» текстовых бейджей.

**Round 4 hard invariants (Часть 1 reveal для blank-mode student, 2026-05-21):**

- **`runPart1OCR` пишет total_part1_score** — после upsert per-KIM rows, SUM ВСЕХ existing earned_score (tutor + ocr + finalize_default) → UPDATE `attempt.total_part1_score`. Result page показывает «X/28» сразу post-OCR, без tutor finalize. Race-safe: tutor manual scores уже в DB, sum включает.
- **Student result polling 5s** — `useStudentMockExamResult::refetchInterval=5000` когда status ∈ {submitted, ai_checking}. Mirror tutor hook. Stop polling при awaiting_review/approved/manually_entered (terminal).
- **3 banner states для Часть 1 reveal:**
  - **Pending** (`isPreApproval && !hasPart1Data`): `Part1WaitingForOCRBanner` — spinner + «Давай проверим твой бланк». Hide Part1Card.
  - **Preliminary** (`isPreApproval && hasPart1Data`): `Part1PreliminaryBanner` + Part1Card с chip «Предварительно» (sky) или «AI оценил» (sky). Tutor может позже скорректировать.
  - **Approved**: no banner, Part1Card с chip «Готово» (emerald).
- **Anti-leak preserved** — Часть 2 НИКОГДА не показывается до status=approved (unchanged). `ai_part1_ocr_json` НИКОГДА не leak'ается ученику (он видит только `earned_score` per KIM, existing contract).
- **Update notification** — existing `notifyStudentApproved` push cascade fires при final approve-all (без изменений). Ученик получает push → opens result → видит финал.
- **При добавлении нового status** в state machine — обновить banner branching logic (matrix pre-approval vs terminal). `hasPart1Data` derive остаётся consistent (просто проверяет `part1_answers.length > 0 || total_part1_score !== null`).

**Validation:**
- `npm run build` ✅ Round 1/2/3/4 все зелёные.
- `npm run smoke-check` ✅ Section 7 humanities mirror still passes.
- Manual smoke (Vladimir, after deploy):
  1. Pilot student submits blank-mode attempt → wait ≤ 60s → tutor opens review → AI Часть 2 cards + Часть 1 OCR cells pre-filled.
  2. Student submits с `part1_fallback` photo (тетрадный лист) → wait ≤ 60s → tutor видит freeform-OCR результат в Part1BlankReviewPanel.
  3. Tutor clicks «Перезапустить AI OCR» на stuck `ai_checking` attempt (Egor's) → endpoint принимает, grader stale-lock reclaims.
  4. Stuck attempt → top banner «AI работает» с countdown + auto-refresh каждые 5s.
  5. Tutor click «Согласен с AI — отправить» при наличии 5 not-graded kims → backend auto-zeroes → student видит result с transparent comments.
  6. Blank-mode student после submit видит spinner banner «Давай проверим твой бланк», через 30-60 сек → sky banner «Предварительный результат Часть 1» + Part1Card с chip «Предварительно». После tutor approve → push → open result → chip «Готово» emerald.

**Файлы (Round 1-4):**
- `supabase/functions/mock-exam-student-api/index.ts` — submit → 'submitted' (Round 1)
- `supabase/functions/_shared/mock-exam-part1-ocr.ts` — `buildPart1FreeformOCRPrompt` (Round 1)
- `supabase/functions/mock-exam-grade/index.ts` — promptMode + ocrPath + 8MB cap + __meta + total_part1_score auto-update (Round 1+4)
- `supabase/functions/mock-exam-tutor-api/index.ts` — retry accepts both paths + stale-lock unblock retry+regrade + auto-zero approve + RU errors (Round 1+2+3)
- `src/types/mockExam.ts` — `prompt_mode?` additive (Round 1)
- `src/components/tutor/mock-exams/MockExamGradingProgressBanner.tsx` — NEW (Round 2+3 ready-CTA)
- `src/hooks/useMockExamAttempt.ts` — refetchInterval 5s on in-flight (Round 2)
- `src/hooks/useStudentMockExamResult.ts` — refetchInterval 5s on in-flight (Round 4)
- `src/pages/tutor/mock-exams/TutorMockExamReview.tsx` — banner mount + retryAllMutation + Part1 status icons + correct_answer row + Part2 false-banner fix + reference solution collapsible + always-enabled approve (Round 2+3)
- `src/pages/student/StudentMockExamResult.tsx` — Part1WaitingForOCRBanner + Part1PreliminaryBanner + Part1Card status prop (Round 4)
- `docs/delivery/features/mock-exams-v1-pilot-polish/ocr-grading-recovery-spec.md` — spec with Round 1-4 sections

**Спека:** `docs/delivery/features/mock-exams-v1-pilot-polish/ocr-grading-recovery-spec.md`.

### 31. max_score шаг 0.5 + invite preview rework (2026-05-23, 1-serene-finch)

Два независимых pilot UX issue, fixed в одной сессии:

**Task 1 — `max_score` шаг 0.5 в конструкторе ДЗ.** Репетитор не мог поставить 12.5 («Макс. баллов» × задача): input `type="number"` без `step`, `parseInt` режет дробную часть, backend `isPositiveInt(max_score)` отклоняет non-integer, колонка `homework_tutor_tasks.max_score` — `INT NOT NULL`. ФИПИ ЕГЭ физика № 21-26 содержит half-step max scores. Симптом: округление искажает балльность задач.

**Task 2 — Telegram link preview для invite-ссылки пугает учеников.** Глобальный OG в `index.html` (text для tutor sales landing «Сократ AI для репетиторов... 7 дней бесплатно... Потом 200 ₽») подтягивался Telegram'ом при вставке `https://sokratai.ru/invite/AR7LWHTZ`. Ученики писали «как это, я платить должен?».

**Files (Task 1):**
- Migration `20260523120000_homework_tutor_tasks_max_score_halfstep.sql` — `max_score INT → numeric(6,1)`, DEFAULT 1 сохранён, transparent для existing rows.
- `supabase/functions/homework-api/index.ts` — новый `isPositiveHalfStepNumber()` helper + замена в 6 callsite (2 validators + 4 insert/update defaults). `order_num` остаётся `isPositiveInt` (intentional — порядок задач всегда integer).
- `src/components/tutor/homework-create/HWTaskCard.tsx` — input `step={0.5}` + `min={0.5}` + `inputMode="decimal"`, локальный `scoreText` string state + sync через `useEffect([task.max_score])`, snap-on-blur (12.7 → 12.5 без error modal), hint «Шаг 0.5 — например 1, 1.5, 12, 12.5».

**Files (Task 2):**
- NEW `supabase/functions/invite-preview/index.ts` (~155 lines) — public edge function (verify_jwt=false), service_role lookup `tutors.name` по `invite_code`, escapeHtml XSS guard, generic fallback для invalid code (не 404 — чтобы preview не выглядел broken в chat), meta-refresh + `window.location.replace` для browser → `/invite/{code}` SPA, telemetry server-side PII-free.
- `src/utils/telegramLinks.ts` — новый `getTutorInvitePreviewLink(code)` отдаёт `https://api.sokratai.ru/functions/v1/invite-preview?c={code}`. Legacy `getTutorInviteWebLink` сохранён в JSDoc как «только для внутренней навигации».
- Share UI source-of-truth (4 файла) теперь компьютит `inviteWebLink` через preview helper: `TutorStudents.tsx`, `TutorHome.tsx`, `TutorHomeworkCreate.tsx`, `mock-exams/AddStudentsToMockExamDialog.tsx`. Downstream (`AddStudentDialog`, `HWAssignSection`, `HWSubmitSuccess`) получают URL через props, не вычисляют.
- `supabase/config.toml` + `.github/workflows/deploy-supabase-functions.yml` — invite-preview зарегистрирована с `verify_jwt = false`.
- `public/sokrat-logo.png` уже существовал (36KB hero version) — используется как og:image.

**Preview text (что увидит ученик в Telegram чате):**
...
- fallback без имени: «Твой репетитор подключил тебя к Сократ AI...»

**Hard invariants (более подробно в `.claude/rules/40-homework-system.md` → секции «Score step invariants» + «Invite preview edge function»):**

1. **Шаг 0.5 для `max_score`** vs **шаг 0.1 для `tutor_score_override` / `ai_score`** — два разных поля, разные правила. НЕ путать. `EditScoreDialog.tsx` использует step 0.1 (CLAUDE.md §40 «Score step invariant 0.1»).
2. **Все 6 callsite `max_score` в `homework-api/index.ts`** должны использовать `isPositiveHalfStepNumber` — не `isPositiveInt`. Грепни `isPositive(Int|HalfStepNumber)\((t|task)\.max_score` перед merge — все совпадения должны быть `HalfStepNumber`.
3. **Двойной write-path в `homework_tutor_tasks`** (CLAUDE.md §0) применим: HWDrawer.tsx **не пишет** `max_score` явно — полагается на DB DEFAULT 1 (numeric 1.0 после миграции). При future change `max_score` — проверять обе пути.
4. **Share-сценарии всегда через `getTutorInvitePreviewLink`**, не `getTutorInviteWebLink`. Legacy `*.WebLink` оставлен только для internal navigation.
5. **Edge function `invite-preview` отдаёт HTML, не JSON**, и `escapeHtml` обязателен для tutor.name. Содержит pattern для будущих preview-style edge functions (e.g. mock-exam preview, kb-card preview).
6. **Backward compat:** legacy `sokratai.ru/invite/{code}` URLs, уже отправленные репетиторами раньше (до этого fix), продолжают работать — claim flow на React странице intact. Просто preview-card в этих legacy сообщениях показывает старый текст из `index.html`.

**При расширении на новый subject / новый share-channel** (WhatsApp / Discord — все используют OG): сам edge function preview работает универсально. При добавлении других public preview-функций (mock-exam invite-preview etc.) — следуй той же архитектуре (escapeHtml + service_role + generic fallback + meta-refresh + PII-free telemetry).

**Plan / spec:** `~/.claude/plans/1-1-serene-finch.md`.

### 32. Quick-add students в ДЗ — параллель с пробниками (2026-05-25)

Закрывает pilot pain point: репетитор создал ДЗ на 3 учеников, через день ещё одному нужно то же задание. Старый путь — Edit → scroll до `HWAssignSection` → отметить → scroll через 15 задач → Save (~10 кликов). Новый — Detail → «+ Добавить учеников» → отметить → «Добавить» (~3 клика). Mirror UX пробников (TASK-17 mock-exams, §29).

**Files (4 файла, ~610 строк):**

| File | Change |
|---|---|
| `supabase/functions/homework-api/index.ts` | + `notifyHomeworkStudentAssigned` helper + `handleQuickAssignStudentsWithNotify` handler + route `POST /assignments/:id/assign-students` |
| `src/lib/tutorHomeworkApi.ts` | + `quickAssignHomeworkStudents` API function + `QuickAssignHomeworkStudentsResponse` type |
| `src/components/tutor/results/AddStudentsToHomeworkDialog.tsx` | NEW — reuse `HWAssignSection` с `existingStudentIds` (locked) + `hideNotify`, inline notify checkbox default ON |
| `src/pages/tutor/TutorHomeworkDetail.tsx` | + `UserPlus` icon, + lazy import dialog, + `addStudentsOpen` state, + кнопка в `DetailActions` между «Редактировать» и «⋮», + conditional mount |

**Hard invariants:**

- **Два endpoint'а коэкзистируют** (см. `.claude/rules/40-homework-system.md` → «Два endpoint'а для assign students»): legacy `/assignments/:id/assign` (edit-flow, без notify, возвращает `students_without_telegram_names`) + new `/assignments/:id/assign-students` (quick-add с cascade notify, возвращает counters). НЕ консолидировать — у них разная UX-семантика и риск регрессии edit-flow контракта.
- **Cascade notify scope = push + telegram, БЕЗ email** (Vladimir UX choice, mirror моков). Bulk notify (`/notify` → `handleNotifyStudents`) с email остаётся для re-engagement через `RemindStudentDialog`. Если когда-то понадобится email для quick-add — расширь `notifyHomeworkStudentAssigned` третьей веткой (нужен homework-specific email template).
- **Status gates ОТСУТСТВУЮТ** (mirror моков): работает на `draft` / `active` / `closed` без warning'ов. Vladimir explicit «разрешать всегда, репетитор сам решает».
- **Идемпотентно**: уже-assigned студенты silently skip через `ignoreDuplicates: true` на UNIQUE `(assignment_id, student_id)`. Counter `skipped_existing` для UI toast «пропущено N уже назначены».
- **HWAssignSection reuse** через existing `existingStudentIds?: Set<string>` + `hideNotify?: boolean` props (additive, backward-compat). При изменении HWAssignSection — проверять что и моки (`AddStudentsToMockExamDialog`), и ДЗ (`AddStudentsToHomeworkDialog`) flow остаются OK.
- **Cache invalidation после success**: `['tutor','homework','detail', id]` + `['tutor','homework','results', id]` + `['tutor','homework','assignments']` — Heatmap сразу подхватывает нового ученика со статусом «Не приступал».
- **`provisionGuidedThread` eager bootstrap** для каждого newly-inserted student_assignment row — mirror existing `handleAssignStudents` поведения. Без этого первый заход ученика в ДЗ упирается в lazy-provisioning latency.

**Сравнение scope с пробниками TASK-17:**

| Action | Mock-exams (§29) | Homework (§32) |
|---|---|---|
| Add students в шапке Detail | ✅ TASK-17 | ✅ §32 (this) |
| Remove individual student (per-row ✕) | ✅ TASK-17 | ❌ deferred (через Edit modal) |
| Delete entire assignment | ✅ TASK-17 (+ ⋮ на list-card) | ✅ existing (в шапке Detail) |
| Cascade notify scope | push + telegram | push + telegram |

Scope homework намеренно уже моков — Vladimir UX выбор «только Add» для focused PR. Remove individual + ⋮ delete на list-card можно добавить P2, если pilot покажет необходимость.

**При расширении (Phase 2 ideas, NOT implemented):**
- Per-row ✕ в HeatmapGrid (mirror моков AddStudentsToMockExamDialog → RemoveStudentFromMockExamDialog)
- Bulk impersonate / preview перед add
- Email leg в cascade notify
- «Похожий пробник уже есть» warning при detect overlap students+tasks

### 33. Tutor polish Phase 9 — save-as-template / invite share / drag-drop (2026-05-25)

Закрывает 3 независимых UX issue от репетитора-француза после Phase 8 deploy. Все три ломают workflow «составил ДЗ → сохранил шаблон → отправил ученикам». В одном PR (Vladimir UX choice) с deploy одним проходом.

**Issue 1 — Save-as-template fails (schema drift bug):**

Репетитор не мог сохранить шаблон DELF B1 ДЗ → toast «Не удалось сохранить шаблон». Root cause: `homework_tutor_templates.subject` CHECK constraint (миграция `20260226100000_homework_20.sql:40`) принимал только legacy subjects (`math, physics, history, social, english, cs`). Миграция `20260414150000_unify_homework_subject_check.sql` (CLAUDE.md §7) применила unified CHECK только к `homework_tutor_assignments`, для templates аналогичной миграции не было.

**Files:**
- `supabase/migrations/20260525120000_unify_homework_templates_subject_check.sql` (NEW) — DROP + CREATE CHECK constraint на `homework_tutor_templates.subject` с 19 canonical + legacy subjects. Mirror `20260414150000`. Idempotent (`DROP CONSTRAINT IF EXISTS`).
- `supabase/functions/homework-api/index.ts::handleCreateTemplateFromAssignment` — добавлен pre-validate guard на `assignment.subject` против `VALID_SUBJECTS_CREATE` (return `400 INVALID_SUBJECT` с russian message). Catch Postgres code `23514` (CHECK violation) → `409 CHECK_VIOLATION` (defense-in-depth — если CHECK constraint снова разойдётся в будущем).
- `src/components/tutor/homework-reuse/SaveAsTemplateDialog.tsx` — catch `HomeworkApiError` с specific codes (`INVALID_SUBJECT` / `CHECK_VIOLATION` / `UNAUTHORIZED`) → russian toast с явным reason вместо generic «Не удалось сохранить шаблон».

**При добавлении нового subject в `SUBJECTS` (`src/types/homework.ts`):** обновлять **ОБА** CHECK constraints одновременно — `homework_tutor_assignments_subject_check` И `homework_tutor_templates_subject_check`. Audit script для всех CHECK constraints (включая `mock_exam_variants.subject` когда колонка будет добавлена, см. §20 tech debt) — отложен в отдельный PR.

**Issue 2 — Invite share URL → canonical claim link:**

Репетитор копировала «ссылку приглашения» и при тесте в браузере видела mojibake-page (raw HTML с broken encoding). Root cause: 5 share callsites копировали URL на edge function `invite-preview` (`api.sokratai.ru/functions/v1/invite-preview?c={code}`) — этот URL предназначен **для Telegram OG scraper**, не для прямого визита пользователя. Когда репетитор открывала его в браузере, либо успешно редиректило на canonical sokratai.ru/invite/{code} (meta-refresh + JS), либо показывало raw HTML.

**Files:**
- `src/utils/telegramLinks.ts` — JSDoc deprecate `getTutorInvitePreviewLink` (НЕ удалён — backward compat с Telegram link preview-картинками в существующих чатах). `getTutorInviteWebLink` JSDoc обновлён как primary share path.
- 5 callsites (вместо 4 из плана — после Phase 32 quick-add появился ещё один): `TutorStudents.tsx` / `TutorHome.tsx` / `TutorHomeworkCreate.tsx` / `AddStudentsToMockExamDialog.tsx` / `AddStudentsToHomeworkDialog.tsx` — заменили `getTutorInvitePreviewLink(code)` на `getTutorInviteWebLink(code)`.

**Trade-off задокументирован в JSDoc:** Telegram preview-card теперь generic (использует global `index.html` OG из landing AI-crawlers refactor 2026-05-19 — sokrat-logo + canonical title), без имени репетитора в description. Реliability важнее в Phase 7+ pilot.

**Hard rule:** **ВСЕ** новые share-surfaces ОБЯЗАНЫ использовать `getTutorInviteWebLink`, не `getTutorInvitePreviewLink`. Grep перед merge: `grep -rn "getTutorInvitePreviewLink" src/` должен показывать только JSDoc + export в `telegramLinks.ts`.

**Issue 3 — Drag-and-drop universal в 4 surfaces:**

Репетитор пожаловалась что drag-drop фото в критерии не работает (только Ctrl+V paste). Исследование показало uniform gap: **НИ ОДНА** из 4 секций (task / solution / rubric / materials) не имела drag-drop. Pattern из KB (`useImageUpload` hook) extracted в новый shared hook.

**Files:**
- `src/hooks/useDragDropFiles.ts` (NEW, ~245 строк) — generic shared hook. Mirror `usePasteImages` signature: `enabled / maxFiles / currentCount / acceptedTypes / compress / onFilesDropped / successToast / telemetryTag`. Returns `{ dragHandlers: {onDragEnter, onDragLeave, onDragOver, onDrop}, isDragging }`.
- `src/components/tutor/homework-create/HWTaskCard.tsx` — 3 секции (task / solution / rubric) каждая обёрнута в `<div {...dragHandlers}>` с `relative` + conditional `<DropOverlay />`. Per-section hooks (НЕ card-level) — drop landing должен быть unambiguous. Compression `false` — уже происходит в `addTaskPhotos / addSolutionPhotos / addRubricPhotos`.
- `src/components/tutor/homework-create/HWMaterialsSection.tsx` — wrapper drag-drop принимает images + PDFs (matching paste's `acceptedTypes`). Extracted `appendFileMaterial` helper переиспользуется paste и drag-drop pathways. Compression `true` (mirror paste).
- `PasteHint` обновлён: «Перетащи фото или вставь скриншот: Ctrl+V» — universal hint работает для обоих путей.

**Key design decisions (mirror §26 Phase 5+ Ctrl+V):**

1. **Drag counter `dragCounterRef`** защищает от flicker overlay на nested drag events.
2. **`preventDefault()` обязателен** на `onDragOver` — иначе browser default handler перехватит drop и navigate to file URL.
3. **Multi-file** — drag-drop часто содержит несколько файлов (в отличие от paste, который один файл из clipboard). Hook trim'ит до `maxFiles - currentCount`, остальные silent + toast warning.
4. **Per-section dropzone в HWTaskCard, не card-level.** Paste идёт через `lastFocusedSection` ref (fallback на 'task'), drop landing — буквальное «куда упало». Иначе drag в rubric секцию случайно попал бы в task через ref state.
5. **`pointer-events-none` на overlay** обязателен — иначе overlay перехватывает drop и hook не fire'ит.
6. **Visual feedback consistent с KB `ImageUploadField`**: `ring-2 ring-dashed ring-accent` на section + centered overlay «Отпустите для добавления».

**При добавлении нового surface где tutor загружает фото:**

1. Импортируй `useDragDropFiles` from `@/hooks/useDragDropFiles` + (опционально) `usePasteImages` для Ctrl+V parity.
2. Spread `{...dragHandlers}` на container с `position: relative`.
3. Render conditional overlay с `pointer-events-none` когда `isDragging`.
4. Lock check через `enabled: !isUploading` + `currentCount: refs.length` + `maxFiles: MAX_X_IMAGES`.
5. Compression: `true` для screenshots (default 4MB / 2048px), `false` если caller имеет свой pipeline (mirror Phase 5+ Ctrl+V decisions).
6. Update inline kbd hint мention обоих путей: «Перетащи или вставь скриншот: Ctrl+V».

**Backward compat:**
- `useDragDropFiles` — additive новый файл, не тронул `useImageUpload` (KB-side). KB продолжает работать как раньше.
- `getTutorInvitePreviewLink` остаётся exported для legacy consumers. Endpoint `invite-preview` не удалён — backward compat с Telegram cache.
- Migration `20260525120000` — idempotent (`DROP CONSTRAINT IF EXISTS`), re-apply safe.
- Pre-validate guard в `handleCreateTemplateFromAssignment` — additive defense-in-depth, не меняет happy path для legacy subjects.

**Out of scope (deferred):**
- Audit script для всех subject CHECK constraints (assignments / templates / mock_exam_variants когда колонка появится).
- Drag-drop в SubmitSheet ученика (Vladimir не выбрал scope в AskUserQuestion).
- Telegram OG preview-card с именем репетитора — отложено в пользу reliability.
- Better error UX (`extractEdgeFunctionError` pattern) для остальных edge function callsites.

**Спека:** `~/.claude/plans/1-functional-meteor.md` Phase 9 раздел. Files (11 файлов + 1 migration, ~250 LOC).

### 34. Homework constructor state preservation — Phase 10 hotfix (2026-05-26)

Production-критический hotfix. Репетитор Elena Ivanova сообщила что добавленные в ДЗ задачи **исчезают** при переключении вкладок браузера и обратно: «удаляет добавленные задачи (даже после кнопки сохранить изменения). Оставляет только 3 штуки, которые в самом начале взяты». Bug блокирует ключевой функционал. Связан с архитектурным паттерном `useQuery` + edit-mode prefill effect — этот класс ошибок исправлялся **5 раз** в апреле 2026 (commits `3660ee3` / `0cfda51` / `baf6638` / `387e0f4` / `270ecd4`).

**Root cause (TutorHomeworkCreate.tsx:389-394 до Phase 10):**
```ts
const editQuery = useQuery({
  ...
  enabled: isEditMode,
  staleTime: 30_000,
  // ❌ default refetchOnWindowFocus: true (React Query default)
});
```

Default React Query поведение — **`refetchOnWindowFocus: true`** — триггерит refetch для каждой active query при возврате на вкладку. Через 30+ секунд (past staleTime) refetch фактически летел на сервер. `existingAssignment` получал новую object reference → `useEffect [isEditMode, existingAssignment]` (line 497) re-fire'ил → race condition с `editPrefilledRef` guard → setTasks с server data → local user-added tasks потеряны.
...
const editQuery = useQuery({
  ...
  queryFn: () => getTutorHomeworkAssignment(editId!),
  enabled: isEditMode,
  staleTime: 10 * 60 * 1000,   // 10 минут — длительность typical edit session
  refetchOnWindowFocus: false, // отключаем focus refetch для write-form
});
```

**Hard invariants для всех новых write-form queries (форма редактирования server data):**

1. **`refetchOnWindowFocus: false` ОБЯЗАТЕЛЬНО.** Любая `useQuery` которая читает initial state, который потом редактирует пользователь в локальной форме — **никогда** не должна auto-refetch'иться. Pattern: pages под `/edit` или `:id/edit`, любые wizard'ы с server-fed initial values. Read-only display lists (`useTutorHomework`, `useTutorGroups`, `useTutorReviewQueue` — все они `refetchOnWindowFocus: true`) — **другая категория**, там focus refetch полезен.
2. **`staleTime: 10 * 60 * 1000`** (10 минут) — типичная длительность edit session. Если tutor сидит дольше — explicit refresh страницы возьмёт свежие данные.
3. **`editPrefilledRef` pattern сохраняется** (existing — see TutorHomeworkCreate.tsx:473-499). Phase 10 fix через **upstream** (отключение refetch), не downstream (изменение guard) — risk изменения guard logic слишком высок.
4. **Reset effect ordering invariant** (CLAUDE.md §16, fix `3660ee3` 2026-04-17) сохраняется — reset effect MUST be declared BEFORE prefill effect для proper init order.

**Smoke check assertion (scripts/smoke-check.mjs section 8):**
- Grep всех `useQuery({ ... })` calls в write-form pages (`src/pages/tutor/TutorHomeworkCreate.tsx` сейчас; добавлять новые в `writeFormPages` array)
- Assert каждый useQuery с `queryKey` имеет `refetchOnWindowFocus: false` в config body
- Exit 1 если не — явное сообщение «Write-form queries MUST disable focus refetch — иначе race condition с prefill effect уничтожит unsaved user edits при tab switch»
- Detect'ит регрессию при future edits constructor

**Manual QA checklist** — обязательный для PR трогающих constructor files. См. `.claude/rules/40-homework-system.md` секция «Homework constructor QA checklist» — 7 точек проверки (tab-switch preserve / save button readiness / KB picker / HWDrawer / drag-drop / subjects / max_score). **Hard rule:** commit message трогающий constructor **ОБЯЗАН** содержать строку `Manual QA: checklist в .claude/rules/40-homework-system.md пройден`.

**Mock-exam parity (Phase 10 investigation note):** TutorMockExamCreate.tsx — wizard для создания, **не** имеет edit mode и не использует `useQuery` с prefill pattern. Phase 10 fix не нужен. Если когда-то добавится Mock-exam edit page — она ОБЯЗАНА следовать те же hard invariants (см. выше).

**Phase 11 deferred (out of scope сейчас):**
- Playwright E2E test для homework constructor — Vladimir выбрал отложить из-за рисков flaky tests (browser focus event не реально симулируется через Playwright single-tab API), setup overhead (~3-4ч), maintenance cost. Phase 10 покрывает через smoke check + manual QA checklist + ChatGPT-5.5 review pattern.

**Files (Phase 10 hotfix):**
- `src/pages/tutor/TutorHomeworkCreate.tsx` — editQuery config (~25 строк включая comment)
- `scripts/smoke-check.mjs` — section 8 (~75 строк)
- `.claude/rules/40-homework-system.md` — QA checklist extension (~65 строк)
- `CLAUDE.md` — §34 documentation (this section)
- **NO** DB migrations, **NO** backend changes — pure client-side hotfix

**При расширении на новые write-form pages:**
1. Add `useQuery` с config `{ refetchOnWindowFocus: false, staleTime: 10 * 60 * 1000 }`. Не пропускать — smoke check fail'нит.
