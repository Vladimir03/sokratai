...
- Push: проверяет `push_subscriptions`, retry 1x при 5xx, удаляет subscription при 410 Gone
- Telegram: retry 2x при 429/5xx с 500ms delay (сохранено из legacy)
- Email: через `sendHomeworkNotificationEmail` / `sendHomeworkReminderEmail`, `@temp.sokratai.ru` исключаются
- Нет каналов → `failed_no_channel`; все каналы failed → `failed_all_channels`
