...
3. Stuck attempts recovery: всегда добавляй stale-lock path (mirror retry-part1-ocr 120s logic), не блокируй tutor вечно по `ai_checking`.
...
**Файлы:**
- `supabase/functions/mock-exam-student-api/index.ts` — submit → 'submitted'
- `supabase/functions/_shared/mock-exam-part1-ocr.ts` — `buildPart1FreeformOCRPrompt`
- `supabase/functions/mock-exam-grade/index.ts` — promptMode + ocrPath + 8MB cap + __meta enrich
- `supabase/functions/mock-exam-tutor-api/index.ts` — retry accepts both paths + stale-lock unblock
- `src/types/mockExam.ts` — `prompt_mode?` additive
- `docs/delivery/features/mock-exams-v1-pilot-polish/ocr-grading-recovery-spec.md` — spec
