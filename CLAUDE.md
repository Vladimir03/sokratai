...
- `RenameFolderModal.tsx` — модалка с pre-filled input, паттерн от `CreateFolderModal`
- `DeleteFolderDialog.tsx` — подтверждение с рекурсивными счётчиками (подпапки + задачи). Счётчики загружаются через `countFolderDescendants()` при открытии

**Хуки:**
- `useRenameFolder()` — мутация, invalidates `['tutor', 'kb']`
- `useDeleteFolder()` — рекурсивно удаляет задачи во ВСЕХ дочерних папках перед удалением корневой (обходит CHECK constraint `kb_tasks_space_check`)
- `countFolderDescendants(folderId)` — экспортированная функция, возвращает `{ subfolderCount, taskCount }` для всего дерева

**Удаление текущей папки:** → навигация к parent breadcrumb или `/tutor/knowledge?tab=mybase`

**Known tech debt:** удаление папки не чистит blob-ы вложений в `kb-attachments` storage (orphaned objects). Нужен серверный cleanup job
...
- **Запрос каталога (модераторы)**: `fetch_catalog_tasks_all(topic_id)` — возвращает все статусы (active + hidden_duplicate + unpublished) для модераторов, только active для остальных
...
- **Fingerprint dedup**: `kb_normalize_fingerprint(text, answer, attachment_url)` + `pg_advisory_xact_lock`
  - Fingerprint = md5(text + '::' + answer + '::' + attachment_url) — включает вложения, чтобы задачи с условием на картинке не считались дублями
  ...
  - Миграция `20260330110000_fingerprint_include_attachment.sql` — добавлен `attachment_url` в fingerprint + ретроактивное раскрытие ложных дублей
...
- **Проблема**: задачи с `moderation_status = 'hidden_duplicate'` были невидимы даже модераторам в каталоге (fingerprint dedup молча скрывала дубли)
- **Решение**: новый RPC `fetch_catalog_tasks_all(topic_id)` — модераторы видят ВСЕ статусы, обычные пользователи — только `active`
- `useCatalogTasksAll(topicId, enabled)` хук в `useKnowledgeBase.ts` — query key `['tutor', 'kb', 'catalog-tasks-all', topicId]`
- `CatalogTopicPage.tsx` — если `isModerator`, использует `useCatalogTasksAll`; иначе — прежний `useCatalogTasks`
- Миграция: `20260330100000_fetch_catalog_tasks_moderator.sql`
