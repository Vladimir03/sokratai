...
## Голосовые сообщения в веб-чате ученика

- Веб-чат больше не использует browser `SpeechRecognition`: голос записывается через `MediaRecorder` в `src/hooks/useVoiceRecorder.ts`.
- Flow: ученик нажимает mic → `ChatInput.tsx` запускает запись → повторное нажатие останавливает запись и отправляет blob в `POST /functions/v1/chat/transcribe-voice` через `src/lib/chatVoice.ts` → `supabase/functions/chat/index.ts` расшифровывает аудио через Lemonfox → текст попадает обратно в composer → ученик проверяет/редактирует и только потом нажимает отправить.
- Во время записи доступна явная отмена; также есть авто-стоп через 120 секунд, чтобы не упираться в backend limit по размеру файла.
- Отправленное после расшифровки сообщение должно идти с `input_method: 'voice'`, чтобы UI помечал его как голосовое и backend не инкрементил usage второй раз.
- Edge route `/transcribe-voice` требует JWT, принимает только allowlist MIME (`audio/webm`, `audio/mp4`, `audio/ogg`, `audio/mpeg`, `audio/mp3`, `audio/wav`, `audio/x-wav`), ограничивает размер до 10 MB и шлёт в Lemonfox явный `model: 'whisper-large-v3'` + `language: 'ru'`.
- Usage semantics: `/transcribe-voice` списывает лимит один раз для free users; следующий `/chat` request должен пропустить второй инкремент только для последнего user message с `input_method: 'voice'`.
- Для Safari/iOS используем MIME fallback `audio/mp4`; не возвращать старый `useVoiceInput` / browser speech recognition без отдельного продуктового решения.
- Secret: `LEMONFOX_API_KEY`.
- Основные файлы: `src/components/ChatInput.tsx`, `src/hooks/useVoiceRecorder.ts`, `src/lib/chatVoice.ts`, `supabase/functions/chat/index.ts`.

## Голосовые сообщения в guided homework

- В `src/components/homework/GuidedChatInput.tsx` тот же draft-first voice flow работает отдельно для полей «Ответ к задаче» и «Обсуждение».
- Во время записи доступна явная отмена; после 120 секунд срабатывает авто-стоп, чтобы не упираться в backend limit по размеру файла.
- Внутри guided homework используется один recorder на оба поля, поэтому нельзя параллельно записывать голосовое для ответа и обсуждения.
- Homework voice НЕ должен ходить через `/functions/v1/chat/transcribe-voice`, иначе смешаются student chat limits и guided-homework flow.
- Канонический route для домашек: `POST /functions/v1/homework-api/threads/:id/transcribe-voice`, клиентский helper живёт в `src/lib/studentHomeworkApi.ts`.
- Route обязан проверять ownership thread через `verifyThreadOwnership`, принимать тот же allowlist MIME, ограничение 10 MB и отправлять в Lemonfox явный `model: 'whisper-large-v3'` + `language: 'ru'`.
- После расшифровки transcript попадает обратно в соответствующий draft textarea и уходит только после явного `Проверить` / `Написать`.
...
- `yookassa-webhook` — **нельзя добавлять JWT**
- Не путать с `tutor_payments` (оплата урока) — это разные сущности
