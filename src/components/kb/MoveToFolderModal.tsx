...
import { useFolderTree, useMoveTaskToFolder } from '@/hooks/useFolders';
...
export function MoveToFolderModal({ task, currentFolderId, onClose }: MoveToFolderModalProps) {
  const { tree, loading, error, refetch } = useFolderTree();
  const moveMutation = useMoveTaskToFolder();
  ...
  const handleMove = () => {
    ...
    moveMutation.mutate(
      { taskId: task.id, targetFolderId: selectedId, sourceFolderId: currentFolderId },
      {
        onSuccess: () => {
          const folderName = findFolderName(tree, selectedId);
          toast.success(`Перемещено в папку${folderName ? ` «${folderName}»` : ''}`);
          onClose();
        },
        onError: () => {
      ...
      <div className="fixed left-1/2 top-1/2 z-[301] flex max-h-[70vh] w-[380px] max-w-[90vw] -translate-x-1/2 -translate-y-1/2 flex-col rounded-2xl bg-white shadow-xl animate-in fade-in-0 zoom-in-95">
        <div className="flex items-start justify-between border-b border-socrat-border px-5 py-4">
          <div className="min-w-0 flex-1">
            <h3 className="text-base font-semibold">Переместить в папку</h3>
        ...
        <div className="flex-1 overflow-auto px-1.5 py-2">
          ...
          {!loading && !error && tree.length > 0 && renderFolders(tree, 0, selectedId, setSelectedId, currentFolderId)}
        </div>
        ...
        <div className="flex justify-end gap-2 border-t border-socrat-border px-5 py-3.5">
          ...
          <button
            ...
            className={cn(
              ...
              selectedId && !moveMutation.isPending
                ? 'bg-socrat-primary'
  ...
  return nodes.map(node => (
    <div key={node.id}>
      ...
      >
        ...
        />
        <span>{node.name}</span>
        {node.id === excludeId && <span className="ml-auto text-[11px] text-muted-foreground/50">текущая</span>}
      </button>
      {node.children.length > 0 && renderFolders(node.children, depth + 1, selectedId, setSelectedId, excludeId)}
    </div>
  ));
}
