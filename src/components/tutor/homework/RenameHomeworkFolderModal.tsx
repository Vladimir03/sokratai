// Переименование папки ДЗ. Клон KB RenameFolderModal на homework-хуки. Запрос Елены (2026-06-17).
import { useEffect, useRef, useState } from 'react';
import { X } from 'lucide-react';
import { toast } from 'sonner';
import { useRenameHomeworkFolder } from '@/hooks/useHomeworkFolders';
import { cn } from '@/lib/utils';

interface RenameHomeworkFolderModalProps {
  folderId: string;
  currentName: string;
  onClose: () => void;
}

export function RenameHomeworkFolderModal({ folderId, currentName, onClose }: RenameHomeworkFolderModalProps) {
  const renameFolder = useRenameHomeworkFolder();
        ...
        <div className="px-5 py-4">
          <fieldset>
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter' && canSave && !renameFolder.isPending) handleSave();
              }}
        ...
        <div className="flex justify-end gap-2 border-t border-socrat-border px-5 py-3.5">
          ...
          <button
            ...
            className={cn(
              ...
              canSave && !renameFolder.isPending ? 'bg-socrat-primary' : 'cursor-default bg-socrat-border',
            )}
          >
