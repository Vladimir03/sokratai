...
export function AddStudentDialog({
  ...
  const finalizeAfterAdd = (
    ...
    // №43/Егор (2026-07-20): показываем логин+пароль ВСЕГДА, когда бэкенд их вернул
    // (в т.ч. для плейсхолдера по имени — логин теперь короткий s{7}@temp.sokratai.ru,
    // вбиваемый). «Вот логин/пароль, всё работает — хочешь меняй». Код для входа
    // тоже доступен (кнопка «Подключить» на карточке). Раньше плейсхолдер прятал
    // модалку (UUID-логин был бесполезен).
