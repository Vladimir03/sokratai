...
import type { AnswerRecord, RoundResult, WeakFormula } from '@/lib/formulaEngine/types';
...
export async function getFormulaRound(roundId: string): Promise<FormulaRound> {
  return requestFormulaRoundApi<FormulaRound>(
    `/formula-rounds/${encodeURIComponent(roundId)}`,
  );
}

export async function getFormulaRoundResults(
  roundId: string,
