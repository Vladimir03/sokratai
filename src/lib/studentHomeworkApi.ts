...
export async function getStudentAssignment(assignmentId: string): Promise<StudentHomeworkAssignmentDetails> {
  ...
  //
  // Phase 8 (2026-05-20) — REGRESSION FIX + gender:
  //   Phase 1 mobile screen (commit bffd97c0, 2026-05-09) SELECT'ил только
  //   `profiles.username`, пропуская `profiles.full_name` — нарушение
  //   canonical 3-уровневого fallback chain (CLAUDE.md §8). Если у ученика
  //   нет tutor_students.display_name (репетитор не выставил), но есть
  //   profiles.full_name (real name из signup) — frontend возвращал
  //   только username, и для autogenerated `telegram_123` обращение по
  //   имени просто пропадало.
  //
  //   Round 2 fix: select `full_name, username, gender` для полного
  //   каскада. Mirror backend `resolveStudentIdentity` в `homework-api`.
  //
  ...
  //
  // Best-effort: RLS may deny tutor_students read; we catch gracefully.
  ...
  try {
    ...
    const tsData = tsResult.data as {
      display_name?: string | null;
      gender?: string | null;
    } | null;
    const profData = profResult.data as {
      full_name?: string | null;
      username?: string | null;
      gender?: string | null;
    } | null;

    // Name resolution (3-tier fallback)
    const curated = tsData?.display_name?.trim() ?? '';
    const fullName = profData?.full_name?.trim() ?? '';
    ...
    // Gender resolution (tutor-curated → profile signup → null)
    ...
    // Non-critical — AI will use neutral forms if name/gender unavailable.
  }
