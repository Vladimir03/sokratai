 ...
 *
 * ЗЕРКАЛО Deno-модуля `supabase/functions/_shared/answer-alternatives.ts` —
 * правки вносить в ОБА файла (конвенция репо «mirror locally», как
 * `attachmentRefs` / `scoreScales`). Дрейф ловит
 * `scripts/test-answer-alternatives.mjs` (smoke-check): оба зеркала гоняются
 * по одному набору векторов.
 *
 * Формат хранения — существующее ТЕКСТОВОЕ поле ответа (`kb_tasks.answer` /
 * `homework_tutor_tasks.correct_answer`), ноль миграций:
 ...
 * `x=1; y=2` («нужны оба») не коллидирует, а « или » = «любой из» = альтернативы.
 * Legacy-строка без « или » и range-маркера → один exact (byte-identical старому).
 *
 ...
 *
 * Потребители (frontend): `AnswerAlternativesField` (чипы «+ вариант» /
 * «от–до» в Create/EditTaskModal). Грейдинг живёт на Deno-стороне.
 */
...
/** Сериализация списка сырых частей (UI чипов) обратно в текстовое поле. */
export function serializeAnswerParts(parts: string[]): string {
  return parts.map((p) => p.trim()).filter(Boolean).join(ANSWER_ALT_JOIN);
}
