...
- **`content_revision` + CAS в RPC — обязателен для любого write контента варианта.** Клиент шлёт загруженную ревизию, расхождение → `VARIANT_STALE` (409). Закрывает lost update между вкладками И stale-бандл, который шлёт задачи без `block_id` и молча стирал бы все привязки. Клиент без ревизии отвергается, только если у варианта ЕСТЬ блоки.
