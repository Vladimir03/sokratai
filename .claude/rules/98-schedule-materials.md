...
## Несколько ДЗ на один урок (запрос Елены, 2026-06-17)

Снят жёсткий лимит 1:1 `homework_ref` на занятие → разрешено **N разных** ДЗ, запрещён только дубль одного. Миграция `20260617120100`.

- **DB:** drop `uq_tlm_one_hw_per_lesson` → `uq_tlm_one_hw_pair_per_lesson (lesson_id, homework_assignment_id) WHERE material_kind='homework_ref'` (дубль-гард). `IF EXISTS` покрывает оба определения старого индекса (`20260602140000` + снапшот `20260602160423`). Дата-миграции нет (пары уже уникальны).
- **Edge `lesson-materials-api::handleCreateMaterial`:** убран 1:1 pre-check; ownership + cross-student (`INVALID_HOMEWORK_REF`) **сохранены**; `23505` → **409 `HW_REF_DUPLICATE`** (рус.); мягкий кап `MAX_HOMEWORK_REFS=10` → 409 `LIMIT_REACHED`.
- **Student edge/типы НЕ менялись** (уже массив-ready: `assembleFeedItems` строит `materials[]`, per-student RLS `tlm_student_select` построчная). **Добавлен tie-breaker `created_at`** в сортировке материалов (несколько `homework_ref` имеют `sort_order=0` → без него порядок «плавал», code review P2).
- **Tutor `LessonMaterialsPanel`:** `find`→`filter` (`homeworkRefs`); список привязанных строк (каждая удаляема); пикер НЕ закрывается на attach (мульти-привязка), уже-привязанные `disabled`+«Привязано»; **фильтр по папкам ДЗ** (синергия с rule 40 folders); кнопка «Добавить ещё ДЗ». Один primary CTA (rule 90). Notify TASK-7 цел.
- **Student `MaterialChips`/`LessonDetail`:** `find`→`filter`+`map` (список чипов/блоков, каждый со статус-бейджем + one-hop deep-link). Авто-линк в `TutorHomeworkCreate` ловит и `HW_REF_EXISTS` (старый edge), и `HW_REF_DUPLICATE` (deploy-skew).
- **При расширении:** новый attach-путь → гард от дубля уже на индексе; новый surface списка материалов → `filter` не `find` для homework_ref; детерминированный порядок → `sort_order` + `created_at` tie-breaker.

Build-лог: memory `project_elena_requests_2026_06_17.md`. План: `~/.claude/plans/1-linked-treehouse.md`.

## Attach = assign — прикрепление ДЗ авто-назначает учеников занятия (запрос Егора, 2026-07-20)

Репорт Егора: ДЗ нельзя прикрепить к занятию Ивана, если оно Ивану не назначено (403 «Это ДЗ не назначено ученику этого занятия»), а пикер при этом показывает ВСЕ ДЗ репетитора без фильтра → UI предлагает то, что бэкенд отвергнет. Решение владельца: **факт прикрепления = назначение**. Миграций нет. Build-лог: memory `project_elena_egor_hw_2026_07_20.md`.

- **Отказ заменён авто-назначением** (`handleCreateMaterial`, ветка `homework_ref`): считаем недостающих учеников занятия (`studentSet` минус существующие `homework_tutor_student_assignments`) → назначаем ВСЕХ недостающих. Групповое занятие → все участники (решение владельца; иначе RLS `tlm_student_select` скрыла бы чип ДЗ от неназначенных).
- **Единственный assign-write-path остаётся в `homework-api` (rule 40 — КРИТИЧНО):** `lesson-materials-api` НЕ пишет в `homework_tutor_student_assignments`, а делает **awaited server-side `fetch`** на `${SUPABASE_URL}/functions/v1/homework-api/assignments/:id/assign-students` с **форвардом пользовательского `Authorization`** (+ `apikey` anon) и **`notify:false`**. Так бесплатно достаются whitelist `tutor_students`, идемпотентный upsert, `provisionGuidedThread`, draft→active. `homework-api` имеет `verify_jwt=false` и сам резолвит тутора через GoTrue → форвард JWT работает. `handleCreateMaterial` получает `authHeader` параметром из роутера.
- **Порядок и fail-closed:** UUID/ownership → studentSet (**сбой запроса участников → 503 `RECIPIENTS_LOOKUP_FAILED`**, ревью ChatGPT-5.6 P1: молча проигнорированная ошибка давала ЧАСТИЧНОЕ назначение при успешном тосте) → кап `MAX_HOMEWORK_REFS` **ДО** assign → assign (сбой → 502 `ASSIGN_FAILED`, рус., rule 97) → `insertMaterial`. Материал не появляется без назначений; обратный частичный случай (assign прошёл, insert упал) безопасен — upsert идемпотентен, назначение семантически желанно.
- **Уведомления:** `notify:false` — ученика уведомляет существующий дайджест материалов при закрытии панели (TASK-7), без двойного пуша.
- **Ответ 201 += `assigned_student_ids`** (additive; старый клиент игнорирует) → тост «ДЗ привязано и назначено: {имена}». Пикер показывает серую подсказку «будет назначено: {имена}» (2 лёгких best-effort запроса при открытом пикере: участники занятия + `homework_tutor_student_assignments` под tutor-RLS). `INVALID_HOMEWORK_REF` остаётся для ownership / занятия без учеников / deploy-skew.
- **При расширении:** новый путь «прикрепить ДЗ» → назначение ТОЛЬКО реюзом `homework-api` (не второй write-path); любой новый резолв получателей занятия — fail-closed (503), не мягкая деградация; assign — всегда ДО создания материала.
