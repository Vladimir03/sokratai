...
- Физика ЕГЭ 2026: № 21-26 — **полные ФИПИ 2026 критерии** в `physics-ege.ts` (получены от Vladimir 2026-05-15 в виде 5 официальных PDF от ФИПИ). Покрыто: № 21 качественная (3 балла, 3 элемента), № 22-23 расчётные (2 балла, элементы I-IV), № 24-25 расчётные (3 балла, элементы I-IV + per-1-балл вариант для отсутствия одной формулы), № 26 расчётная с обоснованием (4 балла = Критерий 1 + Критерий 2). Часть 1 (№ 1-20) — справочные баллы по группам задач. Все «Общие правила оценивания» ФИПИ (альтернативная логика, формулы кодификатора, округление, подмена условия) включены в rubric блоки.

**Спека:** `~/.claude/plans/1-functional-meteor.md` Phase 2.

### Покритериальный грейдинг языков — criteria_breakdown (voice-speaking-mvp Этап 1, 2026-05-27)

Расширение subject-rubric layer: для языковых ДЗ (DELF / ЕГЭ EN / ОГЭ — письмо + устный монолог) AI раскладывает `ai_score` по именованным критериям экзамена. Хранится в `homework_tutor_task_states.ai_criteria_json JSONB` (миграция `20260527180000`, формат `[{label, score, max, comment, kind?}]`), рендерится `CriteriaBreakdownTable` (1 страница) у ученика и репетитора. **Голос (Этап 2) — отдельный PR.** Полный контракт + инварианты: CLAUDE.md §35. Спека: `docs/delivery/features/voice-speaking-mvp/spec.md`.

**Где живёт template критериев:** `_shared/subject-rubrics/languages-ege.ts::CRITERIA_*` (структурированные `SubjectCriterionTemplate[]`), пропагируются через `SubjectRubric.criteria_breakdown_template`. При добавлении формата — добавь `METHODOLOGY_*` + `CRITERIA_*` + ветку в `getLanguagesMethodology` + кейс в `scripts/test-criteria-templates.mjs`.

**Инварианты (детали в CLAUDE.md §35):**
- **Sum-агрегация ONLY** — IELTS (average band) отдаёт `criteria = null`, breakdown не рендерится.
...
### Student assignment load — service_role, draft-tolerant (2026-05-28)

**Инвариант:** ВСЕ student-facing загрузки ДЗ (`/homework/:id` redirect-entry, новый problem-screen, list) идут через **service_role edge function** с ownership по `homework_tutor_student_assignments` link — **НЕ** через direct PostgREST на `homework_tutor_assignments` с RLS.

**Почему (баг 2026-05-28, Эмилия DELF A2 PO):** RLS policy «HW students select assigned assignments» (миграция `20260215100000`) фильтрует `status IN ('active','closed')`. Клиентский `getStudentAssignment` делал direct `.single()` на `homework_tutor_assignments` → для `status='draft'` (или RLS edge) возвращал 0 строк → throw → ученик на `/homework/:id` видел «Не удалось загрузить задание» ДО редиректа на рабочий problem-screen. А `handleGetStudentProblem` (новый screen) грузит через service_role (link-based, без status-фильтра) → draft переваривал. Нестыковка: Telegram «Открыть ДЗ» ведёт на legacy `/homework/:id`, который падал, хотя destination работал.

**Канонический путь:** `getStudentAssignment` (`src/lib/studentHomeworkApi.ts`) → `GET /assignments/:id/student` (`handleGetStudentAssignment`, service_role) + `GET /assignments/:id/identity` параллельно. Ownership по link, тот же anti-leak whitelist что и new screen. Backend резолвит `userId` из JWT (надёжнее client-side `getCurrentUserId()` + RLS). 404 только когда ученик реально не привязан.

**При добавлении нового student-facing ДЗ fetch:** НЕ делать direct PostgREST на `homework_tutor_assignments` — RLS спрячет draft. Ходить в service_role edge endpoint с link-ownership. Если нужен whitelist field — расширять edge handler, не клиентский select.

**Известный secondary gap (не критичный):** `listStudentAssignments` (список `/homework`) всё ещё direct PostgREST `!inner` join → draft не появляется в списке. Не блокер (deep-link path исправлен), но при будущей правке списка — тоже на service_role endpoint.
