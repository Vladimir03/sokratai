...
| `src/components/tutor/home/primitives/SokratBearIcon.tsx` — мишка-маскот на событии «Нужна помощь» (`kind='stuck'`) в ленте `/tutor/home` | «Серьёзное рабочее место, не игрушка» (визуальная идентичность) — playful маскот в tutor-инструменте | НЕ нарушает «нет эмодзи в UI chrome» (это кастомная line-art SVG-иконка в стиле lucide, `currentColor`, не эмодзи и не кадр мультфильма → без копирайта). Сознательный тёплый брендовый акцент: «застрял» = метафора Винни в норе → лёгкая улыбка + положительная привязка к Сократу, а не тревога. Редкий (только `stuck`), янтарный (не красный alarm). Если масштабировать в «систему настроений» на все события — пересмотреть как отдельное продуктовое решение. | Vladimir / 2026-06-01 |
