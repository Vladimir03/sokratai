...
**При расширении:** новый delete/cancel-путь — только через guarded RPC с money-guard (никогда raw `.delete()`); новый scope-вариант — синхронно в RPC + клиентских `*Series`-функциях + 3-way диалогах; новая правка состава — через `tutor_*_lesson_participant`-RPC (не прямой PostgREST на junction); остаток — Round B3 (edit-паритет группы) + Round C (resize). Под v2 balance-модель money-guard удаления свапнётся на reversal ledger-debit.
