...
- **B3 — edit-паритет группы:** `GroupDetailsDialog` секция «Изменить детали» (Длительность/Предмет/Заметки) с series 3-way (reuse `updateLesson`/`updateLessonSeries` — group-row `student_id` NULL, COALESCE no-op). Отмена группы — 3-way для серий (`cancelLessonSeries(scope)`; single occurrence — прежний `runCancelGroupLesson`). Время правится через существующий «Перенести группу» (single occurrence); series-wide time-move — minor follow-up.
