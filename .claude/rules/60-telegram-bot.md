...
**Обновление 2026-07-23 (subject-personalization Ф4, решение владельца): предмет тоже ОБЯЗАТЕЛЕН при добавлении** (77% связок были без предмета → AI-контекст/фильтры/аналитика слепы). Обязательность — **ФРОНТОМ** (`AddStudentDialog`: поле вынесено из аккордеона в основную форму, prefill `resolveStudentSubjectPrefill` — один предмет профиля → он, мульти → last-used-из-профиля → первый; bulk — общий обязательный предмет пачки → `bulkAddTutorStudents(names, subject)` → per-row `tutor_students.subject`). **Backend лоялен** (missing subject допустим — deploy-skew со старым бандлом/PWA не блокирует добавление). Контакт остаётся опциональным.
