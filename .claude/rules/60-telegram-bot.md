...
- **2b+ (Parking Lot):** cutover (убрать payment-write из complete, заморозить `tutor_payments`), авто-debit cron + сводка, reframe «Оплаты»/`/pay` на баланс, отчёт родителю (2c), абонементы (3). Лента операций + редактируемое списание + «должники» — TASK-6.
