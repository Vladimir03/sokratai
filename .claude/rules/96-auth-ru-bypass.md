...
⚠️ **`?next=` валидируется ТОЛЬКО через `resolveSafeNextPath` (`src/lib/safeNextPath.ts`).** Префиксный гард `startsWith('/') && !startsWith('//')` — дыра: `?next=/%5Cevil.example` декодируется в `/\evil.example`, проходит проверку, а браузер нормализует обратный слеш в прямой ⇒ protocol-relative ⇒ `window.location.replace` уводит на чужой origin (проверено вживую 2026-08-02, гард возвращал `true`). Правильное правило одно — разобрать `new URL(value, origin)` и сверить `url.origin`; наружу отдавать пересобранный `pathname+search+hash`. Ссылки между auth-страницами — только `withSafeNext`.

⚠️ **Точка возврата обязана пережить ВСЕ способы входа, не только пароль.** OAuth уходит на внешний домен и возвращается по `redirectPath`, подтверждение email — по `emailRedirectTo`. Захардкоженный дефолт там = ученик молча оказывается не на той странице, хотя данные привязались. Новый флоу с `next` → прокинуть его в оба места, а не только в `navigate()` password-ветки.
