...
- Запись токена ТОЛЬКО через SECURITY DEFINER RPC `tutor_ensure_student_claim_token(p_tutor_student_id)` (ownership `auth.uid → tutors.id`, генерит-если-NULL атомарно, CAS `IS NOT DISTINCT FROM`) ИЛИ внутри `homework-api` под service_role.
