...
- Sender: `Сократ <noreply@sokratai.ru>`, **domain `notify.sokratai.ru`** (верифицированный sender Lovable; `sokratai.ru` → `no_matching_sender`, фикс 2026-07-14)
- **`@temp.sokratai.ru`** emails автоматически пропускаются

**Email queue** (две очереди pgmq):
- `auth_emails` — auth-письма (signup/recovery/magiclink) через `auth-email-hook` (webhook даёт **настоящий** `run_id`). **Работает.**
- `transactional_emails` — ДЗ-уведомления/материалы/OTP через `email-sender.ts` (фабрикует `run_id: crypto.randomUUID()`).
...
**⚠️ Транзакционный пайплайн сломан (инцидент 2026-07-14, диагностировано, инфра Lovable):** email-API Lovable отвечает **`404 run_not_found "Run not found or expired"`** на письма из `transactional_emails`, потому что `@lovable.dev/email-js` требует `run_id` реального «run», а `email-sender.ts` кладёт случайный UUID. Auth-письма живут, т.к. `run_id` приходит из send-email webhook'а. Симптом в `email_send_log`: `template_name='student-login-link'/'homework-notification'` → `status='failed'`/`'dlq'`. **Фикс пайплайна — на стороне Lovable** (их `process-email-queue`/`send-transactional-email` должны резолвить валидный run). **Обход для auth-подобных писем:** роутить через нативный Supabase-путь (`signInWithOtp`/`resetPasswordForEmail`), который фаерит `auth-email-hook` → очередь `auth_emails` (работает) — так починен OTP `student-otp-request` (2026-07-14): `generateLink`+ручная отправка → `signInWithOtp({shouldCreateUser:false})` (auth-email-hook уже переписывает `magiclink`-URL на RU-safe, rule 96). Диагностика «письмо не пришло»: `SELECT template_name,status,error_message FROM email_send_log ORDER BY created_at DESC` — `run_not_found` = транзакционный пайплайн; `sent` у recovery + `failed` у остальных = именно этот баг.
