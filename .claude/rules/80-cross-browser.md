# Cross-Browser Compatibility (КРИТИЧНО)

Продукт используется на Safari (macOS/iOS) — это главный источник багов. **Все правила ниже обязательны.**

## Официальный минимум поддержки (решение Vladimir, 2026-07-15)

**Safari 15 / iOS 15 (iPhone 6s/7 и новее).** У школьников массово старые iPhone, застрявшие на iOS 15 навсегда; Telegram in-app браузер на iOS = тот же WebKit. Любая фича, падающая на Safari 15, — блокер (инцидент Глеба: lookbehind в remark-gfm ронял экран задачи у всех на iOS ≤ 16.3).

**QA-чеклист перед крупным релизом (обязательный пункт):** прогнать критический путь **вход → открыть ДЗ → отправить ответ → получить AI-фидбэк** на реальном старом iPhone (iOS 15/16 — есть у учеников Егора). Автоэмуляции старого WebKit у нас нет; Playwright WebKit ≠ старый Safari. Наблюдаемость крашей после релиза — /admin → вкладка «Ошибки» (`analytics_events.client_error`, писатели `ErrorBoundary`/`MarkdownErrorBoundary` через edge `client-error-report`).

## Build targets
- В `vite.config.ts` установлен `build.target: ['es2020', 'safari15', 'chrome90']`
- В `package.json` есть `browserslist` — используется autoprefixer для CSS
- **НЕ МЕНЯТЬ** эти настройки без веской причины

## Запрещённые паттерны (ломают Safari/iOS)

### JavaScript / TypeScript
- **Голая ссылка на необязательный глобал = `ReferenceError`, а не `undefined`.** Проверка поддержки вида `requestIdleCallback ? … : …` в браузере БЕЗ этого API падает сама. Только `window.X`, `typeof X === 'function'` или `'X' in globalThis`. Инцидент 2026-07-27: такая строка в `index.html` роняла загрузку у ВСЕХ пользователей Safari/iOS, и Яндекс.Метрика там не грузилась вовсе — пре-воронка (rule 101) не видела учеников с iPhone. Гард — smoke-check §2 (список необязательных API, только `index.html`: в `src/` это ловит TypeScript).
- **`RegExp` lookbehind** (`(?<=...)`) — Safari < 16.4 НЕ поддерживает. Используй capturing groups. Vite-таргет `safari15` НЕ спасает: esbuild конвертирует литерал в `new RegExp(...)` → SyntaxError «Invalid regular expression» в рантайме при первом исполнении. **Ловится и в зависимостях**: smoke-check §1 фейлит lookbehind в любом чанке `dist/assets`, §2 — в `src/` (инцидент Глеба 2026-07-15: `remark-gfm` ронял экран задачи у всех учеников на iOS ≤ 16.3).
- **`remark-gfm` напрямую ЗАПРЕЩЁН** — только `@/lib/markdown/remarkGfmSafe` (тот же GFM без autolink-literal, чей email-регэксп несёт lookbehind и исполняется при каждом рендере ReactMarkdown). Тело ReactMarkdown в чатах оборачивать в `MarkdownErrorBoundary` (plain-text fallback: падает пузырь, не страница).
- **`structuredClone()`** — Safari < 15.4. Используй `JSON.parse(JSON.stringify(obj))` или lodash `cloneDeep`
- **`Array.at()`** — Safari < 15.4. Используй `arr[arr.length - 1]` вместо `arr.at(-1)`
- **`Object.hasOwn()`** — Safari < 15.4. Используй `Object.prototype.hasOwnProperty.call()`
- **`crypto.randomUUID()`** — только HTTPS + Safari 15.4+. В dev-окружении может не работать
- **`Date` парсинг** — Safari **строг** к формату. `new Date("2024-01-15 10:30:00")` **СЛОМАЕТСЯ**. Всегда используй ISO: `new Date("2024-01-15T10:30:00")` или `date-fns`
- **`AbortSignal.timeout()`** — Safari < 16. Создавай `AbortController` с `setTimeout` вручную
- **`onLoad` у `<img>` НЕ СРАБАТЫВАЕТ для уже декодированной картинки.** Если файл лежит в кэше браузера (повторное открытие, прогрев соседей, data:-URL), событие `load` выстреливает ДО того, как React повесит обработчик, и `naturalWidth` не приходит НИКОГДА. Любой код, считающий размеры по `onLoad`, обязан дочитывать их у готового элемента (`img.complete && img.naturalWidth > 0`) в эффекте. Симптом: вёрстка сваливается в резервную ветку и картинка вылезает за контейнер (2026-07-31: 779 px в контейнере 570 px). Ни tsc, ни тесты, ни сборка этого не видят — ловится только в браузере.
- **HEIC/HEIF в `<img>` декодирует ТОЛЬКО Safari 17+** (наш baseline iOS 15 — НЕ умеет; Windows/Android — никогда). Любой клиентский upload фото — через `compressForUpload` (native decode → wasm-фолбэк → `HeicDecodeError`, сырой HEIC не загружается никогда); любой рендер фото учеников — через `useHeicImage` (`src/hooks`) с фолбэком-конвертацией legacy `.heic`. Пакет `heic-to` (2.9 МБ wasm-чанк) импортировать ТОЛЬКО динамически внутри `src/lib/heicDecode.ts` и НЕ добавлять в `manualChunks` — статический импорт утащит его в eager-граф загрузки. (2026-07-30, баг Ирины Бочаровой: репетитор на Windows не видел решения, AI проверял вслепую)

- **`appearance: none` НИКОГДА не вешать на голый `input`** — стирает нативный виджет у `checkbox`/`radio` целиком: прозрачный квадрат без рамки и галочки, `accent-color` после этого красить нечего. Тапнуть можно только вслепую, обратной связи нет — форма выглядит сломанной. Правило в `@layer base` било ВСЕ 14 сырых checkbox/radio проекта, и его дважды обходили ЛОКАЛЬНО (`.rtc-consent-checkbox`, `.tst-checkbox` — там прямо написано «рендерился невидимым, white-on-white») вместо починки корня; на третьей поверхности ученица не смогла поставить согласие на публичной странице пробника (2026-08-02). Только `input:not([type='checkbox']):not([type='radio'])`. Гард — smoke-check §25. **Следствие:** подсветку ошибки вешать на строку-обёртку, а не на сам `input` — у нативного чекбокса CSS-рамка не рисуется.

### CSS
- **`100vh`** на iOS — НЕ учитывает адресную строку Safari. Используй `100dvh` или `min-height: -webkit-fill-available`
- **`overflow: clip`** — Safari < 16. Используй `overflow: hidden`
- **`scrollbar-gutter`** — Safari НЕ поддерживает
- **`:has()` селектор** — Safari 15.4+, нестабильно. В Tailwind: `group/peer` утилиты
- **`backdrop-filter`** — нужен `-webkit-backdrop-filter` (autoprefixer добавит, но проверяй)
- **CSS `@layer`** — Safari 15.4+. Осторожно
- **`position: sticky` на `<th>` / `<td>` + `border-collapse: collapse`** — **СЛОМАНО** в Safari/iOS WebKit. Sticky-колонка/строка в таблице просто не липнет. Фикс: использовать `border-separate border-spacing-0` (Tailwind) — sticky тогда работает корректно. См. `HeatmapGrid.tsx`
- **`<table className="w-full">` + `min-w-[Npx]` на `<td>`** — table layout алгоритм игнорирует `min-width` на ячейках и сжимает столбцы под container. `overflow-x-auto` родителя в этом случае **никогда** не активируется. Фикс: `<colgroup>` с `<col style={{ width: 'Npx' }}>` + `table-layout: fixed` + inline `width: max-content` (или `auto`) на table
- **Flex-ребёнок с `overflow-hidden` в `flex flex-col` → сжимается почти в 0** (НЕ Safari-специфично, чистая flexbox-спека, но кусает регулярно). У flex-item'а с `overflow` ≠ `visible` `min-height: auto` резолвится в **`0`** → `flex-shrink: 1` (дефолт) сжимает его, отдавая место не-сжимаемым сиблингам (у тех `overflow: visible` → `min-height: auto` = высота контента). Внутри `overflow-y-auto`-ленты высокая карточка (таблица/блок) с `overflow-hidden` схлопывается в 1-2px, её контент режется собственным `overflow-hidden` → **в DOM есть с полным размером внутреннего контента, но видно 1-2px и прокруткой не достать**. Симптом: «компонент рендерится (есть в Elements, `<ul>` 680px), но на экране пусто и не скроллится». Фикс: **`shrink-0`** (`flex-shrink: 0`) на сам элемент. Диагностика: `getBoundingClientRect().height` секции ≈ 2 при большом `scrollHeight` контейнера. Прецедент: `CriteriaBreakdownTable` в ленте чата ДЗ (2026-06-29) — таблица «Разбор по критериям» была 2px. Любой высокий `overflow-hidden`-блок как прямой ребёнок flex-col-скролла → `shrink-0`.
- **Экран высотой ровно с visualViewport (`h-[var(--vv-h)]` + `overflow-hidden`) при заблокированном скролле страницы** (`HomeworkProblem.tsx`, `TutorChat.tsx`) — **любой блок переменной длины внутри ОБЯЗАН иметь свой `overflow-y-auto` и потолок высоты. `shrink-0` на таком блоке = баг, а не защита.** Здесь `shrink-0` — зеркальная противоположность правила выше: там он спасал схлопывание, тут он запрещает единственный доступный способ уместиться. Контент, не влезший в контейнер, обрезается `overflow-hidden`, а доскроллить его НЕЧЕМ — скролл страницы выключен, своего скролла у блока нет. Симптом: контент обрывается на полуслове, а нижние соседи (лента, композер) уезжают за экран целиком. Прецедент: условие задачи на телефоне (2026-07-31, репорт Глеба) — ученик не мог дочитать задачу с перечнем вариантов и не видел поля ответа; фикс — `ProblemContext.scrollBody`. **Потолок считать от `--vv-h`, а НЕ от `dvh`**: `dvh` — Safari 15.4+, на нашем baseline 15.0 правило молча не применится ровно там, где баг и живёт (плюс `--vv-h` сам ужимается при открытой клавиатуре).

### iOS-специфичные проблемы
- **`position: fixed`** + клавиатура iOS — элемент "прыгает". Используй `position: sticky`
- **Safe Area Insets** — `env(safe-area-inset-*)` для iPhone с нотчем/Dynamic Island
- **Touch events** — 300ms delay на tap. `touch-action: manipulation` решает
- **Auto-zoom на input** — Safari iOS зумит если `font-size < 16px`. **ВСЕГДА** `font-size: 16px`+ на `<input>`, `<textarea>`, `<select>` (паттерн: `text-base md:text-sm`)
- **Пинч-зум РАЗРЕШЁН намеренно (2026-07-31) — `user-scalable=no` в `index.html` не возвращать.** Safari его игнорирует, а WKWebView (встроенный браузер Telegram, где сидит половина учеников) — уважает: график в условии задачи было не увеличить жестом вообще. Расплата — тот самый авто-зум на фокусе, поэтому правило про 16px из строки выше стало обязательным, а не желательным. Сеть безопасности — **нелейерное** правило в `src/index.css` (поднимает `input/textarea/select` с `text-sm|text-xs` до 16px на ≤767px). Именно нелейерное: базовое `input { font-size:16px }` лежит в `@layer base`, а слой `utilities` в каскаде позже — любой `text-sm` его перебивает независимо от специфичности. Шим — страховка, не замена: новое поле пишем сразу 16px.
- **`overflow-x-auto` контейнер с кликабельными child-элементами** — на iOS Safari row-onClick может съесть `touchstart` и заблокировать horizontal swipe. Фикс: `touch-action: pan-x` (Tailwind `touch-pan-x`) на скроллящем `<div>`. Применяй ВСЕГДА, когда внутри scrollable области есть row/cell с onClick handler-ом

## Правила при написании кода
1. **Перед использованием нового Web API** — проверь поддержку на caniuse.com для Safari 15+
2. **Для дат** — всегда `date-fns` (`parseISO`) вместо нативного `Date` парсинга. Используй `src/lib/formatters.ts`
3. **CSS анимации** — предпочитай `transform` и `opacity` (GPU-ускорены)
4. **Тестируй в Safari** — при изменениях CSS layout, scroll-поведения или форм
