...
13. **Перенос занятия — только RPC `tutor_move_lesson`** (Волна 2 расписания, 2026-08-06), включая смену длительности/ученика прошедшего (money-поля одной транзакцией; `student_id` выводит сервер). `_apply_lesson_debit_from_current_cost` для БУДУЩЕГО занятия — no-op, НЕ reverse: путь past→future обязан сам звать `_reverse_lesson_debit` (RPC это делает). Прямой `UPDATE start_at` прошедшего-с-debit ИЛИ уводящий занятие в прошлое блокирован триггером `trg_tutor_lessons_guard_start_move` (GUC `app.lesson_move`) → `MOVE_VIA_RPC`. **Состав/ученик, прочитанные до лока, перечитываются ПОД локом** (move, cancel-with-charge `20260806140300`, series-cost, completion-снапшот); расхождение → `LEDGER_CONFLICT`, не тихое списание не с того ученика.
