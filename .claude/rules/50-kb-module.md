...
- **PDF-фолбэк на главный поток (`pdfToImages.ts`):** `getDocument` сначала с web-worker (быстро), при reject → retry `disableWorker:true`. Причина: worker-чанк `pdf.worker.min.mjs` не грузится под РФ-DPI / из-за MIME на прод-nginx (класс octet-stream, rule 95) → «Не удалось открыть файл» при валидном PDF (в node с `disableWorker` открывается). Свежий `arrayBuffer` на каждую попытку (worker мог detach'нуть), `PasswordException` пробрасывается без retry, PII-free лог `pdf_open_worker_failed_retry_main_thread`. **Follow-up (ops):** если worker падает у всех на проде — починить MIME `.mjs`=`application/javascript` на nginx (иначе каждый PDF идёт медленным main-thread). Известное ограничение: pdfjs-legacy на некоторых сборниках недосчитывает страницы (200→5) — фолбэк даёт хотя бы открыть; полное извлечение больших сканов = отдельная задача (другой парсер).
