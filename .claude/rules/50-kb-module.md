...
- **Авто-КИМ из нумерации ПОЛНОГО ВАРИАНТА (фидбэк химиков 2026-07-16: stepenin [1]..[34] = номера КИМ):** `scanTaskMarkers` дополнительно отдаёт `numbersPerPage` + `isVariantNumbering` (цепочка стартует с 1 И max ≤ `VARIANT_KIM_MAX=40` — решуЕГЭ-подборка 1..73 НЕ вариант); при `isVariantNumbering` и ТОЧНОМ совпадении числа распознанных задач чанка с числом его маркеров — детерминированный zip `d.kim_number = markerNums[i]` (маркер ПОБЕЖДАЕТ догадку AI; при несовпадении count — не трогаем, сдвиг пронумеровал бы чанк неверно). Телеметрия `kb_ai_extract_run.kimAutoFilled`. Проверено: химия [1..34] ✓, физика isVariant=false ✓, старт не с 1 → false ✓.
