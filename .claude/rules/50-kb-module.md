...
- **Скоуп — ТОЛЬКО destructive (решение владельца):** create/edit тем/подтем/источников остаются глобальными (`kb_require_moderator()`); удаление + перенос — через `kb_require_moderator_subject(p_subject)`: **`is_admin` bypass ПЕРВЫМ** (владелец — не модератор, но админ; иначе `kb_require_moderator()` его бы отсёк), иначе роль moderator + `p_subject = ANY(tutors.subjects)`; пустые subjects → deny. Прод: Егор=physics, Мила=social, Эмилия=french, Светлана=maths, владелец=is_admin. Клиентский гейт-зеркало: `canMod = isModerator && mySubjects.includes(topic.subject)` (`useTutorProfile().subjects`).
- **«Section» — НЕ сущность**, это `kb_topics.section TEXT`; delete-section = все темы `WHERE subject+section+((filter='olympiad' AND kind='olympiad') OR (kind='exam' AND exam=filter))`.
...
- **`space_check` (folder+topic не оба NULL): nulling `topic_id` ВСЕГДА в одном UPDATE с `folder_id`** (иначе CHECK-violation). **P0-гарды НЕ откатывать:** template-RESTRICT, folder-check только при непустой теме, foreign-source block.
- **UI:** `TaskCard` kebab «Снять публикацию» → **«Перенести в Мою базу»** (`onMoveToMyBase`, `FolderDown`); `CatalogTopicPage` «Удалить тему» + subject-гейт колбэков; `KnowledgeBasePage`(CatalogHome) «Удалить раздел» на заголовке секции; `SubtopicManager` `canDelete`. Переиспользуемые: **`FolderPickerModal`/`FolderTreeSelect`** (пикер личной папки, фильтр корня «сократ»; НЕ ломает `CopyToFolderModal` для обычной «К себе»), **`DeleteCatalogDialog`** (красный хром `DeleteFolderDialog` + встроенный пикер + счётчики). RPC ещё не в `types.ts` → `as never` на границе `supabase.rpc` (rule 99).
- **При расширении:** новый destructive-путь модератора → через `kb_require_moderator_subject` (не голый `kb_require_moderator`); перенос задачи из каталога → через `_kb_mod_copy_to_base` (own-relocate/orphan-copy/foreign-block, template-гард); nulling topic_id — в одном UPDATE с folder_id; foreign-moderator source — блокировать, не трогать.
