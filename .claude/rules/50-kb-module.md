...
- **Текстовый слой PDF** (`pdfToImages.ts::renderPdfPagesToFiles` → `pageTexts[]`, выровнен с `files`; <40 симв = null-скан): `InputStage` держит `pageTextByFileRef: Map<File, string|null>`; `buildChunkText` кладёт в вызов «— ТЕКСТ СТРАНИЦЫ N —» страниц чанка (точный источник условий, промпт велит НЕ пере-OCR-ивать; в словах бывают разрывы переносов «Ка мень») + «— ПРОДОЛЖЕНИЕ —» (первые 600 симв следующей страницы — для задачи, разрезанной границей; новых задач оттуда не начинать). Кап `CHUNK_TEXT_CAP=59k` (зеркало edge MAX_TEXT_CHARS).
- **Ожидание по маркерам — `src/lib/taskMarkers.ts::countSequentialTaskMarkers`**: «N.» (точка, НЕ «N)» — внутризадачные списки вариантов не матчатся) + «Задание N» + «[N]» (формат stepenin.ru — варианты химии/биологии), принимается ТОЛЬКО монотонная цепочка (+1, допуск одного пропуска), цепочка <3 → все null (не сборник). Проверено на реальных PDF: физика решуЕГЭ [15,16,22,19,1] = ровно 73; химия stepenin [5,4,5,4,3,4,8,1,0,0] = ровно 34 (полный вариант ЕГЭ). Недооценка безопасна, переоценка — нет.
