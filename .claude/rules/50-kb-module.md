...
- **Дерево тем ЕГЭ обществознание — миграция-сид `20260721120000_kb_seed_social_ege_topics.sql`:** `subject='social'`, `kind='exam'`, `exam='ege'`, `kim_numbers='{}'`, 75 тем по **актуальному Кодификатору ФИПИ 2026** (5 разделов: Человек в обществе. Духовная культура / Экономическая жизнь общества / Социальная сфера / Политическая сфера / Правовое регулирование общественных отношений). **Идемпотентно** (`INSERT…SELECT…VALUES…WHERE NOT EXISTS` по `subject+exam+lower(name)`) — не дублирует темы, заведённые Milada вручную. Подтемы v1 НЕ заводим — Milada добавляет self-serve (`SubtopicManager`/`kb_mod_*`), она же правит формулировки. **Новое предметное дерево каталога — миграцией-сидом** (как все topic-сиды), не Lovable-MCP (тот для kb_tasks).
...
- **Deploy:** миграция (Lovable на push, тем нет до синка — каскад просто пуст, graceful) → фронт `deploy-sokratai`. Edge не трогаем.
