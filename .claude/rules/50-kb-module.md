...
- **Модели загрузчика (решение владельца 2026-07-16, ОТДЕЛЬНО от грейдинга):** extract/refine → `google/gemini-3.5-flash` (vision №1 Roboflow — точнее bbox/OCR сканов); авто-повтор недобора → **эскалация** на `google/gemini-3.1-pro-preview` (клиент шлёт `boost:true` в body — deploy-skew-safe, старый edge игнорирует); обе строки при отказе шлюза (400/404/422) → **fallback** `google/gemini-3-flash-preview` (`callLovableJson` опции `{model, fallbackModel}`, лог `kb_ai_extract_model_fallback`). Константы в `kb-ai-extract/index.ts` (`EXTRACT_MODEL`/`EXTRACT_BOOST_MODEL`/`EXTRACT_FALLBACK_MODEL`). **Грейдинг ДЗ НЕ трогали** — там «не финализировать до бенчмарка» (memory `project_ai_model_architecture_2026_07_12.md`). Эффект мерить встроенной метрикой «Найдено X из ~Y» + `crop_action` (edited vs suggested) + `token_usage_logs` (модель echo'ится шлюзом — cost-slice работает сам).
