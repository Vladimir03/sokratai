...
## Канонический формат ответа — один сериализатор, и его понимают НЕ все потребители

`src/lib/answerAlternatives.ts` (+ Deno-зеркало `_shared/answer-alternatives.ts`, дрейф ловит smoke): несколько верных ответов — через `« или »`, числовой диапазон — `2,1 – 2,3` (тире **с пробелами**; компактное `1941-1945` диапазоном НЕ считается). Промпт `kb-ai-extract` учит модель этому формату для **ВСЕХ** назначений загрузчика, а понимает его только грейдинг ДЗ (`tryDeterministicShortAnswerMatch`). **Чекеры Части 1 пробника сравнивают строку БУКВАЛЬНО** — поэтому mock-путь ОБЯЗАН нормализовать ответ (`firstExactAnswer` в `aiExtractToVariantTaskDraft`), а не просто прятать UI-чипы: иначе ученик с верным «2» получает 0 против сохранённого «2 м/с² или 2» (ВОЛНА 9, ревью). Новая поверхность ввода ответа или новый маппер → явно решить, понимает ли потребитель мульти-формат.

**Реконсиляция «критерии ↔ балл» живёт в UI (`onChange` редактора пишет `Math.round(Σ AI-max)` в поле балла), а НЕ в commit** — зеркало `CreateTaskModal`. Обратный порядок (`criteriaScore ?? manualScore` при сохранении) даёт «репетитор видит 5, сохранилось 3» и мимо реконсиляции проезжают мапперы, которые её не знают (hw).
