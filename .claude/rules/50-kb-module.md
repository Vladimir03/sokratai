...
- **Ожидание по маркерам — `src/lib/taskMarkers.ts::countSequentialTaskMarkers`**: «N.» (точка, НЕ «N)» — внутризадачные списки вариантов не матчатся) + «Задание N», принимается ТОЛЬКО монотонная цепочка (+1, допуск одного пропуска), цепочка <3 → все null (не сборник). Проверено на реальном PDF: per-page [15,16,22,19,1] = ровно 73. Недооценка безопасна, переоценка — нет.
