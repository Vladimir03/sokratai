...
- **MRR = run-rate**, не выручка: Σ суммы ПОСЛЕДНЕГО succeeded-платежа `plan='tutor_ai_start'` каждого репетитора за 35 дней (30 подписки + 5 grace). Ручные гранты НЕ в MRR (но в «Платящие» = валидный premium из `profiles`, `expires_at IS NULL` = бессрочный). Δ = сравнение двух rolling-снапшотов, подписывать «Δ run-rate», не «за неделю». **Известное ограничение: возвраты не вычитаются** — `yookassa-webhook` ищет платёж по `object.id`, а у refund-события там ID возврата (платёж в `object.payment_id`) → возвращённая оплата остаётся `succeeded`. Дисклеймер в tooltip; фикс — отдельная money-задача с ревью.
