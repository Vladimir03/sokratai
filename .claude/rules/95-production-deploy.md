...
8. **`npm ci` БЕЗ `--prefer-offline`.** Флаг доверяет метаданным кэша машины без ревалидации → деплой падает `ETARGET` на версии, которая в реестре есть, а в кэше VPS ещё нет (2026-07-27, три оборванных деплоя). Версии держит лок, не кэш. Отличать от `EUSAGE` (тот = лок разошёлся с `package.json`).
9. **Деплой только с чистого `main`.** `git pull --ff-only` не проверяет ни имя ветки, ни чистоту дерева — забытая на VPS feature-ветка уехала бы в прод.

Гарды: smoke-check **§20** (SW-инварианты) и **§21** (нет `modulepreload` на `/src/*`; `deploy.sh` не вернул destructive `rm -rf`, сохранил `--size-only`/`touch -c`/`--exclude='/.*'`/`--delete` **в КОДЕ, не в комментариях**, порядок «ассеты → корень», lock/branch-гарды стаба, **нет `--prefer-offline` у `npm ci`**).
...
- **Зависшая загрузка ОБЯЗАНА иметь таймаут и выход.** Спиннер без предела — не «загрузка», а тупик: ученик под VPN 27.07 смотрел на «Загрузка фото условия...» бесконечно, а условия задач были ТОЛЬКО картинками (`task_text` = «[Задача на фото]»), поэтому он не видел задания вообще. Любое состояние ожидания контента → таймаут (`useConnectionTrouble`, 8 с) → подсказка + «Повторить».
- ⚠️ **Исключение из «не винить сеть»: подсказка про VPN ученику разрешена** (`ConnectionTroubleNotice`). Запрет ниже — про ЛОЖНЫЕ «VPN»-алярмы в кабинете репетитора, где виноват был наш кросс-граничный хоп. Здесь причина подтверждена экспериментом (выключил VPN — загрузилось), формулировка уточняется пробником `connectionProbe` (сервер ответил, а контент нет → VPN; сервер молчит → интернет) и ВСЕГДА содержит вторую версию. На экране входа — только **после неудачной попытки**, не по таймауту ввода.
