...
### Шаг 6.5 — Публикация в Каталог (ТОЛЬКО по явной команде владельца)

По умолчанию задачи остаются в черновиках на ревью модератора. Если владелец просит опубликовать сразу:
- **Резолвить темы live** (`kb_topics` по subject+exam+kind) и разложить задачи по НИМ; создать подпапки-темы внутри дерева «сократ» модератора (у Егора ОГЭ живёт в «сократ / ОГЭ / <Тема>», folder `725bfa19-...`).
- **ОДИН UPDATE**: `folder_id` (→ подпапка в «сократ») + `topic_id` + при необходимости `kim_number`/`solution` → триггер `trg_kb_after_update_moderation` CASE A вызовет `kb_publish_task` (копия `owner_id=NULL`). Требования CASE A: `published_task_id IS NULL`, `topic_id NOT NULL`, `kb_is_in_socrat_tree(folder)`, `has_role(owner,'moderator')`, изменился folder (или topic с NULL).
- **Задача без `topic_id` МОЛЧА не опубликуется** (rule 50 CASE A) — проставлять тему всем.
- Верификация: `published_task_id IS NOT NULL` у всех + копии `owner_id IS NULL`, `moderation_status='active'` (не `hidden_duplicate`), `attachment_url`/`source_label` перенесены.
- Публикация обратима: правки источника авто-синкаются (CASE B resync); откат — `kb_mod_unpublish` / hard-delete RPC.
...
- **НИКОГДА** не публиковать в Каталог по своей инициативе — по умолчанию только в черновики модератора; публикация (шаг 6.5) — лишь по явной команде владельца.
- **НИКОГДА не хардкодить и не переиспользовать id тем между сессиями.** Модератор перестраивает таксономию: Егор 23–26.07 удалил 5 ОГЭ-тем и создал 14 новых. Удаление темы делает benign-детач чужих задач — `topic_id=NULL` + **`source_label` перезаписывается на `'my'`** + переезд в выбранную им папку (выглядит как «задачи пропали»). Всегда резолвить темы live-запросом перед вставкой/публикацией и сверять `topic_id`/`source_label` ранее загруженных задач.
