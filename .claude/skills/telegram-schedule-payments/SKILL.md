...
### Обязательные поля ученика — имя + ПРЕДМЕТ (обновлено 2026-07-23)

Решение Vladimir (онбординг-активация v2, 2026-07-01): минимум для **добавления** — имя (контакт опционален). Контакт/канал нужен **до первой отправки ДЗ**, а не до создания: репетитор заводит плейсхолдер по имени, копит ему ДЗ, и подключает ссылкой/QR (share-gate) при первой выдаче (create-then-claim). Это **ослабляет** прежнее правило «имя + 1 контакт» (2026-06-07) — баг «репетитор не знает контакт → заводит фейковый email». Для **редактирования** — только имя.

**Обновление 2026-07-23 (subject-personalization Ф4, решение владельца): предмет тоже ОБЯЗАТЕЛЕН при добавлении** (77% связок были без предмета → AI-контекст/фильтры/аналитика слепы). Обязательность — **ФРОНТОМ** (`AddStudentDialog`: поле вынесено из аккордеона в основную форму, prefill `resolveStudentSubjectPrefill` — один предмет профиля → он; мульти → last-used ЕСЛИ он из профиля, **иначе ПУСТО** (ревью ChatGPT-5.6 2026-07-23: «первый элемент массива» — не профильный дефолт, а произвольный; молча сохранённый неверный предмет, особенно bulk на 50 строк, хуже пустого required-поля); bulk — общий обязательный предмет пачки → `bulkAddTutorStudents(names, subject)` → per-row `tutor_students.subject`). **Backend лоялен по НАЛИЧИЮ, строг по СЛОВАРЮ**: missing subject допустим (deploy-skew со старым бандлом/PWA не блокирует добавление), но значение валидируется по `CANONICAL_SUBJECT_IDS` (`_shared/subjects.ts`) — неканоническое → null + PII-free warn, иначе мусор размножился бы на всю пачку. Контакт остаётся опциональным.
- **`tutor-manual-add-student`** (add): contact-gate `email || telegram` **УДАЛЁН** (был ~стр. 181–186); имя — единственный обязательный gate; email-формат проверяется только если задан; нет контакта → temp-email плейсхолдер. **Короткий логин (Егор, 2026-07-20):** temp-email теперь `s{7}@temp.sokratai.ru` (`generateUniqueShortTempEmail`, алфавит без 0/1/o/i/l) вместо `manual_{uuid}@…` — вбиваемый. **Pre-check уникальности обязателен** (`find_auth_user_id_by_email`) + для СГЕНЕРИРОВАННОГО email коллизия в `email_exists`-ветке → регенерация, **НЕ re-resolve** (иначе привязка к чужому аккаунту, cross-tenant; тьютор-заданный email → re-resolve как раньше). Суффикс `@temp.sokratai.ru` НЕ менять (гварды по коду). Action **`bulk-add-students`** `{ names[] }` (≤50) → `createPlaceholderByName` (partial `{created[], errors[]}`). Телеметрия `tutor_first_student_added`.
- **`AddStudentDialog`** (frontend): contact-gate-toast убран; плашка «без контакта — это нормально»; bulk-режим (textarea имён + `bulkAddTutorStudents`). **`StudentCredentialsModal` показывается ВСЕГДА, когда бэкенд вернул логин+пароль — включая name-only плейсхолдера** (Егор 2026-07-20: логин теперь короткий и вбиваемый, «вот логин/пароль, всё работает, хочешь меняй»; раньше пряталось, т.к. UUID-логин был бесполезен). Три способа входа сосуществуют: логин+пароль · короткий код (`ConnectStudentSheet`) · OAuth. Модалка несёт подсказку «можно не менять / код на карточке / привязать почту-ВК-ТГ позже».
- **Вход плейсхолдера = короткий многоразовый claim-код (№43, 2026-07-20)**: `ConnectStudentSheet` показывает 8-симв. код `XXXX-XXXX` (mono, click-to-copy, `formatClaimCode` из `telegramLinks.ts`) над QR — репетитор диктует, ученик вводит на `/login` («У меня есть код от репетитора») или открывает ссылку `/c/{код}`. Код живёт до установки почты+пароля; надж «Подключить» на `StudentCard` виден и для заходившего-но-незарегистрированного. Канон механики (гейты/форматы/rate-limit) → rule 96 «Онбординг-активация v2».
...
### Полировка расписания по репортам владельца (2026-08-07, `0eefbe4`)
- **`PostLessonSheet` УДАЛЁН — не возрождать.** После Phase 2b он стал байт-в-байт той же обёрткой вокруг `LessonMaterialsPanel`, что и `LessonMaterialsDrawer`, и давал ВТОРУЮ кнопку материалов в карточке занятия. Единственный вход в материалы — «Материалы» (drawer). Вместе с ним удалены недостижимые `handleCompleteLesson` / `handleCancelLessonFromSheet` (complete-with-paid из кабинета не вызывается с cutover) и ветка конфетти.
- **Закрытие шита материалов — `commitPendingAndNotify({ strict })`, а не голый `flushNotifyOnClose`.** Сначала досохраняем введённую, но не добавленную ссылку записи, потом notify-дайджест. `strict: true` («Готово») при битой ссылке возвращает false → хост НЕ закрывается (ввод не теряется); `strict: false` (Esc/overlay) сохраняет валидное и закрывает всегда — иначе недописанный URL держал бы шит заложником. Новый хост панели обязан звать именно этот метод.
- **Полоска типа занятия = цвет из легенды** (`getLessonTypeStripeColor` → `getLessonTypeColor`) + белая внутренняя грань. Прежние «мягкие» оттенки (у обычного урока `bg-white/60`) на цветной ячейке ученика были не видны — репетитор считал, что тип не отображается.
- **`consultation` выведен из `LESSON_TYPES`** (данные прода: 4 занятия, 1 репетитор, февраль). Старые строки НЕ мигрированы: подпись в `LEGACY_LESSON_TYPE_LABELS`, цвет — фолбэк. Выводя тип из выбора, ВСЕГДА оставляй legacy-подпись, иначе история молча переименуется в «Урок».
- **Точка у имени в `StudentCard` = цвет ученика.** Янтарная точка «AI не настроен» переехала на `AlertTriangle`: рядом с цветными точками групп она читалась как цветовая метка (репорт). Правило: цветная точка = идентичность (группа/ученик), иконка = статус/предупреждение.
- **Три входа в одно действие — антипаттерн.** «Добавить занятие/Личное дело» жили в шапке, под сеткой и в меню по клику на ячейку; нижний ряд удалён. Редкие действия (ссылка для записи, настройки календаря, напоминания) — под одним kebab; в шапке одна primary-кнопка (rule 90).
