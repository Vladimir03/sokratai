---
name: ai-quota-subscriptions
description: AI-квота сообщений и подписки: get_subscription_status, лимиты 10/50 и boost от платящего репетитора, админ-выдача тарифа, самообслуживаемая оплата YooKassa тарифа AI-старт, возвраты и payment_refunds, промо BLINOV_20, демо-разбор репетитора. Загружай при работе с квотами AI, подписками, тарифами или платежами YooKassa.
---

<!-- Извлечено из .claude/rules/99-ai-quota-subscriptions.md 2026-07-25. Текст ниже — ДОСЛОВНО прежнее
...
- **Возврат = отдельная append-only строка `payment_refunds`** (PK = YooKassa refund id → повторная доставка идемпотентна by construction; частичных возвратов на платёж может быть несколько). Запись — только SECURITY DEFINER RPC `yookassa_record_refund` (service_role); RLS без политик → клиенты видят 0 строк.
- **⚠️ ИНВАРИАНТ грантов service_role-only RPC (инцидент 2026-07-15): `REVOKE FROM PUBLIC` НЕДОСТАТОЧНО.** Supabase default privileges схемы `public` грантят EXECUTE ролям `anon`/`authenticated` НАПРЯМУЮ — отзывать надо явно: `REVOKE ... FROM PUBLIC; REVOKE ... FROM anon, authenticated; GRANT ... TO service_role;` (эталон — `yookassa_activate_subscription`, `20260702150000:118-120`). Миграция `20260715130000` этого не сделала → `yookassa_record_refund` сутки исполнялась под anon-ключом (клиент мог фабриковать возвраты → занижать MRR; денег не двигало); закрыто `20260715150000`. **Новая service_role-only RPC → тройной revoke + верификация PostgREST-пробой под anon (ждём 42501 permission denied, НЕ ответ функции).**
