...
- **Видимость:** репетитор — колонка «Самост.» в heatmap (скрыта при `work_mode='independent'`, `results.work_mode` в корне ответа) + строка в разборе + `EditScoreDialog` («Помощь AI: N → самостоятельность NN%»; для legacy-строк остаётся прежний текст про деградацию). Ученик — раскрываемый чип на экране задачи и строка в «Результате работы» (решение владельца: метрика **открыта**, «мы именно его мотивируем думать головой»).
