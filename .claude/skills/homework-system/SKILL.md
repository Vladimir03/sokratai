...
- **В отчёт родителю метрику НЕ тащим** — `_shared/student-progress-build.ts` шарится с публичным отчётом, `ai_help_events` там намеренно не селектится (видимость = отдельное решение).
