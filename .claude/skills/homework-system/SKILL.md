...
description: Глубина системы домашних заданий SokratAI (homework_tutor_*, guided chat) — специфика грейдинга по предметам и критериям, CEFR/языки, физика Часть 2 flowchart, голосовые задания, экран задачи ученика, шаблоны и Банк ДЗ, папки ДЗ, share-ссылки и превью, «требует проверки», конструктор ДЗ. Загружай, когда работаешь с ДЗ, guided-чатом, проверкой ответов AI, конструктором ДЗ, шаблонами или прогрессом учеников. Жёсткие инварианты (dual write-path, anti-leak, шаги баллов, QA-чеклист конструктора) живут в .claude/rules/40-homework-system.md и всегда в контексте.
---

<!-- Извлечено из .claude/rules/40-homework-system.md 2026-07-25. Текст ниже —
     ДОСЛОВНО прежнее содержимое (1516 строк, ~57 700 токенов), которое грузилось
     в каждую сессию. Инварианты, нарушаемые первой же правкой, оставлены в
     rules; здесь — глубина, подгружаемая по надобности. См. AGENTS.md
     «Бюджет контекста». -->
...
- **Классификация в конструкторе** — полный каскад `TaskClassificationFields` (variant: `hideAnswerFormat`+`hidePrimaryScore`) в HWTaskCard; № КИМ редактируем → физика Часть 2 получает ФИПИ-flowchart-грейдинг из ручных задач (вопрос Егора закрыт). Авто-балл по ФИПИ подсказывается под «Макс. баллов» — **явный `scoreTouchedRef`** гейтит применение (blur ставит touched при «≠авто», «Сбросить»/invalid снимают, Σ критериев→touched); НЕ эвристика `max===1` (ручная «1» иначе затиралась). На снимок едет только `kim_number`; topic/exam/difficulty/subtopic/source_label — на зеркало Базы.
- **Каскад-поля (exam/difficulty/topic_id/subtopic_id/source_label) — исключение из «новое поле → в buildTaskSignature»:** они НЕ персистятся в `homework_tutor_tasks` (снимок round-trip'ит только `kim_number`), поэтому в подписи их НЕТ (иначе phantom-dirty «сохранено», а после reload — пусто). Их правка на существующей задаче уезжает в Базу через **«Обновить в Базе»**: `computeTaskContentFingerprint` ВКЛЮЧАЕТ каскад (divergence-детект видит правку темы), push-body шлёт их **presence-семантикой — только непустые** (edit-prefill Базу не грузит; слепой `topic_id: null` затёр бы тему источника; backend `homeworkTaskFieldsToKbUpdate` мержит по `key in t`). Новое KB-mirror-only поле (без колонки в снимке) → тем же путём: fingerprint + push presence, НЕ buildTaskSignature.
- **Перф конструктора (rule 40 high-risk):** `HWTaskCard` мемоизируется через обёртку `TaskCardRow` (memo + стабильные per-index диспатчеры), НЕ прямым `memo(HWTaskCard)` (внутренности не трогаем); `HWTasksSection.onChange` = `Dispatch<SetStateAction<DraftTask[]>>` (functional-updater'ы — async `handleAddFromKB` мержит на живом prev, не stale snapshot); справочники каскада (`useTopics`/`useSubtopics`/`useKbSources`) — `quiet` (staleTime 10 мин, без focus/interval-рефетчей: хук монтируется в КАЖДОЙ карточке).
...
**Дефолты конструктора (`TutorHomeworkCreate.tsx`, обновлено 2026-07-07):** `meta.subject` в create-mode = `resolveTutorDefaultSubject(кэш профиля, readHwLastSubject())` (`src/lib/tutorSubjects.ts`; last-used → профиль → physics), читается **СИНХРОННО** через `queryClient.getQueryData(TUTOR_PROFILE_CARD_KEY)` в lazy-init — НИКАКИХ эффектов-клобберов/новых useQuery (QA-класс багов этого файла); edit-prefill/template-load/URL `?subject=` пути не изменены. После успешного create — `saveHwLastSubject(meta.subject)`. **HWDrawer (path B) больше НЕ хардкодит `subject:'physics'`** — селектор «Предмет ДЗ» в футере корзины (полный SUBJECTS; prefill-приоритет: ЕДИНЫЙ `HWDraftTask.subjectSnapshot` всех задач корзины → resolve; touched-guard; смешанные snapshots → молчим). **`subjectSnapshot`** — новое поле-носитель path B (ревью р.2 `87afcf9`): пишется в `hwDraftStore.addTask(task, subtopic, topic, subjectHint)` из `kb_topics.subject` темы (`CatalogTopicPage`; FolderPage hint не передаёт — undefined = прежнее physics-поведение эвристик); `resolveCheckFormatFromKb` в addTask получает его же (физ-КИМ-эвристика subject-aware). Новое поле корзины → дописывать в `HWDraftTask` + `addTask` (rule 40 path B carrier-типы). Смена предмета → открыть L1.
...
- **`criteria_breakdown=null`** для физики — языковой sum-table неприменим; визуальная трасса живёт в **отдельном** `ai_nodes_json` (см. Phase C ниже).

**Phase C — визуальная трасса блок-схемы (2026-07-04, задеплой Lovable + `deploy-sokratai`):**
- **Хранение — `homework_tutor_task_states.ai_nodes_json JSONB`** (миграция `20260704160000`, additive + column-GRANT `authenticated`, mirror `ai_criteria_json`). Shape `{ score, max_score, confidence, steps: [{node, verdict:'yes'|'no'|'partial', note?}] }`. **student-visible** (feedback-слой; strip его НЕ трогает — spread оставляет; GRANT нужен явно, rule-40 column-GRANT инвариант). **Отдельный столбец, НЕ `ai_criteria_json`:** трасса = дерево решений (балл — исход ветки), а не Σ критериев → в языковой sum-table «врала» бы (напр. `Лишние записи: yes` = плохо).
- **Положительная полярность узлов (КРИТИЧНО для UI):** все `step(...)` в `walkPhysicsFlowchart` сформулированы как «критерий выполнен?» → `verdict='yes'` ВСЕГДА = хорошо (✓), `partial`=⚠, `no`=✗. Бывшие bad-polarity узлы переименованы (`Лишние записи`→`Нет лишних записей`, `Ошибки в объяснении`→`Объяснение без ошибок`, `Не хватает N законов`→`Все необходимые законы/явления приведены` partial/no, `Ошибка в одной формуле`→`Не более одной ошибки в формулах` partial/no). **Балл-логика НЕ изменена** (только display-строки; тесты `test-physics-flowcharts.mjs` §12 добавили polarity-гард: полный балл → все узлы `yes`).
- **Проброс:** `walk.trace` → `GuidedCheckResult.flowchart_trace: PhysicsFlowchartTracePayload` (в `evaluatePhysicsPart2`) → `runStudentAnswerGrading` пишет `ai_nodes_json` во ВСЕ 3 verdict-ветки (CORRECT/ON_TRACK/INCORRECT, рядом с `ai_criteria_json`) + echo в responseData `flowchart_trace`. `nodesJson` без ренормализации (физика `deterministic_score` → `effectiveAiScore===ai_score`). `THREAD_SELECT` несёт `ai_nodes_json`.
- **Рендер — `PhysicsFlowchartTrace.tsx`** (shared student+tutor, mirror `CriteriaBreakdownTable`): узлы ✓/⚠/✗ (emerald/amber/rose waiver) + footer «Балл по блок-схеме N/M». Mount: `HomeworkProblem.tsx` (ученик, тот же `criteriaRef` scroll-target) + `GuidedThreadViewer.tsx` (тутор, `showConfidence` → low-confidence-хинт). **Взаимоисключение с `CriteriaBreakdownTable`** гейтами (физика: `ai_criteria_json=null`, `ai_nodes_json=set`; языки — наоборот). `shrink-0` обязателен (rule 80 flex-collapse).
- **Инвалидация эталона при правке (Phase C, `handleUpdateAssignment`):** `referenceResetFieldsIfChanged` сбрасывает `ai_reference_*` у существующей задачи ТОЛЬКО если реально изменились `task_text`/`task_image_url`/`solution_text` (сравнение до/после через доп. SELECT этих полей). Закрывает 2 бага: (а) правка условия → грейдинг сверял бы новое решение со старым эталоном; (б) тутор добавил своё `solution_text` → stale AI-эталон побеждал бы его (reference-priority `aiReferenceSolution ?? solutionText`). `enqueueReferenceGeneration` (уже в конце) регенерирует eligible.

**При расширении:** новый узел/схема → `physics-flowcharts.ts` (+ тест) + описание узла в `physics-node-prompt.ts`; balл — только код `walkPhysicsFlowchart` (не модель); **новый узел ОБЯЗАН быть положительной полярности** (yes=выполнено), иначе ✓ в трассе врёт; новый детерминированный грейдинг-путь → `deterministic_score` + (если есть трасса) `flowchart_trace`→`ai_nodes_json`; generic-физика (без № 21-26) → holistic fallback (осознанно). **Tutor-view AI-эталона** (репетитор видит/правит `ai_reference_solution`) + **Phase D** (eval на размеченных работах Егора → калибровка узел-промпта) — отложены.
...
**Fix — явный `homework_tutor_tasks.cefr_level`** (миграция `20260529180000`, nullable + CHECK `A2/B1/B2/C1`; **A1 добавлен `20260714120000`** — запрос Эмилии). Селектор «Уровень (CEFR)» в `HWTaskCard` (gated `cefrLevelEnabled` = subject ∈ {french, english, spanish}). `null` = авто-детект (прежнее поведение).
...
- Французская ветка: `A1 → delf-a1-*`, `A2 → delf-a2-*`, `B2/C1 → delf-b2-*`, `B1/else → delf-b1-*`. **A1-рубрика (2026-07-14, запрос Эмилии) + A2-рубрика добавлены** (`METHODOLOGY/CRITERIA_DELF_{A1,A2}_ECRITE/ORALE`; Σ écrite=25, Σ orale AI-gradable=23 + phonétique tutor_only). **A1/A2-критерии = DRAFT, валидирует Эмилия.** C1 пока маппится на B2 (селектор offers A1/A2/B1/B2/C1). **Добавление уровня CEFR (чеклист):** ветка в `detectLanguageFormat` + `LanguageFormat`-тип + METHODOLOGY/CRITERIA + switch-cases + `buildResponseLanguageInstruction` (A1/A2 → русский в auto: `useTarget` их исключает) + `CefrLevel`-тип (`types.ts` + `cefrDetect.ts`) + оба детектора (regex) + оба CHECK (`homework_tutor_tasks` + `kb_tasks`, отдельная миграция) + `VALID_CEFR_LEVELS`/`normalizeCefrLevel`/`CEFR_LEVELS`(kb_snapshot) + все type-литералы `'A2'|'B1'|'B2'|'C1'` (grep) + `<option>` селектора + prefill-гарды + smoke `test-criteria-templates.mjs` (sum-row + forces-rubric + auto-язык).
...
- **Column-GRANT whitelist на `homework_tutor_tasks` (миграция `20260630170000`, КРИТИЧНО) — единственная защита от ПРЯМОГО PostgREST.** Edge column-whitelist защищает только edge-ответы; RLS-политика «HW students select tasks of assigned assignments» (`20260217110038`) даёт ученику row-SELECT его ДЗ, а RLS фильтрует строки, не колонки → без column-GRANT ученик читал бы `solution_text`/`correct_answer`/`ai_reference_solution`/`rubric_*`/`grading_criteria_json`/`ocr_text`/`solution_steps` из консоли (тот же класс, что закрытая дыра `mock_exam_variant_tasks`, rule 45). Фикс — `REVOKE SELECT FROM anon, authenticated` + `GRANT SELECT` только безопасных (id, assignment_id, order_num, task_text, task_image_url, max_score, check_format, task_kind, kim_number, cefr_level); зеркало `homework_tutor_task_states` (`20260516120100`). Tutor/student edge читают через service_role (bypass GRANT). **Инвариант: новая tutor-only колонка → НЕ гранить `authenticated`; новая student-safe → добавить в GRANT явной миграцией. `*`-select для authenticated заблокирован навсегда.** `ai_reference_solution` (AI-эталон, Phase 3/Phase A) — tutor-only, в whitelist НЕ входит.
...
**При расширении:** новое scoring/review-действие на `task_states` → новая SECURITY DEFINER RPC (multi-query НЕ воспроизводить); review-флаг — explicit-orthogonal (не выводить из status); bulk-count на фронте = RPC WHERE; manual/no-AI grade → персистить override даже при значении `0` (`aiScore == null` → always-override). **«Проверено»-агрегаты (бейдж/очередь/роллап) считают force-close проверкой** — `isTaskScoreReviewed` (2026-07-20), см. секцию «Требует проверки»; RPC при этом НЕ расширены.
...
**Промоушен шаблона в ССЫЛОЧНЫЙ режим — только при полном контентном паритете (ревью ChatGPT-5.6, 2026-07-23; НЕ откатывать).** После `tasks_migrated_at` GET синтезирует задачи из ЖИВЫХ строк Базы и **игнорирует** сохранённый `tasks_json` (он остаётся audit'ом), поэтому `handleCreateTemplateFromAssignment` ставит маркер только когда выполнены ВСЕ три гейта:
1. **Нет повторов `kb_task_id`** — junction имеет UNIQUE `(template_id, kb_task_id)`, дедуп схлопнул бы ДЗ из N задач в шаблон из N−k (две одинаковые задачи получают один kb-id по fingerprint — кейс реальный).
2. **`templateTaskContentEquals(kbTaskToTemplateTaskJson(kb), snapshot)` для КАЖДОЙ задачи** (`kb_snapshot.ts`, единственная точка сравнения двух форм). Ловит: правку KB-задачи в конструкторе без «Обновить в Базе» (в ДЗ новая версия, в Базе старая); округление дробного `max_score` при зеркалировании в `primary_score`; **выключенные `include_rubric` / `include_ai_settings`** — снимок их занулил/опустил, а синтез вернул бы из Базы, нарушая контракт toggle'ов диалога.
3. **Маркер `tasks_migrated_at` пишется с проверкой ошибки**, и при её сбое junction-строки **откатываются** — иначе остаётся partial-state (junction есть, маркера нет), в котором счётчик задач задваивался.

Сравнение КОНСЕРВАТИВНО: ложное «не равно» безопасно (остаёмся на точном legacy-снимке), ложное «равно» — нет. Провенанс (`source_kb_task_id`) из сравнения исключён (у авто-зеркала id появляется только после INSERT'а). Тест — `scripts/test-template-ref-parity.mjs` (smoke §18). Новое поле в `kbTaskToTemplateTaskJson` попадает в сравнение автоматически.

**Сам промоушен — АТОМАРНАЯ RPC `hw_template_materialize_refs(_template_id,_tutor_id,_kb_task_ids)`** (миграция `20260723180000`): junction-строки + `tasks_migrated_at` одной транзакцией, ownership + `FOR UPDATE` внутри, `sort_order` из `WITH ORDINALITY`. Прежний multi-query flow (INSERT → UPDATE → компенсирующий DELETE) при сбое И маркера, И отката оставлял orphan junction-строки: они через `ON DELETE RESTRICT` блокировали удаление задачи из Базы («используется в шаблоне» без единого шаблона, который её показывает). **Новый transactional action на шаблонах → своя RPC, multi-query не воспроизводить** (rule 40-конвенция, зеркало `hw_tutor_force_complete_task`).

**`hw_template_task_counts` — ВЗАИМОИСКЛЮЧАЮЩИЙ `CASE`** (migrated → junction, legacy → `jsonb_array_length`; миграция `20260723160000`): складывать нельзя, иначе partial-state даёт ~2N. Сбой RPC → `task_count: null` (клиент рисует «—»), **никогда не выдуманный 0**; точечный fallback тянет `tasks_json` только для непосчитанных id.

**Идемпотентность сохранения (`creation_request_id`, миграция `20260723170000`):** клиент генерит ключ ОДИН раз на попытку и шлёт неизменным во всех ретраях; UNIQUE `(tutor_id, creation_request_id)`; edge при повторе возвращает уже созданный шаблон (pre-check + обработка `23505` на гонке). Уникальность по `assignment_id` **запрещена** — post-factum несколько шаблонов из одного ДЗ легальны (AC-16). Генератор — `src/lib/clientUuid.ts::generateClientUuid` (Safari 15 не имеет `crypto.randomUUID`, rule 80; чат-овый `generateClientMsgId` — алиас на него, не копия).

**Ключ обязаны слать ОБА клиента** (ревью 5.6 р.2): чекбокс конструктора (`TutorHomeworkCreate`, ключ на submit) И post-factum `SaveAsTemplateDialog` (ключ на открытие диалога, живёт через ретраи). Без ключа строка получает `creation_request_id = NULL`, а partial-UNIQUE такие строки не покрывает → «201 отправлен, ответ не дошёл» + «Сохранить» ещё раз = дубль. **В диалоге ключ ПЕРЕСОЗДАЁТСЯ, если тьютор изменил payload** (название/теги/тумблеры) между попытками — иначе сервер вернул бы первый шаблон со старым названием и правки молча потерялись бы (ловушка наивной идемпотентности). Новый клиент save-as-template → тот же паттерн.

**Schema drift subject-CHECK — ЗАКРЫТ 2026-07-23 (инцидент, читать перед правкой словаря предметов).** `homework_tutor_templates_subject_check` два месяца оставался легаси-списком (`math, physics, history, social, english, cs`), потому что миграция `20260525120000_unify_homework_templates_subject_check.sql` **лежала в репо, но Lovable молча пропустил её при синке** (в `supabase_migrations.schema_migrations` нет ни одной записи с этим constraint'ом; соседние миграции того дня применены). Симптом: INSERT шаблона с subject вне легаси-списка → `23514 check_violation` → generic 500 → тихий toast «Не удалось сохранить как шаблон». Ломались **все 3 пути** (чекбокс конструктора, диалог на карточке ДЗ, форк из Банка) у 13 репетиторов: french 22 ДЗ/3 тьютора, chemistry 19/3 (репорт Ульяны), maths 14/7, russian 5/2. Нашли только по жалобе — два месяца.

Что сделано (**не откатывать**):
- Миграция `20260723150000_fix_homework_templates_subject_check_retry.sql` — повторное применение (старый файл не трогаем, правка применённых миграций запрещена) + применена вручную через Lovable MCP с верификацией `pg_constraint`.
- **Гард `17. Subject CHECK parity` в `scripts/smoke-check.mjs`** (через `hw_subject_check_defs()`, той же миграции): сверяет оба CHECK между собой и покрытие `SUBJECTS` из `src/types/homework.ts`. Best-effort — без креды warn+skip. **Это единственная защита от класса «миграция в репо ≠ применена»** (rule 50/95). Прогонять с `VITE_SUPABASE_URL=https://api.sokratai.ru` + publishable-ключом перед релизом, трогающим словарь предметов.
- **`templateWriteError()`** в `homework-api/index.ts` — единый маппинг ошибок записи шаблона (`23514 → 409 CHECK_VIOLATION` с русской фразой, rule 97) во **всех** write-сайтах: `POST /templates` (обе ветки refs/legacy), `/templates/:id/fork`, `/assignments/:id/save-as-template`. Новый write в `homework_tutor_templates` → через него, не голый `jsonError(500)`.

**Инвариант остаётся:** добавляешь предмет в `SUBJECTS` → миграция расширяет **ОБА** CHECK (`homework_tutor_assignments` + `homework_tutor_templates`) одновременно; гард §17 упадёт, если забудешь.
...
- **Save-path — ОДИН, серверный (консолидировано 2026-07-23).** Оба UI-входа (**чекбокс «Сохранить как шаблон»** в конструкторе и **диалог `SaveAsTemplateDialog`** на карточке ДЗ) идут в `POST /assignments/:id/save-as-template` → `handleCreateTemplateFromAssignment`, который читает задачи **из БД** по `assignment_id`. Клиентский `createTutorHomeworkTemplate` из конструктора **удалён**: он вторым `tasks.map` пересобирал payload вручную и уже дал P0 field-parity 2026-06-03, терял `source_kb_task_id` (шаблон не становился ссылочным) и писал пустой `task_text` вместо `[Задача на фото]`. Чекбокс дожидается создания ДЗ (фаза шаблона — последняя) и шлёт `{include_rubric:true, include_materials:false, include_ai_settings:true}`; сбой не фатален (ДЗ уже выдано) → `toast.error` + action «Повторить» (паттерн `attachToLesson`). **НЕ `save_as_template`-флаг** — тот в `handleCreateAssignment` живёт, но UI его НЕ шлёт (мёртвый путь, оставлен defensive). Legacy `POST /templates` (`handleCreateTemplate`) endpoint СОХРАНЁН ради deploy-skew (старые бандлы / stale PWA) + нормализует AI-поля server-side (review P1-3: валидный `check_format`, `resolveWriteTaskKind`, cefr lang-only), но **новых клиентов туда не вести** — иначе дубль write-path и класс «шаблон потерял поле X» возвращаются.
...
**Build-recipe — структура токенов (2026-06-19, important при добавлении buildable-формул):** карточка «Собери формулу» имеет ОДИН числитель + ОДИН знаменатель и различает варианты по **instance-key** (`BuildFormulaCard`), а `generateBuildFormula` **НЕ дедупит** `options` (distractors уже uniq + не пересекаются с correct) → формула с повторяющимся оператором (`A + B + C` нуждается в ДВУХ `+`) собирается корректно. Дробное слагаемое в сумме (`A + \frac{C}{2}`) задаётся **единым композитным токеном** `\\frac{C}{2}` (один чип, `denominatorTokens` пуст), НЕ раскладкой по общему знаменателю — иначе сборщик дал бы `(A+C)/2`. Сгруппированный числитель (`\frac{(v+v_0)t}{2}`) — композитный токен `(v + v_0)`. Образцы: `kin.25_e` (√ как чип), `kin.66/68/69_e`, `kin.28/34_e`. `areTokenListsEquivalent` — алгебраическое равенство через **знаковые слагаемые** (multiset, 2026-06-19): сложение/вычитание КОММУТАТИВНЫ (`x_0 + v_x t` = `v_x t + x_0`, `A + B − C` = `−C + B + A`), но `A − B ≠ B − A` (знак каждого слагаемого сохраняется); множители внутри слагаемого — bag (`2π R` = `R 2π`); числитель и знаменатель сравниваются отдельно (структура дроби не теряется). Новый buildable-рецепт с суммой — НЕ требует фиксированного порядка слагаемых.

**Layout раунда (`FormulaRoundScreen`, 2026-06-19):** область вопроса = ДВА слоя — внешний `flex-1 overflow-y-auto` (только скролл) + внутренний `min-h-full flex justify-center` (центрирование). НЕ `flex justify-center` + `overflow-y-auto` на ОДНОМ элементе: высокая карточка (BuildFormula с раскрытыми панелями) тогда центрируется так, что её верх уезжает под шапку с прогрессом и недоступен скроллом (rule 80, классический flexbox-баг). `min-h-full`: короткая карточка центрируется, высокая растёт и скроллится сверху.

**Фидбэк TrueOrFalse — «формула» vs «ответ» (2026-06-19):** на «Формула верна?» вердикт-шапка `FeedbackOverlay` = про ОТВЕТ ученика («✓ Правильно!» / «✗ Ты ошибся»), а верность САМОЙ показанной формулы — отдельным зелёным/красным баннером «Эта формула верна/неверна». Сигнал `FeedbackPayload.formulaShownIsCorrect` (= `question.correctAnswer` для layer 3 TrueOrFalse, иначе `null` → баннера нет для Build/Situation) прокидывается через `FormulaRoundScreen`. Иначе ученик путал «формула неверна» с «ответ неверный» (фидбэк Елены/Vladimir: красное «Неверно» сверху рядом с «Твой ответ: неверно»).
...
**Т8 — бейджи вида работы (2026-07-25, запрос репетиторов «или нужно будет прописывать в названии?»):** единый чип `src/components/homework/WorkModeChip.tsx` (нейтральный, rule 90) — рендерится **ТОЛЬКО для `independent`** («домашка с Сократом» = дефолт, чип на ней шум). Поверхности: список ДЗ репетитора (`AssignmentCard`, `work_mode` в `handleListAssignments` есть с `cedc920`) · шапка деталки (`ResultsHeader`) · список ДЗ ученика (`StudentHomework` — потребовал добавить `work_mode` во **вложенный PostgREST-select** `listStudentAssignments`, оно не приходит автоматически) · карточка занятия (`LessonDetail`, поле добавлено в `HwInfo` + `loadHomeworkInfo` в `student-lessons-api`) · текст уведомления о назначении (**ОБА** канала: `notifyHomeworkStudentAssigned` для quick-assign и `defaultMessage`/`pushPayload` в bulk `/notify`; кастомный `messageTemplate` репетитора НЕ трогаем) · отчёт родителю (`work_mode` в `student-progress-build` + два блока в `PublicStudentReport`, самостоятельные первыми — «родителям интереснее самостоятельный результат»). Anti-leak §5 не нарушен: это вид работы, а не подсказки/решения. ⚠️ До 2026-08-05 whitelist edge СРЕЗАЛ `work_mode`/`independence_pct` из публичного отчёта — фронт был готов, поля не доезжали («разрыв фичи»). Метрика самостоятельности в отчёте — с 2026-08-05, см. блок «Отчёт родителю» ниже.

**При расширении:** новый write-path `work_mode` → create/update/list/шаблоны (включая явный column-list форка) + student-whitelist'ы; новый AI-путь → 403-гейт ДО квоты; новый способ закрыть задачу → писать балл явно + CAS; новая поверхность со списком работ → чип через `WorkModeChip` (не свой бейдж), а вложенный select — проверить, что `work_mode` в него включён; Т7 («% самостоятельности»), Т9 (настройка момента разбора) и Фаза 2 (сводка ошибок) — отложены, см. spec.md §10.

### «% самостоятельности» + смерть деградации балла (Т7, 2026-07-25)

Модель утверждена владельцем: штрафуем не балл, а отдельную метрику процесса. Миграции `20260725140000` (колонки + `hw_bump_ai_help_events`) и `20260725150000` (два SQL-дубля цепочки балла).

- **Формула — ТОЛЬКО `_shared/score-compute.ts`** (`computeIndependencePct` / `aggregateIndependencePct`): `100% − 10 п.п. × ai_help_events`, floor 0; агрегат по работе — **средневзвешенный по `max_score`** задач (сложные весят больше). Не дублировать в UI: разойдётся на первом изменении шкалы.
- **Обращение к помощи AI = 3 точки инкремента** (`bumpAiHelpEvents`, атомарная RPC): разбор ошибки (`ON_TRACK` и `INCORRECT` — одна сдача = ОДИН минус), подсказка по кнопке, ответ Сократа в обсуждении (`handlePostThreadMessage`, `role='assistant' && kind='ai_reply' && status='active'`). НЕ считаются: `CORRECT`, авто-вступление (`kind='system'`), `CHECK_FAILED`, реплики живого репетитора, системный low-confidence downgrade CORRECT→ON_TRACK (это решение системы, не помощь).
- **N нельзя считать SQL-каунтом по `homework_tutor_thread_messages`** — сервер пишет и вердикты, и подсказки как `kind='ai_reply'` → счёт даст 2–3×. Только код.
- **`ai_help_events` NULLable без DEFAULT.** NULL = «данных нет» (работа до релиза / массовый force-complete) → в UI «—» и исключение из знаменателя агрегата. `DEFAULT 0` соврал бы «100% самостоятельно» на всей истории. Ветки, закрывающие задачу (CORRECT / independent), материализуют `COALESCE(prev,0)`.
- **Балл задачи = ЛУЧШИЙ за все попытки** (`best_earned_score numeric(5,2)`, в `computeFinalScore` ВЫШЕ `earned_score`). Fail-safe: колонка, забытая в SELECT → `undefined` → прежнее поведение, но поверхности разойдутся, поэтому она добавлена в **7 SELECT-листов** (4 в `homework-api` + `THREAD_SELECT`, `_shared/student-progress-build.ts`, `student-lessons-api`, `admin-homework`) и в **оба SQL-дубля** (`hw_tutor_force_complete_task`, `hw_tutor_review_task`). Ручной балл репетитора в `best_earned_score` НЕ пишется — иначе override «липнет» после сброса.
- **Деградация балла ОТМЕНЕНА** (`computeAvailableScore` удалён, надгробие в `guided_ai.ts`): `available_score` = `max_score`. Убран и хак «3-й+ ON_TRACK считаем подсказкой» — `hint_count` теперь означает буквально «нажал кнопку». Два штрафа за одно и то же не возвращать.
- **P0 anti-leak:** `stripIndependentPreRevealFields` зануляет `best_earned_score` и `ai_help_events` — иначе балл самостоятельной утечёт ученику до сдачи.
- **Видимость:** репетитор — колонка «Самост.» в heatmap + строка в разборе + `EditScoreDialog` («Помощь AI: N → самостоятельность NN%»; для legacy-строк остаётся прежний текст про деградацию). Ученик — раскрываемый чип на экране задачи и строка в «Результате работы» (решение владельца: метрика **открыта**, «мы именно его мотивируем думать головой»). ⚠️ **При `work_mode='independent'` колонку НЕ СКРЫВАТЬ, а показывать пометку «без AI»** (`results.work_mode` в корне ответа; per-task вторая строка в ячейках при этом не рисуется). Первоначальное тихое скрытие читалось как поломка — репорт Елены 2026-08-05 «Самостоятельность сломалась» на её ПЕРВОЙ самостоятельной работе, при идеальных данных в БД. Тот же принцип, что «Объяснение метрики ОБЯЗАНО быть видимым»: отсутствие цифры тоже надо объяснять на экране, а не скрытием.
- **Объяснение метрики ОБЯЗАНО быть видимым, а не только в `title`** (ревью-фикс P1, 2026-07-25). `title` не показывается на телефоне и планшете — то есть у ВСЕХ учеников и у репетитора с iPad; без него «Самост. 70%» рядом с «подсказок: 1» читается как произвольное число или двойной штраф. Сейчас: у ученика чип — `<button aria-expanded>`, тап раскрывает абзац с формулой; на экране результата формула видимой подстрокой; у репетитора — легенда в шапке блока результатов (включая расшифровку «—» = нет данных). Новая поверхность с метрикой → тот же принцип, `INDEPENDENCE_TOOLTIP` остаётся вспомогательным для мыши. Терминология единая: **«Самостоятельность»** там, где есть место, **«Самост.»** в тесных (никаких «Сам-но»).
- **Тексты сбоя автопроверки начинаются с «это технический сбой, а не оценка твоего решения»** и НЕ упоминают списание баллов (с отменой деградации это и неверно, и само наводит на мысль об оценке). Меняешь текст → правь `isCheckFailedFeedbackText` (детект второго сбоя подряд идёт по началу строки; legacy-префиксы там оставлены для строк, записанных до 2026-07-25).
- **Счётчик инкрементится ПАРАЛЛЕЛЬНО с UPDATE балла** (`Promise.all`), а не после: ученик и так ждал AI, добавлять ему round-trip нельзя. Fire-and-forget нельзя — клиент сразу инвалидирует problem-query и получил бы старый процент.
- **Отчёт родителю (РЕШЕНИЕ ПРИНЯТО 2026-08-05, отменяет прежнее «не тащим»):** метрика в отчёте ЕСТЬ — агрегат «Самостоятельность N%» в основных метриках (нейтральная подпись + видимое пояснение «подсказки — часть обучения») и per-work % в свёрнутом списке работ. **Родителю только ТОЧНЫЕ значения**: сервер (`public-student-report/report_shape.ts`) отдаёт `independence_pct: null` при `independence_is_estimate` — «≈»-оценки остаются репетитору, решение на сервере, не на фронте. Агрегат — среднее точных per-work по обычным домашкам (без independent и пробников), по ПОЛНОМУ списку периода, не по капу 10. `summary`/`target` там же переведены на **явные whitelist'ы** — новое внутреннее поле в `StudentProgressPayload` публичным автоматически НЕ становится (канарейка-тест в `report_shape.test.ts`).
- **Известная неточность:** ответ AI в обсуждении персистит КЛИЕНТ (`HomeworkProblem.tsx`) — потеря запроса недосчитает N и завысит самостоятельность. Лечится переносом persist в edge `chat`, вне скоупа.

**⚠️ P0-инцидент 2026-07-27 (репорты Елены и Егора) — `handleGetResults` был ВОСЬМЫМ SELECT-листом.** Метрику правили в 7 местах, а тот SELECT, что кормит таблицу результатов репетитора (`homework-api/index.ts`, внутри `handleGetResults`), пропустили. Симптом: `independence_pct = null` И по каждой задаче, И в агрегате — репетитор видит «—» там, где помощь фактически была, хотя `ai_help_events` в БД записан верно. Вторым, незаметным следствием того же пропуска цепочка `computeFinalScore` в этой функции не видела `best_earned_score` → балл в таблице разошёлся бы с экраном ученика при повторной, менее удачной сдаче. **Гард: smoke-check §22** — любой `.select(...)` по `homework_tutor_task_states`, тянущий `earned_score`, обязан тянуть и `best_earned_score`; исключения только через явный allow-list в самой проверке. Диагностика такого класса: сверить БД (`select ai_help_events … from homework_tutor_task_states`) с payload'ом эндпоинта — расхождение всегда означает забытую колонку в SELECT, а не ошибку расчёта.

**«≈»-оценка для работ до 2026-07-26** (решение владельца: «—» на всех старых работах читалось как «метрика не работает»). Развилка ЕДИНСТВЕННАЯ — `resolveIndependencePct` в `_shared/score-compute.ts`: есть `ai_help_events` → точный расчёт; нет, но есть активность → `estimateIndependencePctLegacy(hint_count, wrong_answer_count)`; активности нет → `null` («—»). Флаг `independence_is_estimate` едет во всех ВНУТРЕННИХ ответах (results / student problem / student result / scores), и UI обязан рисовать «≈» (исключение — публичный отчёт родителю: там «≈»-значения режутся СЕРВЕРОМ в null, см. блок «Отчёт родителю» выше): **оценка завышена** — ответы Сократа в обсуждении в legacy-полях не сохранялись. Агрегат помечается оценочным, если оценочна хотя бы одна вошедшая задача. Новый эндпоинт с метрикой → только через `resolveIndependencePct`, иначе одна поверхность покажет «—», а другая «≈90%» по тем же данным.

**Где метрика видна (2026-07-27, запрос Егора «по каждой задаче 2 метрики у ученика и учителя»):** ячейка heatmap — две строки (балл 14px + процент 10px в существующие 44px, ширина 56px и высота строки НЕ меняются) · мини-карточка разбора — третья центральная строка (четыре угла заняты, вертикальный люфт ≈29px) · итоговая колонка «Самост.» · строка под мини-карточками · `EditScoreDialog` · у ученика — чип на экране задачи, «Результат работы» и **список ДЗ** (одна строка «7/7 · самостоятельность 96%» только у завершённых; итоги грузит **отдельный** роут `GET /student/assignments/scores` — балл считает сервер, формулу на клиенте не дублируем) · **отчёт родителю** — агрегат по работе (без разбивки по задачам; anti-leak-барьер `student-progress-build` соблюдён: `hint_count` и тексты там не отдаются наружу, только агрегированный процент).

**`HwTaskStateRow` в `_shared/student-progress-build.ts` — единый тип строки task_states** (возвращаемый тип загрузчика + внутренняя Map + локальный алиас). Раньше это были три копии инлайн-типа, и они уже разошлись по `best_earned_score`. Deno-код локальным `tsc` не проверяется (esbuild типы не смотрит), поэтому дрейф всплывает только на деплое — новое поле добавлять В ЭТОТ тип.

### Сбой автопроверки — `message_kind='check_failed'` (T0, 2026-07-25)

Репорт Ульяны: ученица дважды получила «Автопроверка не сработала… попробуй ещё раз или перейди в режим „Обсудить“» и приняла это за оценку своего ответа. Диагноз по `hw_ai_check_events`: 18 `CHECK_FAILED` подряд с `failure_reason='gateway_error'`, latency 220–550 мс — мгновенный отказ **AI-шлюза** (исчерпан лимит AI-кредитов Lovable), к релизу отношения не имел. Мониторинг расхода → rule 101.

- **Текст выводится ИЗ ПРИЧИНЫ, единственный конструктор — `buildCheckFallback(reason)`** (`guided_ai.ts`). Не собирать `{ ...CHECK_FALLBACK, failure_reason }` руками (объект удалён): при мёртвом шлюзе оба прежних совета вредны — повтор даёт ту же ошибку, а обсуждение идёт через ТОТ ЖЕ шлюз и в самостоятельной работе его нет вовсе. `gateway_error`/`timeout`/`empty_model_response` → «Сократ недоступен, это на нашей стороне, ответ сохранён»; остальное (`invalid_json` и пр.) → «попробуй отправить ещё раз».
- **Сбой пишется отдельным `message_kind='check_failed'`** (миграция `20260725120000` расширила CHECK), а НЕ `feedbackKind`. Клиент (`GuidedChatMessage`) рендерит его нейтральной **системной плашкой** без аватара/подписи AI — на обеих perspective. Deploy-skew безопасен: старый бандл отрисует обычной репликой.
- **Второй сбой подряд** по задаче добавляет «мы уже знаем о проблеме» — детект без новой колонки: `isCheckFailedFeedbackText(task_states.last_ai_feedback)`. Правишь текст сбоя → правь и этот детектор (матч по началу строки).
- **Сигнал репетитору — новый `kind='check_failed'` в ленте «Последние действия»** (`handleGetRecentDialogs`, приоритет ВЫШЕ `submitted`: сдал, но ждёт РУЧНОЙ проверки; чип «Проверьте вручную», янтарный — виноват шлюз, не ученик). Детект: последнее assistant-сообщение треда среди `{check_failed, check_result, ai_reply}` имеет kind `check_failed`. **Очередь «Требует проверки» намеренно НЕ меняли** — она про подтверждение баллов и требует `completed`-тред, а сбой оставляет задачу `active`. Бейдж «N ответов ждут» в деталке ДЗ — отложен (лента сильнее: репетитор смотрит главную).
...
- **Таблица `homework_folders`:** `tutor_id → auth.users(id)` (FK-дрейф — как assignments, НЕ `tutors.id`), `parent_id` self-FK CASCADE (**вложенность ВКЛЮЧЕНА 2026-07-20**, см. секцию «Вложенные папки» ниже), `name`, `sort_order`. RLS `tutor_id=auth.uid()` → CRUD папок прямым PostgREST (`src/lib/tutorHomeworkFoldersApi.ts` + `src/hooks/useHomeworkFolders.ts`, ключ `['tutor','homework','folders']`).
...
- **Конструктор (обновлено 2026-07-13, фикс Елены):** селектор «Папка» = **create-only** (стейт `createFolderId`, НЕ в `meta` → edit-snapshot/dirty не задета). **Вынесен из «Расширенных» на L0** (всегда виден). `FolderCard` обобщён (`taskWord`/`showChildCount`).
  - **Создание со страницы папки** (`HomeworkFolderPage` «Создать ДЗ») ведёт на `/tutor/homework/create?folder=<id>`; конструктор lazy-init `createFolderId` из `searchParams.get('folder')` **синхронно (не эффект — без клоббера prefill)**. Backend `validateOwnedFolderId` отклоняет чужой/битый id — client-side НЕ чистить (скрыло бы потерю явно выбранной папки). Раньше кнопка вела на `/create` без папки → ДЗ уходило в «Без папки».
  - **`createFolderId` ОБЯЗАН быть в deps submit-`useCallback`** — иначе выбор папки последним действием перед «Создать» захватывается устаревшим замыканием (папка не сохранится). Это стрельнуло именно после выноса селектора на L0 (стал часто использоваться последним).
- **Перенос ДЗ в папку** (kebab карточки → `MoveHomeworkAssignmentToFolderModal` — кастомная модалка, НЕ Radix Dialog): `DropdownMenuItem.onSelect`, открывающий кастомный оверлей, **НЕ должен звать `e.preventDefault()`** — иначе Radix-меню остаётся открытым в modal-режиме (`pointer-events:none` на body), и первый тап по модалке съедается закрытием меню («выбор папки только со 2-го клика», баг Елены 2026-07-13). Открывать модалку через `requestAnimationFrame` (после закрытия меню). Пункт меню в `Portal` → навигацию родительского `<Link>` не триггерит.
- **При расширении:** новый write-path к `folder_id` → через edge + триггер уже покрывает; новый surface со списком ДЗ → реюз `folder_id` из ответа, не дублируй серверный фильтр; селектор папки в конструкторе — L0 + create-only + **в deps submit-колбэка**; kebab→кастомная модалка — без `preventDefault` в `onSelect` + rAF.

Build-лог: memory `project_elena_requests_2026_06_17.md`. План: `~/.claude/plans/1-linked-treehouse.md`.

### Вложенные папки ДЗ + reparent + drag&drop (запрос Елены, 2026-07-20)

Подпапки «по аналогии с базой задач» (у Елены ~100 ДЗ/год), глубина без лимита, перенос существующих папок, DnD. Миграция `20260720130000` (только guard-триггер — колонка `parent_id` была с 20260617120000).

- **Guard-триггер `hw_folder_parent_guard`** (BEFORE INSERT OR UPDATE OF parent_id): (1) ownership родителя через `homework_folder_owned_by` — RLS parent_id НЕ проверяет, INSERT-гейт обязателен; (2) anti-cycle walk-up (depth cap 50). RAISE `check_violation` с маркерами `cycle`/`does not belong`/`too deep` → `mapFolderGuardError` в `tutorHomeworkFoldersApi.ts` мапит на рус. фразы (rule 97). Reparent = прямой PostgREST UPDATE (`moveHomeworkFolder`) под RLS + триггер-backstop; **клиентский cycle-гард — `collectDescendantIds`** (модалка disable'ит self+поддерево, DnD — no-op+toast).
- **Дерево-утилиты — `src/lib/homeworkFolderTree.ts`** (pure, локальное зеркало KB buildTree — тот module-private/KB-typed): `buildHomeworkFolderTree` (orphaned parent → root), `collectDescendantIds` (ВКЛЮЧАЯ root), `countDirectChildren`, `recursiveAssignmentCounts`, `flattenTreeWithDepth` (для `<option>` с `— `-префиксами), `buildFolderBreadcrumbs`. **Счётчики КЛИЕНТСКИЕ осознанно** (в отличие от KB-RPC `kb_folder_recursive_counts`): `/tutor/homework` и так грузит полный список assignments; папок мало. Семантика KB: задания РЕКУРСИВНО, подпапки ПРЯМЫЕ.
- **Удаление НЕ изменилось (КРИТИЧНО):** подпапки каскадятся (`parent_id ON DELETE CASCADE`), задания ВСЕГО поддерева → SET NULL «Без папки» (каждая каскадная папка SET-NULL'ит свои). `DeleteHomeworkFolderDialog` показывает subtree-счётчики (`subfolderCount` + subtree `assignmentCount`).
- **UI:** `/tutor/homework` — только `rootFolders` (`showChildCount=true`); `HomeworkFolderPage` — крошки (client parent-chain) + секция «Подпапки» + кнопки «Подпапка»/«Переместить»; delete → navigate к родителю; `FolderCard` получил additive `onMove?` (FolderInput; KB не передаёт → ноль изменений KB); `MoveHomeworkFolderModal` (reparent, клон shell'а move-модалки) + move-модалка ДЗ рендерит дерево с отступами; фильтр пикера `LessonMaterialsPanel` — **subtree-inclusive**; селект конструктора — `— `-отступы (инварианты `createFolderId` не тронуты).
- **DnD (`homeworkDnd.tsx`) — DESKTOP-энхансмент:** native HTML5, MIME `application/x-sokrat-hw`, payload `{type:'assignment'|'folder', id}`; module-level `currentDrag` (protected-mode: `getData` недоступен в dragover); `HwDraggable` (didDrag-гард на onClickCapture — карточка ДЗ = `<Link>`) + `HwFolderDropZone` (enter/leave counter → ring на валидной цели; `preventDefault` на dragover обязателен). Wiring: карточки папок/ДЗ обеих страниц + крошки. **iOS/touch: DnD не работает (rule 80) — канон остаётся kebab/иконки-модалки, НЕ убирать их.**
- **При расширении:** любой новый write parent_id → триггер уже покрывает, но клиентский гард (`collectDescendantIds`) обязателен в UI; новый счётчик/итератор папок → tree-утилиты, не ad-hoc обход; крошки/subtree — из `homeworkFolderTree.ts`.
...
- **Единый source of truth «полностью проверено»** — `src/lib/homeworkReview.ts::isStudentWorkFullyReviewed(allTasks, taskScores)`: каждая задача ДЗ имеет **проверенный** task_score. **«Проверено» для задачи = `isTaskScoreReviewed(ts)` = `tutor_reviewed_at != null ИЛИ tutor_force_completed_at != null`** (решение владельца 2026-07-20, «force-close = проверено»): force-закрытая задача (`ai_score=NULL`) — уже принятое решение репетитора; раньше она была невидима bulk-RPC (`ai_score IS NOT NULL`) и per-task кнопке → бейдж блокировался НАВСЕГДА, ДЗ «висело» в «N на проверку» (повторный репорт Елены 2026-07-20). **Пустой `allTasks` (ДЗ без задач, edge) → `true`** (vacuously — нечего проверять, иначе висит вечно). Mirror условия в Deno-роллапе (`handleListAssignments` — SELECT несёт оба поля) и в дешёвом чеке очереди. **RPC `hw_tutor_review_all_ai`/`reviewableCount` НЕ расширены** (bulk-count строго = RPC WHERE — инвариант ниже); гейт быстрой кнопки «Подтвердить» в `GuidedThreadViewer` дополнен `!isTaskScoreReviewed` (иначе чип «Проверено» и кнопка рендерились бы одновременно у ai_score+force-closed задачи).
...
## Фото ученика: просмотр, поворот, разметка — 2026-07-31

Джоб Ульяны (химия, пилот): **U21** «показать ученику, ГДЕ он ошибся, прямо на
его фото» и **U22** «его нельзя развернуть, приходится копировать». Тот же
запрос у Эмилии (J5) и Дианы. Спека — `docs/delivery/features/photo-annotate/spec.md`.
Инварианты, нарушаемые первой правкой, — в rule 40.

### Почему появился единый просмотрщик

Разведка показала, что канонического вьюера фото в кабинете не было вообще:
...
`AdminHWThreadView`), **6+ мест, где «увеличить» = сырая вкладка браузера**
(тред ДЗ, пробники ×5, Банк ×3), и HEIC-восстановление ровно на **2**
поверхностях из ~15. Ни одна копия не умела зум, панораму и поворот.

`PhotoGallery` и `FullscreenImageCarousel` сохранили контракт props и стали
тонкими обёртками — существующие вызовы не правились.

### Поворот: почему таблица, а не перезапись файла

Владелец спрашивал явно. Разница не в объёме данных:
...
Единственный плюс перезаписи — AI видит правильно «из коробки» — закрывается
строкой в промпте грейдинга.

### Геометрия разметки

Координаты — пиксели НЕповёрнутого фото, поэтому поворот ПОСЛЕ разметки не
сдвигает пометки. Обратный поворот — целочисленной перестановкой
(`unrotateVector`), не через sin/cos: тригонометрия даёт 6.1e-17 вместо нуля и
координаты копят дрейф. Толщина пера и кегль — от диагонали снимка: на фото
4000 px линия в 3 px волосок, на скриншоте 600 px клякса.

### Растеризация — два прохода, не один SVG

Документ, загруженный как `<img>`, **не подтягивает внешние ресурсы**, поэтому
`<image href="https://…">` внутри него не отрисуется. Фото кладётся
`drawImage`, разметка — самостоятельным SVG поверх. Анти-taint через blob-URL
(иначе `toBlob` бросает SecurityError). Текст в растре рисуется общесистемным
шрифтом — у SVG-как-картинки нет доступа к шрифтам документа; подписи короткие,
это осознанный размен.

### Маркер и стрелка живут в модели ДОСКИ

`StrokeElement.opacity` и `ShapeKind='arrow'` добавлены в
`src/lib/whiteboard/model.ts`, поэтому доска закрыла ими **B5 и B6** своего
P1-бэклога тем же коммитом.

- Маркер — `fill-opacity`, а НЕ `opacity` узла: последний даёт тёмный шов на
  пересечении двух маркерных штрихов.
- Стрелка — один `<path>` с ОБВОДОЧНЫМ наконечником (залитый треугольник при
  толстом пере превращается в кляксу), ус ≤ трети древка.
- Ластик для линии и стрелки меряет расстояние до отрезка, а не до bbox.

### Ввод: политика Pencil / палец / ладонь

Карта указателей хранит **тип**, а не только координаты. Пинч — только когда
ОБА указателя `touch`; пока активно перо, касания игнорируются целиком (ладонь
приходит как `touch` и иначе отменяла бы штрих). `getCoalescedEvents` для пера —
длинные дуги перестают быть угловатыми. `setPointerCapture` в try: на
неактивном `pointerId` браузер бросает `NotFoundError`, и необработанное
исключение обрывало обработчик до старта жеста.

### Чему научил фикс-пасс ревью

- `onLoad` не ловит уже декодированную картинку (rule 80) — фото вылезало за
  сцену.
- `crypto.randomUUID` в `uploadTutorHomeworkTaskImage` (старый код) стал
  критическим, когда от него завязалась отправка разметки.
- HEIC надо декодировать и на ЭКСПОРТЕ: во вьюере его показывает `SafeImage`,
  поэтому репетитор видит фото и теряет работу только на последнем шаге.
- Черновик: закрытие обязано СОХРАНЯТЬ, выбрасывать — только явная «Отмена».
  Раньше оба выхода вели в discard, и обещание «труд не потеряется» не
  выполнялось нигде.
- Позиционное сопоставление ref↔url врёт (см. rule 40).
