...
- **`npm ci` БЕЗ `--prefer-offline`.** Флаг брал метаданные пакетов из кэша VPS **без ревалидации** — деплой падал на версии, которая в реестре есть, а в кэше ещё нет. Экономил ~40 сек и стоил трёх оборванных деплоев 27.07. Детерминизм версий держит лок, кэш тут ни при чём. Гард — smoke-check §21.

### Три оборванных деплоя 27.07 — два РАЗНЫХ диагноза под похожим симптомом

Все три упали на шаге 2/10 (`npm ci`), прод не тронут ни разу (валидация `dist` идёт до касания докрута — инвариант 5). **Различать по коду ошибки, а не по «снова npm ci»:**

| Код | Причина | Лечение |
|---|---|---|
| `EUSAGE` (`lock file ... does not satisfy`) | лок разошёлся с `package.json` | `rm package-lock.json && npm install --package-lock-only`, коммит лока. Гард — smoke-check §23 |
| `EUSAGE` + `Missing: <pkg>@<ver> from lock` | лок не кросс-платформенный: пакет с платформенными биндингами (`os`/`cpu` в optionalDependencies) записан только под ОС генерации | убрать источник (случай 27.07: `vitest@4` тянет свой `vite`, требующий `rolldown` с 16 биндингами → понижен до `^3.x`), затем проверить лок на Linux руками агента Lovable |
| `ETARGET` (`No matching version found for X@Y`, а версия в реестре ЕСТЬ) | устаревшие метаданные в npm-кэше машины, замаскированные `--prefer-offline` | `npm cache clean --force` на VPS + флаг убран из скрипта |

**Проверка лока на Linux (перед деплоем, если трогали deps):** попросить агента Lovable выполнить в проекте `npm ci --dry-run --no-audit --no-fund` и вернуть код выхода. Windows-прогон §23 этого НЕ покрывает. Заодно сверить `git diff` до/после — агент не должен переписать лок.
