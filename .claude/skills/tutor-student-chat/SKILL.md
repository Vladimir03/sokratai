...
- Контекст: 15 сообщений ASC с префиксом «Репетитор {имя}:»/«Ученик {имя}:». `callLovableJson` (буфер, не стрим — realtime доставит обоим), source `tutor_student_chat`. Промпт — **прямой помощник, не сократический** (репетитор присутствует). Сбой → assistant-строка с фразой ошибки.
- **Фото (2026-08-05, репорт Светланы Тихоновой).** Раньше в промпт уезжали только картинки ТРИГГЕРНОГО сообщения, а сообщение без подписи с фото выбрасывалось из контекста целиком (`if (!h.content?.trim()) continue`) — присланная тетрадь была невидима ДВАЖДЫ, и «@СократAI посмотри» отвечало вслепую. Теперь: фото-без-подписи входит как `[фото]`/`[N фото]`; отбор — до 2 из триггера, дальше добор из окна истории от свежих к старым, общий кап 3 (`AI_MAX_IMAGES`), дедуп по storage-ref; бакет только `tutor-chat-uploads`.
  - ⚠️ **Картинка привязывается к СВОЕМУ сообщению** (`msgIndexByRowId` → `attachImagePart`), не к хвосту: иначе решение ученика читается моделью как приложенное к сообщению репетитора. Типовая раскладка «условие от репетитора + решение от ученика» держится именно на этом.
  - Кап 3, а не 2, чтобы «условие + решение» пролезли, когда в самом упоминании уже есть своё фото. Держать низким: кэша у шлюза нет (см. `docs/delivery/features/ai-request-optimization/research.md`), картинки уезжают base64 в КАЖДОМ вызове.
...
- **Shared** (`src/components/chat/*`, `src/hooks/chat/*`, `src/lib/tutorStudentChatApi.ts`, `src/types/tutorStudentChat.ts`) — нейтральная зона, НЕ импортит tutor/student модули.
- **Ссылки кликабельны (запрос Елены 2026-07-13), канон — `src/lib/chatLinkify.ts`**: детект `http(s)://` + `www.*` + bare `sokratai.ru/…` (общий bare-domain запрещён — ложные срабатывания), БЕЗ lookbehind (граница слова проверяется кодом), трейлинг-пунктуация не в ссылке, email не матчится. Два потребителя: `HumanContent` (ChatBubble) — React-`<a>` без innerHTML (+ сосуществует с подсветкой @СократAI); `MathText` markdownLite-путь — `linkifyEscapedHtml` ПОСЛЕ `applyMarkdownLite` (URL-класс останавливается на `<`/`>` вставленных тегов; обратный порядок корёжил бы href) и построчно через `LINEBREAK_PLACEHOLDER` (иначе `_` плейсхолдера засасывается в URL). Наш домен → same-tab (PWA); внешние → `_blank` + `noopener noreferrer nofollow`. Кандидат валидируется ОДИН раз (`validateCandidate`): userinfo-трюк (`https://sokratai.ru@evil.example`) и bidi-control символы → остаются текстом; закрывающие скобки отрезаются по БАЛАНСУ пар. 44px hit-area для inline-ссылок в тексте НЕ делаем намеренно (типографика; Telegram-норма) — только `touch-manipulation`. Guided-ДЗ и AI-чат ученика НЕ линкифицируют bare-URL (осознанный скоуп владельца).
- **Свои пузыри БЕЗ аватара — Telegram-модель, осознанно** (решение владельца 2026-07-13, фидбэк Елены): аватар партнёра/автора рендерится только слева у чужих; репетитор свои сообщения без аватара видит НАМЕРЕННО — ученики его аватар видят (шапка + входящие, источник `tutors.avatar_url`, тот же что в ДЗ-диалогах). НЕ «чинить» добавлением своего аватара.
- **Аватар ученика** — `profiles.avatar_url`/`gender`, ставится на `/profile` (`StudentAvatarSection` + shared `src/components/common/AvatarUpload.tsx` + `src/lib/studentAvatarApi.ts`, бакет `avatars`, миграций нет); чат/группы/ДЗ читают его автоматически. `ConversationView` (prop `perspective`), `ChatBubble` (KaTeX **только** для assistant), `ChatComposer` (свой, НЕ реюз `ChatInput` — тот однофайловый + voice-флоу), `ConversationRow`, `ReadTicks`.
