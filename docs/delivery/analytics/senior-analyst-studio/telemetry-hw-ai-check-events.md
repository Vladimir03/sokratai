...
## Пробел покрытия `tutor_correction` — найден и починен (2026-08-04)

**Что было.** Событие писалось из ОДНОГО места (`homework-api::handleSetTutorScoreOverride`), а
write-path'ы размазаны по ДВУМ edge-функциям: массовое закрытие — в `homework-api`, «проверено» и
reopen — в `tutor-progress-api`. Замер 01.07–04.08: в БД **29** правок / **51** force-complete /
**538** «проверено» → в телеметрии **4** / **3** / **0** (≈15% реальных действий).

**Как починено.** Логирование перенесено на **DB-триггер** `trg_hw_log_tutor_correction`
(миграция `20260804120000`) — он ловит любой путь записи, включая будущие. Инструментирование
отдельных хендлеров убрано (иначе одиночная правка задваивалась). `check_completed` по-прежнему
пишет edge-функция: вердикт/уверенность нигде не персистятся, их знает только она.

Два предохранителя в триггере (не убирать): `WHEN` — срабатывает только на изменение тьюторских
полей (ученический поток не задет); `EXCEPTION WHEN OTHERS` — сбой аналитики не может уронить
сохранение балла репетитором.

**Статус:** код в `main`, ждёт применения миграции + редеплоя `homework-api` через Lovable.

**Для аналитика:**
- События триггера помечены `meta.source = 'db_trigger'` — по нему отделять их от 7 старых
  edge-событий (и от возможных дублей, если миграция применится раньше редеплоя).
- **До накопления данных** override-rate и review-coverage считать по ретроспективной выгрузке
  (Файл 1) — там поля персистентны за весь период. `check_completed` надёжен с 01.07.
- Появились новые `correction_kind`: `review`, `review_reopen` (раньше их не было вовсе).
