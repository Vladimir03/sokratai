...
| Что | Статус |
|---|---|
| **Т7** «% самостоятельности» в обычных ДЗ (агрегат + колонка + тултип-формула) | второй срез Ф1; модель расчёта утверждена владельцем 2026-07-25 (`100% − 10 п.п. × обращения к AI`, балл = лучший за все попытки, деградация балла отменяется, агрегат взвешенный по `max_score`) — план `~/.claude/plans/1-2-clever-wilkes.md` |
