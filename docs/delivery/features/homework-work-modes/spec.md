...
| Что | Статус |
|---|---|
| **Т7** «% самостоятельности» в обычных ДЗ (агрегат + колонка + тултип-формула) | второй срез Ф1 |
| **Т8** бейджи вида работы (списки репетитора/ученика, деталка, карточка занятия, уведомление о назначении) | второй срез Ф1; поле `work_mode` в list-SELECT'ы уже проброшено |
