...
>
> **Статус 2026-07-27 (четвёртый заход — P0 по репортам Елены и Егора):** Елена: «ученик решает с подсказкой, а у него 1/1 и самостоятельность прочерк»; Егор: «по каждой задаче должно быть 2 метрики у ученика и учителя». Корневая причина одна: `handleGetResults` не селектил `ai_help_events` / `best_earned_score` — данные в БД были верны (проверено по её работе `153f7532…`: задача №3 → 1 обращение, №7 → 2, агрегат 96%), но не доезжали до фронта. Исправлено + **гард smoke-check §22** (любой скоринговый SELECT обязан тянуть `best_earned_score`; проверено, что гард ловит именно ту ошибку). Плюс по решениям владельца: «≈»-оценка вместо «—» для работ до 26.07 (`resolveIndependencePct`), метрика в ячейке таблицы второй строкой и в мини-карточках разбора, итоги в списке ДЗ ученика (новый роут `GET /student/assignments/scores`), самостоятельность по работе в отчёте родителю. Введён единый `HwTaskStateRow` вместо трёх разошедшихся копий типа. Валидация: `tsc -p tsconfig.app.json` — 13 pre-existing (0 новых), `vite build`, `smoke-check` 22/22, `esbuild` 5 edge.
>
> **Статус 2026-07-25 (третий заход — Т7 + ревью UX):** реализована метрика «% самостоятельности» (миграции `20260725140000` + `20260725150000`), отменена деградация балла, балл задачи = лучший за все попытки. Внешнее UX/перф-ревью (ChatGPT-5.6) дало FAIL с 4×P1 + 4×P2 — **все закрыты** коммитом `a71d1b4`: тексты сбоя («это технический сбой, а не оценка твоего решения», без «баллы не списаны»), объяснение метрики стало ВИДИМЫМ вместо `title` (у ученика чип-кнопка с раскрытием, у репетитора легенда), заголовок карточки ДЗ не переполняется на 360px, чип «Самостоятельная · без подсказок» на ученических поверхностях; контраст, `role="status"`, единая терминология «Самост.», `Promise.all` вместо последовательных RPC. **Не выполнено (нужен владелец): прогон критического пути на реальном iPhone iOS 15/16** — условие ревьюера для выката на пилот; автоэмуляции старого WebKit в проекте нет (rule 80).
>
