...
| TASK-11 | «Последние действия учеников» v2.0 — event feed: фикс ложного «Открыл задачу №N» (genuine-activity filter) + реальный сигнал `student_opened_at` + 5 видов (opened/wrote/submitted/completed/stuck) + ChatGPT-5.5 review round-2 | ✅ done | P1 bugfix | Claude Code | migration `20260601120000`, `homework-api/index.ts` (`handleGetStudentProblem`+`handleGetRecentDialogs`), `tutorHomeworkApi.ts`, `useTutorRecentDialogs.ts`, `ChatRow.tsx`, `RecentDialogsBlock.tsx` | AC-V1..AC-V5 (детали: `phase-1-follow-up-recent-dialogs.md` §8) |
