...
**Status:** In progress (TASK-1..10 ✅ done, REVIEW pending)
...
| TASK-10 | «Активность учеников» group-by-group rendering: новый Segment режим `groups` (default), заголовки + счётчики групп, «Без группы» в конце | ✅ done | P1 | Claude Code | `useTutorStudentActivity.ts` (fetch groups+memberships), `StudentsActivityBlock.tsx` (GroupRowsFragment + grouping logic + FolderTree icon + default sort), `tutor-dashboard.css` (group-header-row styling) | AC-G1..AC-G5 |
| TASK-11 | «Последние действия учеников» v2.0 — event feed: фикс ложного «Открыл задачу №N» (genuine-activity filter) + реальный сигнал `student_opened_at` + 5 видов (opened/wrote/submitted/completed/stuck) + ChatGPT-5.5 review round-2 + UX round-3 (тёплый «застрял» → Сократ-мишка/янтарь, anti-dup чип↔preview) | ✅ done | P1 bugfix | Claude Code | migration `20260601120000`, `homework-api/index.ts` (`handleGetStudentProblem`+`handleGetRecentDialogs`), `tutorHomeworkApi.ts`, `useTutorRecentDialogs.ts`, `ChatRow.tsx`, `RecentDialogsBlock.tsx`, `SokratBearIcon.tsx` | AC-V1..AC-V5 (детали: `phase-1-follow-up-recent-dialogs.md` §8) |
...
- [x] TASK-10 ✅ done (2026-04-22) — group-by-group rendering в «Активность учеников» (новый Segment режим `groups`, default при наличии групп у репетитора)
