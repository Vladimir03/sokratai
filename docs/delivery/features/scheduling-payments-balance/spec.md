...
>
> **Changelog:**
> - **v13 (2026-07-13)** — **фикс периода «Отчёта родителю»** (репорт Елены: показывал «с 1 по 30 июня» с концом В БУДУЩЕМ вместо «по сегодняшний день»). Решения владельца (AskUserQuestion): период = **снимок** (фиксируется при создании); пресет «последний месяц» → **текущий месяц с 1-го по сегодня**; + **свой диапазон**. Реализация (`ParentReportDialog`): `last_month` = `startOfMonth(now) … сегодня` (конец всегда ≤ сегодня, не в будущее; было — последние 30 дней) + новый `period_kind='custom'` (поля «с … по …», Safari-safe календарь-поповер `DateField` mirror `DateTimeField`, кламп конца ≤ сегодня, валидация `start > end` **после** клампа — иначе две одинаковые будущие даты давали перевёрнутый диапазон, P2 ChatGPT-5.6). Компактная подпись одного месяца («за 1–13 июня 2026», `PublicStudentReport`). Миграция `20260713160000` (CHECK `period_kind IN ('all','last_month','custom')`, аддитивно/идемпотентно). **edge/anti-leak/снимок НЕ тронуты** (edge читает `period_start/end` с ссылки как раньше; `custom` — просто строка в payload → deploy-skew-safe). ChatGPT-5.6: CONDITIONAL → PASS (P2 закрыт). build+tsc+eslint+smoke+RU-grep чисто. Деплой: миграция ПЕРВОЙ (Lovable) → `deploy-sokratai` (**редеплой edge не нужен**).
