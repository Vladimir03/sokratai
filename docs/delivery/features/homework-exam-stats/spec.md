...
Для этого система должна перестать терять экзаменную metadata задачи при переносе из KB в ДЗ, научиться хранить её на уровне `homework_tutor_tasks`, агрегировать результаты одинаково для classic и guided chat и отдавать на фронт exam-oriented response: `№ задания → балл → статус → изменение vs предыдущий результат этого же ученика`.
...
- Guided chat и classic mode считаются разными путями данных, поэтому итоговая аналитика остаётся общей и «внутренней», а не экзаменно-ориентированной.
...
- Унифицировать подсчёт для `classic` и `guided_chat`.
...
- **AC-5:** Для `classic` и `guided_chat` одна и та же exam summary строится из разных источников (`submission_items` и `task_states`), но фронт получает единый exam-oriented response shape.
...
- `supabase/functions/homework-api/index.ts` — расширить create/update/get details/get results; добавить helper нормализации classic/guided task results; optional endpoint student history (P1)
...
Правило расчёта `earned_score`:
- использовать `ai_score`, если он уже установлен;
- для reviewed submissions учитывать tutor-overridden score, если он записан;
- статус брать из submission/status item-комбинации, не по guessed heuristics.
...
| Guided chat и classic будут считать `earned_score` по-разному | Высокая | Ввести общий внутренний `TaskResultFact` helper и тестировать оба источника на одинаковый response contract |
...
- [ ] TASK-4: homework-api aggregation — unify classic + guided into `TaskResultFact` and build exam summary
...
- [ ] TASK-7: README + QA — update `homework-api` docs and validate both classic/guided exam-tagged paths
