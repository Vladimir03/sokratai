...
Реализационные задачи для `spec.md` (`docs/delivery/features/voice-speaking-mvp/spec.md`, **v0.3**).
...
>
> **AC-нотация:** spec.md v0.3 не имеет нумерованных AC (это draft) — каждая задача ниже
...
- **AC (Given/When/Then):**
  ...
  - *(B, P0 playback)* *Given* запись завершена, *When* до нажатия «Отправить», *Then* доступно **прослушивание своей записи** + «Перезаписать»; отправить вслепую нельзя.
  - *(B, P0 transcript)* *Given* speaking сдан и оценён, *When* ученик смотрит разбор, *Then* виден **транскрипт** под меткой «Распознанная речь» рядом с таблицей критериев (не сырой user-бабл).
  - *(B, P1 progress)* *Given* ученик нажал «Отправить», *When* идёт STT+grading, *Then* двухфазный индикатор «Распознаю речь… → Проверяю по критериям…», не фриз.
  - *(B, P1 cap)* *Given* запись достигла ~6:00, *When* идёт запись, *Then* предупреждение «осталась минута»; хард-стоп на 7:00.
- **Что делаем:**
  ...
  - **(B)** В `HomeworkProblem` composer добавить ветку `task_kind==='speaking'`: рекордер на **существующем** `useVoiceRecorder` (iOS Safari m4a/webm уже решён — НЕ вводить новый, `.claude/rules/80-cross-browser.md`). Upload через voice-helper (новый `uploadStudentThreadVoice` или generalize `uploadStudentThreadImage`) → `voice_ref` → submission. Скрыть numeric-row (как humanities §18).
    - **[P0] Playback-before-submit:** после остановки записи — локальный `<audio>` плеер на blob (URL.createObjectURL, revoke на unmount/re-record) + кнопка «Перезаписать». Primary CTA «Отправить» активна только после записи.
    - **[P0] Транскрипт ученику:** транскрипт (= `submission_payload`-content) рендерить в разборе под меткой «Распознанная речь», над `CriteriaBreakdownTable`. Уже приходит в thread как content сообщения — нужен лейбл, не raw bubble.
    - **[P1] Двухфазный прогресс:** optimistic UI после submit — сначала «Распознаю речь…», затем (по resolve) «Проверяю по критериям…» (mirror existing TypingDots, §16 optimistic pattern).
    - **[P1] Cap + warning:** хард-кап **7 мин** + size cap (= `MAX_VOICE_BYTES` TASK-7); предупреждение на ~6:00. `useVoiceRecorder` уже даёт `recordingDurationSeconds` — порог в consuming-компоненте, не в хуке.
- **Guardrails:** только `useVoiceRecorder`, никаких новых MediaRecorder-обёрток. `URL.revokeObjectURL` на cleanup (memory leak). `touch-action: manipulation` на контролах рекордера. `task_kind` union в `homework.ts` — additive optional (не сломать существующие 'numeric'|'extended'|'proof').
...
- **AC (Given/When/Then):**
  - *Given* ученик сдал speaking-задачу, *When* репетитор открывает тред, *Then* на 1 странице: **нативный `<audio controls>`** + транскрипт + таблица критериев (компонент TASK-4) + «Изменить балл».
  - *Given* RU-юзер, *When* плеер грузит аудио, *Then* URL через `rewriteToProxy()` (browser-facing), аудио играет (не `*.supabase.co`).
- **Что делаем:** для speaking-submission рендерить **нативный `<audio controls>`** (не кастомный play/pause — нативный даёт scrubbing + скорость 1.5× бесплатно, это ровно то, что нужно для прослушивания 5-7 мин фонетики), транскрипт под ним, переиспользовать `CriteriaBreakdownTable` (TASK-4) + existing override (§6). Аудио signed URL → `rewriteToProxy()`. Всё на 1 странице.
- **Guardrails:** тайм-коды «клик на момент» / голос-коммент репетитора — **OUT** (Spec §3). `<audio>` с `preload="metadata"` (не грузить весь файл до play). `touch-action: manipulation`. Не дублировать `CriteriaBreakdownTable`.
...
> Каждый промпт самодостаточен. Перед запуском любого — агент читает `spec.md v0.3` и соответствующий раздел `tasks.md`. После любой задачи: `npm run build && npm run smoke-check` (lint — если затронут TS).

### TASK-0 (pre-flight) — Claude Code

```
Прочитай spec.md v0.3 + tasks.md TASK-0. НЕ пиши код. Изучи и зафиксируй точки переиспользования:
...
```
Прочитай spec.md v0.3 (§5) + tasks.md TASK-1 + CLAUDE.md §23 (GRANT-whitelist на homework_tutor_task_states).
...
```
Прочитай spec.md v0.3 (§3,§5) + tasks.md TASK-2 + CLAUDE.md §19 (subject-rubric layer).
...
```
Прочитай spec.md v0.3 (§3,§5) + tasks.md TASK-3 + CLAUDE.md §12 (как elements_check устроен в mock-exam-grade), §16 (runStudentAnswerGrading — единый источник), §6 (шаг 0.1), §27 (humanities leak skip).
...
```
Прочитай spec.md v0.3 (§6) + tasks.md TASK-4 + .claude/rules/90-design-system.md.
...
> Перед кодом прочитай: docs/delivery/features/voice-speaking-mvp/spec.md v0.3 + соответствующий
...
Прочитай: spec.md v0.3 §5 (Миграции) + tasks.md TASK-6 + образец feature-флага (миграция
...
```
[+ Преамбула + Mandatory end block]
Прочитай: spec.md v0.3 §3,§5 (Subject→language map) + tasks.md TASK-7. Изучи существующий Groq Whisper:
...
```
[+ Преамбула + Mandatory end block]
Прочитай: spec.md v0.3 §3,§5 (API) + tasks.md TASK-8 + CLAUDE.md §0 (двойной derive task_kind — 4 backend
...
```
[+ Преамбула + Mandatory end block]
Прочитай: spec.md v0.3 §3 шаг 1-2, §6 + tasks.md TASK-9 + .claude/rules/80-cross-browser.md (useVoiceRecorder,
...
- [P0] Playback-before-submit: после stop — локальный <audio> на blob (URL.createObjectURL + revoke на
  unmount/re-record) + «Перезаписать». «Отправить» активна только после записи. Отправка вслепую запрещена.
- [P0] Транскрипт ученику: в разборе под меткой «Распознанная речь» (над CriteriaBreakdownTable) — это
  content сообщения submission, нужен лейбл, не raw user-bubble.
- [P1] Двухфазный прогресс после submit: «Распознаю речь… → Проверяю по критериям…» (mirror TypingDots, §16).
- [P1] Cap 7 мин + warning на ~6:00 (recordingDurationSeconds из useVoiceRecorder — порог в компоненте) +
  size cap = MAX_VOICE_BYTES (TASK-7).
- Upload аудио (новый uploadStudentThreadVoice или generalize uploadStudentThreadImage) → voice_ref → submission.

AC: tutor «устный» → 'speaking' в обе write-path; ученик на iPhone Safari видит рекордер (не numeric), один CTA;
прослушал ДО отправки + re-record; после оценки видит транскрипт «Распознанная речь» + критерии; двухфазный прогресс.
Guardrails: только useVoiceRecorder. URL.revokeObjectURL на cleanup. touch-action: manipulation. После merge —
🚀 Deploy needed. Manual: iPhone Safari + Android Chrome (запись + playback + отправка).
```
...
```
[+ Преамбула + Mandatory end block]
Прочитай: spec.md v0.3 §6 + tasks.md TASK-10 + CLAUDE.md §35 (CriteriaBreakdownTable) + RU-bypass
...
НАТИВНЫЙ <audio controls> (НЕ кастомный play/pause — нативный даёт scrubbing + скорость 1.5× бесплатно,
это нужно для прослушивания 5-7 мин фонетики) + транскрипт под ним + CriteriaBreakdownTable (компонент
TASK-4, НЕ дублируй) + кнопку «Изменить балл» (override §6). Аудио signed URL через rewriteToProxy() (browser-facing).

AC: репетитор открывает тред speaking-задачи → audio controls со scrubbing/скоростью, видит транскрипт +
таблицу критериев + правит балл; RU-юзер → аудио играет (URL не *.supabase.co).
Guardrails: тайм-коды «клик на момент» / голос-коммент репетитора — OUT (Spec §3). <audio preload="metadata">,
...
3. docs/delivery/features/voice-speaking-mvp/spec.md v0.3 + tasks.md (ревьюируемая задача)
...
- UX P0: playback-before-submit (ученик не отправляет вслепую, URL.revokeObjectURL на cleanup)?
  транскрипт виден ученику под меткой «Распознанная речь»? репетитор — нативный <audio controls> (scrub+speed)?
- UX P1: двухфазный прогресс STT→grading (не фриз)? warning на ~6:00 + хард-кап 7 мин?
