...
- **Что делаем:** таблица-ревью на 20+ задач, «Принять все», массовая тема/источник; ~~`kb_insert_tasks_batch(jsonb)`~~ батч-RPC ОТКЛОНЁН (rule 40/50 single write-site).
- **Статус: SHIPPED 2026-07-12 (волна 2).** `DraftTable` (desktop-default, expand-row = DraftCard) + `BulkActionsBar` + review-модель `ReviewOverrides` (пре-резолв темы при входе в ревью) + `useCreateTasksBulk` (цикл по `insertTask`, параллельность 3, per-row статусы, «Повторить неудачные»). Инварианты → rule 50 «Волна 2».
...
- **Статус: v1 SHIPPED 2026-07-12 (волна 2)** как per-draft «Переспросить AI» (`mode:'refine'` в той же edge, комментарий ≤2000, мерж только контент-полей). Мерж/сплит нескольких карточек одной репликой — не реализован (следующая итерация).
