...
- **Follow-up fix (post-pilot, 2026-05-08):** боевое использование вскрыло два дефекта, оба чинятся в одном PR.
  1. **«Сохранить» disabled когда tutor хочет поставить балл, равный AI.** Префилл `valueText` шёл из `aiScore` first, и `isUnchanged` сравнивал ввод с `aiScore`. При AI=1, earned=0.8, override=null репетитор не мог зафиксировать override=1 (это был *real* change — earned/AI приоритет создавал спред 0.8 vs 1, override нужен чтобы зафиксировать 1). Фикс: префилл `currentOverride ?? finalScore ?? aiScore`; `isUnchanged` сравнивает только с `currentOverride` (когда override отсутствует — любое сохранение это новая строка, не no-op).
  2. **Заголовок диалога вводил в заблуждение.** Показывал только `AI: 1/1`, скрывая что фактически отображается `0.8/1` (earned_score деградация). Фикс: новый блок «Текущий балл: X/Y (AI: Z/Y, снижено на Δ за подсказки/неверные попытки)» + AI-комментарий (`ai_score_comment`) когда есть.
- **Дополнительные UX-расширения (post-pilot, 2026-05-08, тот же PR):**
  - **A — кнопка «Изменить балл» в `GuidedThreadViewer`** (новый primary entry point). Под header'ом задачи блок: `Балл: X/Y · AI: Z/Y · [chip ручная правка] · [Pencil] Изменить балл`. Открывает тот же `EditScoreDialog`. Pencil-icon-only на мобиле, с подписью на десктопе. Это был самый запрошенный ER из пилота — карандашик 12px на TaskMiniCard оказался необнаружимым.
  - **C — ученик видит обе оценки + комментарий тутора.** TaskStepper tooltip: при override → `Балл репетитора: X/Y` (главное) + `AI: Z/Y` + публичный комментарий в плашке. Completed view: итог теперь считается через `final_score` (а не `earned_score`), и отдельная секция «Правки репетитора» рендерит per-task разбор для мобильных пользователей (tooltip недоступен).
- **Backend additive:** `THREAD_SELECT` расширен полями `tutor_score_override`, `tutor_score_override_comment`, `tutor_score_override_at`. `handleGetResults.task_scores[*]` теперь содержит `ai_score`, `ai_score_comment`, `tutor_score_override`, `tutor_score_override_comment`. `stripStudentSensitiveTaskStateFields` продолжает удалять только `ai_score_comment` (tutor-only); все три override-поля **намеренно** идут к ученику per UX answer C.
- **Что НЕ сделано из исходных UX-вопросов:** D (Pencil overlay на основной HeatmapGrid) — отказ; B (lazy-create task_state для не приступавших) — оставлен 404; E/F/G/H (bulk / co-teachers / понижение enforcement / audit log) — отдельные итерации.
- **Follow-up #2 (post-pilot, 2026-05-09):** ещё два бага вылезли через день после `16cd70b`.
  1. **Override на незавершённой задаче не виден в HeatmapGrid и TaskMiniCard.** Guard в `handleGetResults` (`supabase/functions/homework-api/index.ts:2655–2656`) пропускал task_states с `status='active'` через `continue`, не проверяя `tutor_score_override` и `ai_score`. Override сохранялся в БД и читался `GuidedThreadViewer`, но frontend cellMap не получал клетку. Фикс — расширенный guard: skip только когда thread активен И ни один scoring-сигнал (completed / override / ai_score) не присутствует. Контракт: `.claude/rules/40-homework-system.md` → «Heatmap cell inclusion invariant».
  2. **Шаг override = 0.5, шаг AI prompt = 0.5 — пользователь хочет parity 0.1.** DB `numeric(5,2)` поддерживает 0.1, миграция не нужна. Изменения в 5 точках: `homework-api/index.ts:2957–2958` (validator), `guided_ai.ts:949` (AI prompt), `EditScoreDialog.tsx` × 3 (валидация, `<input step>`, hint). Контракт: `.claude/rules/40-homework-system.md` → «Score step invariant». Backward compat — старые 0.5-стэппед записи валидны (0.5 ∈ multiples of 0.1).
- AC-5d / AC-5e в `spec.md`. Файлов тронуто: 3 кода + 4 документа.
