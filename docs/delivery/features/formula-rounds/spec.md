...
### Текущее поведение
Homework flow поддерживает два workflow: `classic` (задачи + ответы + AI-проверка) и `guided_chat` (пошаговый разбор). Оба ориентированы на полноценные задачи. Нет лёгкого micro-drill для повторения формул.
