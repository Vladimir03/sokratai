...
3. **Только guided chat**: classic mode не поддерживает enforcement (и планируется к удалению)
4. **Default `short_answer`**: наименее ограничительный, не ломает текущее поведение
5. **Fallback `max_score: 1`**: если `primary_score = null` в KB — текущее поведение сохраняется
