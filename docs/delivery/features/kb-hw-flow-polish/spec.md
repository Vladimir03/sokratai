...
**Статус:** P0 + P1 implemented, pending Lovable preview QA for latest tutor edit-flow fixes
...
5. Edit-flow существующего ДЗ не должен повторно гонять reorder без изменения задач: сохранение идёт по dirty sections, новые ученики в edit-mode добавляются append-only и получают notify только для себя.
...
- **P1-1**: `HWTaskCard.RubricField` — в collapsible «Критерии проверки» добавляем read-only блок «Эталонное решение (из БЗ)» — рендерится только если `task.kb_snapshot_solution` не пуст ИЛИ `kb_solution_image_refs.length > 0`. Текст через `MathText`, фото — как read-only thumbnails с click-to-zoom (без удаления/добавления). Ученик и AI это поле не видят. Реализовано.
- **P1-2**: `HWTaskCard` шапка — рядом с `SourceBadge` показываем `source_label` как маленький серый текст (`text-xs text-muted-foreground`). Только для репетитора (карточка вся — tutor surface). Read-only. Заодно отображаем в `HWDrawer` в шапке карточки. Реализовано.
- **P1-3**: AI-bootstrap по умолчанию OFF — меняем fallback с `?? false` на `?? true` в edit-mode load и template-apply. Реализовано.
- **P1-4**: tutor homework edit-flow hardening — реализовано:
  - `PUT /assignments/:id` receives `tasks` only when `tasksDirty`
  - `POST /assignments/:id/notify` supports optional `student_ids`
  - existing assigned students stay append-only in edit-mode
  - reorder failures surface as `TASK_REORDER_FAILED`
  - KB provenance round-trips through edit-mode (`kb_task_id`, `kb_snapshot_text`, `kb_snapshot_answer`, `kb_snapshot_edited`) and is preserved after edit-save
...
- **P1**: extend `GET /assignments/:id` response. Каждая задача получает optional поля:
  ```ts
  kb_task_id?: string | null;                     // from homework_kb_tasks.task_id
  kb_snapshot_text?: string | null;              // from homework_kb_tasks.task_text_snapshot
  kb_snapshot_answer?: string | null;            // from homework_kb_tasks.task_answer_snapshot
  kb_snapshot_solution?: string | null;           // from homework_kb_tasks.task_solution_snapshot
  kb_snapshot_edited?: boolean;                   // from homework_kb_tasks.snapshot_edited
  ...
  ```
  Fallback: если исходная задача удалена из КБ (`kb_task_id` не существует в `kb_tasks`) — snapshot-поля берём из `homework_kb_tasks`, derived-поля из `kb_tasks` = `null`.
...
- **AC-11** *(P1-4)*: если tutor в edit-mode меняет только title/deadline/materials или добавляет учеников, `PUT /assignments/:id` не отправляет `tasks`, reorder RPC не вызывается, а новые ученики получают notify только по `student_ids`.
- **AC-12** *(P1-4)*: после edit-save существующего ДЗ с KB-задачами у задач сохраняются `source_label`, `kb_snapshot_solution` и read-only provenance блок; edit не стирает исходные KB links.
...
**Open manual checks before full pilot rollout:**
