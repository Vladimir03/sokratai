...
- **AC-P6 (F6):** В задачах Часть 1 где требуется ответ с единицами измерения (Дж, Кл, мкВ и т.д.), в `task_text` явно указано «Ответ дайте в …». Проверяется визуально на 5 задачах Часть 1 (KIM с известными требованиями к единицам).
- **AC-P7 (F7):** В задачах на соответствие (KIM 6 и любая другая в варианте 1) в `task_text` видны структурированные данные (markdown таблица / структурированный список) с двумя столбцами / списками. Ученик может прочитать соответствие.
...
- [ ] TASK-6 (F6+F7): Update `variant1-tasks.json` — добавить единицы в текст условий Часть 1 + markdown-таблицы в задачах на соответствие. Регенерация seed через `scripts/build-mock-exam-seed.py`. Новая миграция `20260514120000_resync_mock_exam_variant_1_content.sql` — idempotent UPDATE per task_id
