...
| AC-P11 (Часть 1 drill-down + tutor_comment + AC-P9 auto-migration) | LANDED 2026-05-26 → APPROVED после hotfix `809d836` (ChatGPT-5.5 review) | Trigger от репетитора через Володю: «ткнул в ячейку Часть 1, провалился в задачу + оставил комментарий ученику». Plus AC-P9 partial credit не применялся для form-mode pilot data Егора (был submitted ДО `2f76adf`). Решение: (1) migration `20260526110000` добавляет `tutor_comment TEXT NULL` колонку; (2) migration `20260526120000` PL/pgSQL one-time backfill пересчитывает все existing form-mode и OCR rows с partial credit logic (preserves score_source='tutor'); (3) новый `Part1TaskDrillDownDialog` component — modal с условием задачи, ответом ученика, правильным ответом, score override, comment textarea, collapsible «Показать условие»; (4) `Part1SummaryCard` form mode — collapsible rows list + click → drill-down + universal recheck button; (5) `Part1BlankReviewPanel` blank mode — 🔍 icon next to inline grid input → drill-down dialog; (6) StudentMockExamResult Part1Card — sky comment rows под balance rows. **Hotfixes 2026-05-26** (8 issues от ChatGPT-5.5 review): H1 [P0] separate commit `cd1257b` — paused status больше не утечёт correct_answer (AC-P10 + AC-P11 interaction); H2-H7 commit `809d836` — default_exam_mode persist (AC-P10 Phase 2), drill-down read-only на approved, signed URL для task_image_url, server-side total_part1_score recompute, parity doc-comment в migration, touch target h-9 w-9. См. CLAUDE.md §15a «Часть 1 drill-down + tutor_comment + AC-P9 auto-migration» + «AC-P11 hotfixes». **Follow-up bug 2026-06-01** (Вадим/Елена): recheck «Применить критерии ФИПИ» на `approved` attempt поднимал `total_part1_score` через partial credit, но не пересинхронизировал stored `total_score` → ИТОГО застывал (19 вместо 21 = part1 19 + part2 2). Fix: resync `total_score = part1 + part2` во всех 3 part1-write-handler'ах (recheck/manual-score/finalize) + backfill migration `20260601140000`. Invariant в rule 45 («`total_score` is STORED, not derived»). Сопутно: лимит фото Часть 2 поднят 7 → 10. |
