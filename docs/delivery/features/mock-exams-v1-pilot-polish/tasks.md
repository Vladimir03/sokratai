...
**Agent:** Claude Code
**Files (landed):**
- `src/hooks/useMockExamAssignment.ts` — extracted `POLLING_ATTEMPT_STATUSES: ReadonlySet<MockExamAttemptStatus>` (изначально {submitted, ai_checking, awaiting_review}; **review fix 2026-05-14: добавлен `in_progress`**) + helper `hasPollingActiveAttempts(detail)`; conditional `refetchInterval` callback returns `30_000` ms когда non-terminal attempts present, иначе fallback на существующий `getTutorBackgroundRefetchInterval`; добавлен debug `useEffect` с ref-sentinel `wasPollingActiveRef` для one-shot `console.info('[mock-exam-detail-polling] active=true', { assignment_id, awaiting_count })` лога при OFF→ON transition. Сохранены: queryKey, staleTime, gcTime, retry, retryDelay, refetchOnWindowFocus, refetchOnReconnect.

**Review fix 2026-05-14 (Codex):** изначально `POLLING_ATTEMPT_STATUSES` не содержал `in_progress` — это пропускало сценарий «tutor открыл страницу пока ученик ещё пишет работу, ученик сдал → tutor не видит submit до window-focus». Codex flagged как AC-P2 FAIL. Fix: добавлен `'in_progress'` в Set. Теперь polling активен пока есть attempts в любом non-terminal статусе, выключается только при `{approved, manually_entered}` или отсутствии attempts вовсе. Нагрузка чуть выше (любой active attempt → polling), но это правильная семантика для AC-P2.
