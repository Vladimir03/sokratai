...
| Sprint 1 UX polish | ✅ Done | `53348f7` | Follow-up на основе pilot feedback + двух раундов code review (Claude Code + GPT-5.4): (a) preview toolbar «Поделиться ссылкой» теперь открывает реальный `ShareLinkDialog` через lazy+Suspense+conditional mount (ранее — `toast.info` stub); (b) русификация ShareLinkDialog + SaveAsTemplateDialog (english-термины «read-only» / «correct_answer» / «rubric_text» / «disable_ai_bootstrap» заменены; названия клавиш Enter/Backspace оставлены); (c) 48×48 thumbnails + multi-photo бейдж в `SaveTasksToKBDialog` через batched `useKBImagesSignedUrls` (один hook-call на весь dialog, не per-task N+1); (d) AC-12 anti-leak сохранён — rubric_* в thumbnail payload не тянется, в тексте диалога более мягкая формулировка про рубрику; (e) rename навигационных меток «База знаний» → «База задач» (SideNav + KnowledgeBaseFrame + KBPickerSheet, 3 места); (f) inline rename шаблонов в `TutorHomeworkTemplates.tsx` — click по заголовку → Input, Enter/blur сохраняют через `PATCH /templates/:id` (frontend шлёт ТОЛЬКО `{ title }`, backend hard-whitelist `title/tags/topic` не трогали); (g) **double-PATCH fix**: `commitInFlightRef: useRef<boolean>` отсекает повторный вызов `commitRename` от `onBlur` (браузер блюрит Input когда `saving` триггерит `disabled={true}`; useRef читается синхронно, useState не помог бы); (h) `TemplateCard` обёрнут в `React.memo` per rule performance.md (list-item защита от unrelated parent re-renders). |
