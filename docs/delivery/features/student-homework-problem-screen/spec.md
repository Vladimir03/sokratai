...
- **AC-15 (P0, new 2026-05-11 preview-QA #10 — codex review fix):** Step progress / next-task правильность. `homework_tutor_task_states(...)` subselect в `THREAD_SELECT` обязательно включает `task_order` field. Без него `HomeworkProblem` sort + done-indices читают `undefined` → completed circles белые, «Следующая задача» target random.
