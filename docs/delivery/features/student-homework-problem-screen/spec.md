...
**Phase 3.1 hotfixes (✅ landed 2026-05-13):** 3 пилот-QA баги после Phase 3 deploy: (a) graceful 401 handling в `requestStudentHomeworkApi` — refresh+retry на 401, на persistent failure auto-redirect через `signOut()` + `AuthGuard.onAuthStateChange`; (b) `task_kind`/`check_format` sync — 4 backend write-paths + HWDrawer client INSERT теперь пишут оба поля через `deriveTaskKind` / `deriveTaskKindFromCheckFormat`, backfill migration `20260513120000_resync_task_kind_from_check_format.sql` исправила existing affected rows; (c) Ctrl+V paste в SubmitSheet (`onPaste` на `DialogContent`, dual `clipboardData.files`/`items.getAsFile()` для Chrome/Safari/Firefox). Commits `c879b29` / `ca1ed1c` / `52681af`. Контракт в `.claude/rules/40-homework-system.md` → секция «Phase 3.1 hotfixes».

**Phase 4 cutover (отдельная спека `student-homework-problem-cutover.md`, partially landed):** Phase 3 уже включил universal redirect (часть scope Phase 4 landed раньше). Осталось: физически удалить `GuidedHomeworkWorkspace.tsx`, `GuidedChatInput.tsx`, `TaskStepper.tsx` (после Phase 3 stable ≥7 дней) + grep tutor-side `GuidedThreadViewer` callsite'ы, если есть пересечения. NB: `GuidedChatInput.tsx` ещё используется как reference для paste handler в Bug #2 fix — удалять рано.
