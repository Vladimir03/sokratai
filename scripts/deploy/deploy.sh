...
# Отдельный маркер «канарейка хоть раз взводилась». ОБЯЗАН быть отдельным от
# POOL_STATE: тот появился на деплое РАНЬШЕ, чем в скрипт добавили канарейку, и
# первый же запуск с канарейкой увидел «пул наш, а канарейки нет» и закричал
# — при том что retention в том же прогоне отработал идеально (прошлый entry
# отдавал 200, пул вырос 8.7M → 13M). Ложная тревога 2026-07-27, ровно тот
# случай, против которого написано «шумный алерт = выключённый алерт».
#
# Живёт в $SNAPSHOTS, а не в $ASSETS: если бы он лежал в пуле, вычистка пула
# унесла бы и улику, и свидетеля разом.
CANARY_ARMED="$SNAPSHOTS/.canary-armed"
RETENTION_DAYS=30
KEEP_SNAPSHOTS=10
...
if [ -f "$CANARY_ARMED" ] && [ ! -f "$CANARY" ]; then
