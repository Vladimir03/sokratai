...
**Deploy:** a frontend change does **not** reach prod automatically. After touching `src/**`, `index.html`, `package.json`, `vite.config.ts`, `tailwind.config.ts`, or `public/**`, end your final message with a **"🚀 Deploy needed"** block (rule 95): `ssh … 185.161.65.182 && deploy-sokratai`. **Migrations and edge functions do NOT auto-deploy** (corrected 2026-07-27 — the previous claim here was false). Lovable's sync pulls *code*; applying migrations and deploying functions is a separate, explicit step through the Lovable agent — and it is the **only** path: the prod Supabase project belongs to Lovable's organisation, not the owner's, so no token of ours can deploy functions to it (403, confirmed twice). The GitHub workflow was deleted for that reason; don't recreate it. After pushing an edge function, **verify**: `curl -X OPTIONS https://api.sokratai.ru/functions/v1/<name>` → 404 = not deployed, 503 = boot-fail, 401/200 = live. (rule 96 #11)

---

## CRITICAL — cross-cutting invariants

- **Словарь предметов — ЕДИНЫЙ РЕЕСТР `src/lib/subjects/registry.ts` (2026-07-23).** Он был продублирован ≥9 раз (`SUBJECTS`, `KB_SUBJECTS`, дательный падеж, humanities-множество ×4, `VALID_SUBJECTS_*`, `SUBJECT_LABELS_DENO` ×2, CHECK'и БД), и расхождение с CHECK'ами два месяца ТИХО ломало сохранение шаблонов у 13 репетиторов. Теперь: фронт — производные от реестра; edge — **сгенерированный** `_shared/subjects.generated.ts` (`npm run generate:subjects`, файл коммитится); БД — CHECK'и. **Добавляешь предмет:** строка в реестре → `npm run generate:subjects` → миграция на **ОБА** CHECK'а (`homework_tutor_assignments` + `homework_tutor_templates`) → `npm run smoke-check`. Гарды: §7 (реестр = зеркало), §19 (зеркало не устарело), §17 (`check-prod-schema.mjs`, сверка с CHECK'ами ПРОДА — `homework_tutor_assignments`, `homework_tutor_templates` **и** `mock_exam_variants`; fail-closed в CI-job `prod-schema-guard`). Пропуск миграции = ДЗ/шаблоны/варианты пробников этого предмета молча не сохраняются. **Языковой предикат — только `subjectRequiresCefr` (фронт) / `SUBJECTS_REQUIRING_CEFR` (Deno)**, никаких локальных `['french','english','spanish']`: расхождение блокирует создание ДЗ (бэкенд требует CEFR, форма поля не показывает). (rule 40)
...
- **Markdown / regex:** never import `remark-gfm` — only `@/lib/markdown/remarkGfmSafe` (autolink-literal carries a lookbehind → «Invalid regular expression» kills the screen on Safari < 16.4); wrap chat `ReactMarkdown` in `MarkdownErrorBoundary`. Lookbehind is banned in **any** client code and is smoke-check-enforced in `src/` **and** `dist/assets` (catches deps). Baseline = Safari 15 / iOS 15. (rule 80)
...
- **AI usage logging (observability):** any new AI-gateway path should thread `onUsage` (via `_shared/token-usage.ts::makeUsageLogger`) into its `callLovableJson` and add a `source` to `TokenUsageSource` — so token cost is sliceable by type + tutor (`token_usage_logs`). Fire-and-forget, no PII; new `source` needs **no** migration (plain `text`). New voice path → `transcribeAudio(..., logContext)`. **Кэш-хит — только через `pickCachedTokens(usage)`** (форм у поля 4; своя копия молча даст вечные NULL), и `cached_tokens = NULL` ≠ `0`: NULL — шлюз не прислал, 0 — кэш не сработал; сольёшь — замер бессмыслен. Spec: `docs/delivery/features/ai-usage-logging/spec.md`, план оптимизации: `docs/delivery/features/ai-request-optimization/research.md`.
